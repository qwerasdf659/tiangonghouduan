...
async function testMaterialSystemAPIs() {
  try {
    ...
    if (assetTypesRes.data.data && assetTypesRes.data.data.length > 0) {
      console.log(
        `   示例: ${assetTypesRes.data.data[0].asset_code} - ${assetTypesRes.data.data[0].display_name}`
      )
    }
    console.log('')
    ...
    if (rulesRes.data.data && rulesRes.data.data.length > 0) {
      ...
      console.log(
        `   示例: ${rule.from_asset_code}(${rule.from_amount}) → ${rule.to_asset_code}(${rule.to_amount})`
      )
    }
    console.log('')
    ...
    const userRulesRes = await axios.get(`${BASE_URL}/api/v4/material/conversion-rules`, {
      headers
    })
