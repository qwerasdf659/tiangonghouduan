# 🔌 接口对接规范文档 - 餐厅积分抽奖系统v2.0

> **前后端API接口对接完整规范** - 基于v2.0多业务线分层存储架构的权威接口文档
...
- **基于代码**：实际运行的v2.0架构代码（app.js + 5个v2路由模块）
...
> **注意**：当前v2.0架构暂未实现独立的认证接口，认证逻辑需要配合用户管理系统实现。以下为设计规范：
...
```json
{
  ...
  "consumptionAmount": 156.80      // 确认消费金额（可选）
}
```
...
> 本文档基于v2.0多业务线分层存储架构的实际API实现，确保接口文档与后端代码完全一致，为前后端对接提供准确可靠的技术参考。 