...
class PermissionService {
  ...
  async checkPremiumSpaceStatus (userId) {
    try {
      ...
      // 检查是否满足解锁条件
      const canUnlock = user.total_points >= this.premiumSpaceConfig.requiredCumulativePoints &&
                        user.available_points >= this.premiumSpaceConfig.unlockCostPoints

      return {
        hasPermission: accessRecord.is_unlocked && !isExpired,
        isExpired: isExpired || false,
        expiresAt: accessRecord.expiry_time ? moment(accessRecord.expiry_time).format('YYYY-MM-DD HH:mm:ss') : null,
        remainingTime,
  ...
  async renewPremiumSpace (userId, renewType = '24hours', confirmSpend) {
    ...
    try {
      ...
      const newExpiryTime = new Date(Math.max(now.getTime(), currentExpiry.getTime()) + renewConfig.duration * 60 * 60 * 1000)
  ...
  async getUserPermissionStats (userId) {
    try {
      ...
      return {
        ...
        lastUnlockTime: accessRecord.unlock_time ? moment(accessRecord.unlock_time).format('YYYY-MM-DD HH:mm:ss') : null,
        todayUnlockCount,
