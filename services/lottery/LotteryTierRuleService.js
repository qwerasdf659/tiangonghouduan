...
class LotteryTierRuleService {
  ...
  async create(data, options = {}) {
    ...
    if (!tier_name || !['high', 'mid', 'low'].includes(tier_name)) {
      throw new BusinessError(
        '档位名称（tier_name）必须是 high/mid/low 之一',
        'LOTTERY_REQUIRED',
        400
      )
    }
    if (tier_weight === undefined || tier_weight < 0 || tier_weight > this.WEIGHT_SCALE) {
      throw new BusinessError(
        `档位权重（tier_weight）必须在 0 到 ${this.WEIGHT_SCALE} 之间`,
        'LOTTERY_REQUIRED',
        400
      )
    }
    ...
    if (!campaign) {
      throw new BusinessError(
        `活动不存在：lottery_campaign_id=${lottery_campaign_id}`,
        'LOTTERY_NOT_FOUND',
        404
      )
    }
  ...
  async createTierRules(data, options = {}) {
    ...
    if (totalWeight !== this.WEIGHT_SCALE) {
      throw new BusinessError(
        `权重之和(${totalWeight})必须等于${this.WEIGHT_SCALE}`,
        'LOTTERY_REQUIRED',
        400
      )
    }
  ...
  async update(tier_rule_id, data, options = {}) {
    ...
    if (!rule) {
      throw new BusinessError(
        `档位规则不存在：tier_rule_id=${tier_rule_id}`,
        'LOTTERY_NOT_FOUND',
        404
      )
    }
