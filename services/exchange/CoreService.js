 ...
 * @updated 2026-07-11（技术债务方案 7.4-4：参数/资格/限购校验私有方法抽至 OrderValidator，
 *          经构造注入持有；对外方法签名与 getService('exchange_core') 服务键不变）
 */
...
class CoreService {
  ...
  constructor(models) {
    ...
    /*
     * 订单校验器（技术债务方案 7.4-4，2026-07-11 拆分）：
     * 参数/资格/限购类校验私有方法（复合门槛校验、prop/barter 守卫、退款防刷）
     * 纯搬移至 OrderValidator，经构造注入持有（不独立注册服务键，防单例状态分裂）。
     * 事务上下文仍由本服务经 options.transaction 透传，调用链层级不变。
     */
    this.orderValidator = new OrderValidator(models)
  }
  ...
  async exchangeItem(user_id, exchange_item_id, quantity = 1, options = {}) {
     ...
     * ═══════════════════════════════════════════════════
     */
    const extraConsumption = await this.orderValidator.assertRedeemRequirement(
  ...
  async cancelOrder(user_id, order_no, options = {}) {
    ...
    this.orderValidator.assertNotBarterOrder(order, '取消')

    // F1 选 A：虚拟道具单买入即消耗、禁止退款（防止变相"官方回收"撞星石货币属性红线）
    await this.orderValidator.assertNotPropOrder(order, transaction)
  ...
  async rejectOrder(order_no, operator_id, remark, options = {}) {
    ...
    this.orderValidator.assertNotBarterOrder(order, '拒绝')
  ...
  async refundOrder(order_no, operator_id, remark, options = {}) {
    ...
    this.orderValidator.assertNotBarterOrder(order, '退款')

    // F1 选 A：虚拟道具单买入即消耗、禁止退款
    await this.orderValidator.assertNotPropOrder(order, transaction)

    // 退款防刷检查（三层防护，从 system_settings 读取配置，初始值全 0 = 关闭）
    await this.orderValidator.checkRefundRules(order, transaction)
  ...
  /**
   * 复合门槛的额外消耗执行（与 OrderValidator.assertRedeemRequirement 校验对称，同一事务内）
   *
