...
const AdminSystemService = require('./AdminSystemService') // 管理后台系统服务（系统配置唯一权威实现）
...
const { DiyServiceFacade } = require('./diy') // DIY 统一入口（Facade 内部委托 4 个子服务，路由只使用 'diy' 键）
...
// 注：LotteryAlertDetectorService（告警检测引擎）由 scripts/maintenance/scheduled_tasks.js 定时任务直接 require，不注册服务键
...
// V4 模块化服务（DrawOrchestrator 由 UnifiedLotteryEngine 内部持有单一实例，不注册服务键）
const {
...
class ServiceManager {
  ...
  async initialize() {
    ...
    try {
      ...
      // V4统一抽奖引擎（实例化服务，内部持有 DrawOrchestrator 单一实例）
      ...
      this._services.set('diy', DiyServiceFacade) // DIY 饰品设计引擎（Facade 委托到 4 个子服务，路由统一走此键）
       ...
       * 注：资产组合查询统一由 asset_query（QueryService.getAssetPortfolio）提供，无独立服务
       */
