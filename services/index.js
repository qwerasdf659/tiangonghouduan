...
const AdminSystemService = require('./AdminSystemService') // 管理后台系统服务（代理层，配置逻辑已拆分至SystemConfigService）
const SystemConfigService = require('./SystemConfigService') // 系统配置服务（从AdminSystemService拆分）
const SystemMonitoringService = require('./SystemMonitoringService') // 系统监控服务（从AdminSystemService拆分）
...
const TradeDisputeService = require('./TradeDisputeService') // 售后申诉与纠纷服务（兑换/消费）
...
} = require('./diy') // DIY 拆分后子模块（Facade + 4 个子服务）
...
const AdCampaignService = require('./AdCampaignService') // Phase 3: 广告计划管理服务（用户端）
const AdCampaignAdminService = require('./AdCampaignAdminService') // 广告计划管理服务（管理端，拆分自 AdCampaignService）
...
const AdTagAggregationService = require('./AdTagAggregationService') // DMP 标签聚合服务
const AdAntifraudService = require('./AdAntifraudService') // 反作弊服务
const AdImpressionLogService = require('./AdImpressionLogService') // 广告曝光日志服务
const AdClickLogService = require('./AdClickLogService') // 广告点击日志服务
...
const ActivityService = require('./ActivityService') // 活动查询服务
const ActivityBudgetService = require('./ActivityBudgetService') // 活动预算管理服务（拆分自 ActivityService）
const ActivityLifecycleService = require('./ActivityLifecycleService') // 活动生命周期管理服务（拆分自 ActivityService）
const AuditLogService = require('./AuditLogService') // 审计日志服务（写入）
const AuditLogQueryService = require('./AuditLogQueryService') // 审计日志查询服务（拆分）
const UserAddressService = require('./UserAddressService') // 用户收货地址服务
...
// 注：MarketListing 域子服务（C2C 二级市场挂牌）已随 C2C 下线删除（2026-06-05 阶段五）

// Exchange 域子服务
const {
  CoreService: ExchangeCoreService, // 核心兑换操作（~450行）
  QueryService: ExchangeQueryService, // 用户端查询服务（~645行，含空间统计）
  AdminService: ExchangeAdminService, // 管理后台操作（拆分为 4 个子服务 + Facade）
...
const {
  ...
  StatsService: ReportingStatsService, // 系统/仪表盘统计
  UserStatsService: ReportingUserStatsService, // 用户维度统计（V4.7.1 拆分）
  MultiDimensionStatsService: ReportingMultiDimensionStatsService // 多维度组合统计（B-25/B-27）
} = require('./reporting')

/*
 * AdminLottery 域子服务
 * 2026-06-04 合规改造：CoreService（per-user 暗箱干预）已下线，对应文件已删除
 */
const {
  ...
  DisplayService: LotteryCampaignDisplayService // 活动展示控制（精选/隐藏/排序）
...
const RedemptionAdminService = require('./RedemptionAdminService') // 兑换订单管理端服务（管理员核销/取消/过期）
const BackpackService = require('./BackpackService') // 背包双轨查询服务（assets[] + items[]）

/*
 * 注：C2C 交易市场服务（TradeOrderService/TradeOrderQueryService/EscrowCodeService/OrphanFrozenCleanupService）
 *     已随 C2C 下线删除（2026-06-05 阶段五）
 */
...
const MerchantRiskAlertService = require('./MerchantRiskAlertService') // 商家风控告警管理服务
...
const AttributeService = require('./exchange/AttributeService') // EAV属性定义管理（品类绑定/预设值）
const ExchangeChannelPriceService = require('./exchange/ExchangeChannelPriceService') // 兑换渠道定价管理
const ProductSeriesService = require('./exchange/ProductSeriesService') // 产品系列（商品编码体系可读系列号轨道）
const SupplierService = require('./exchange/SupplierService') // 供货商主数据 + 货号辅助查询（商品编码体系 §3.8）
const PurchaseOrderService = require('./exchange/PurchaseOrderService') // S1 采购单管理
const ProductBatchService = require('./exchange/ProductBatchService') // S2 产品批次管理
const ProductBundleService = require('./exchange/ProductBundleService') // S4 组合商品管理
const ExternalChannelMappingService = require('./exchange/ExternalChannelMappingService') // S5 外部渠道映射
const ConsignmentService = require('./exchange/ConsignmentService') // S3 二手寄卖管理
...
const CrystalMultiplierService = require('./lottery/CrystalMultiplierService') // 水晶奖品倍率计算服务（判定/合并/夹紧/成本刹车，水晶奖品倍率活动设计方案 §18.3）

const LotteryAlertService = require('./lottery/LotteryAlertService') // 抽奖告警服务（B1 实时告警列表API）
const LotteryAlertDetectorService = require('./lottery/LotteryAlertDetectorService') // 抽奖告警检测引擎（从LotteryAlertService拆分）
...
const PendingCenterService = require('./dashboard/PendingCenterService') // 待处理中心服务
const PendingHealthScoreService = require('./dashboard/PendingHealthScoreService') // 待办健康度评分服务（§3.1.1）
...
const SealosStorageService = require('./SealosStorageService') // Sealos 对象存储服务
const SmsService = require('./SmsService') // 短信验证码服务（Phase 1：Redis存取 + 频率限制）
const CaptchaService = require('./CaptchaService') // 人机验证服务（腾讯云天御，P1.5 用户端发码前置校验）
const AdminLoginSecurityService = require('./AdminLoginSecurityService') // 管理端登录安全（P0.6 失败锁定+IP白名单+审计）

/**
 * V4.6 管线编排器
 *
 * 抽奖执行入口：使用 DrawOrchestrator 编排 Pipeline 执行
 * （2026-06-04 合规改造：ManagementStrategy per-user 暗箱干预已下线，对应文件已删除）
 */
...
/*
 * 注：market 域服务（PriceDiscoveryService/MarketAnalyticsService，C2C 行情/价格发现）
 *     已随 C2C 下线删除（2026-06-05 阶段五）
 */
...
class ServiceManager {
  ...
  async initialize() {
    ...
    try {
      ...
      this._services.set('exchange_barter', new ExchangeBarterService(this.models)) // 以物易物（B2C 官方合成：旧物核销→官方产出，2026-06-05 合规整改阶段六）

      // 注：C2C 用户间竞拍（auction_core/auction_query）已随 C2C 下线删除（2026-06-05 阶段五，官方竞价复用 exchange/bid）
      ...
      this._services.set('system_config', SystemConfigService)
      this._services.set('system_monitoring', SystemMonitoringService)

      /*
       * AdminLottery 域子服务
       * 2026-06-04 合规改造：admin_lottery_core（per-user 暗箱干预）已下线
       */
      ...
      this._services.set('lottery_campaign_display', LotteryCampaignDisplayService) // 活动展示控制（静态类）
      ...
      this._services.set('trade_dispute', new TradeDisputeService()) // 售后申诉与纠纷（兑换/消费，实例化，需延迟加载 models）
      ...
      this._services.set('diy', DiyServiceFacade) // DIY 饰品设计引擎（Facade 委托到 4 个子服务）
      this._services.set('diy_template', DiyTemplateService) // DIY 模板管理
      this._services.set('diy_work', DiyWorkService) // DIY 作品管理
      this._services.set('diy_material', DiyMaterialService) // DIY 素材管理
      this._services.set('diy_admin_query', DiyAdminQueryService) // DIY 管理后台查询
      ...
      this._services.set('ad_campaign', AdCampaignService) // Phase 3: 广告计划管理（用户端）
      this._services.set('ad_campaign_admin', AdCampaignAdminService) // 广告计划管理（管理端）
      ...
      this._services.set('ad_tag_aggregation', AdTagAggregationService) // DMP 标签聚合
      this._services.set('ad_antifraud', AdAntifraudService) // 反作弊
      this._services.set('ad_impression_log', AdImpressionLogService) // 广告曝光日志
      this._services.set('ad_click_log', AdClickLogService) // 广告点击日志
      ...
      this._services.set('activity_budget', ActivityBudgetService)
      this._services.set('activity_lifecycle', ActivityLifecycleService)
      this._services.set('audit_log', AuditLogService)
      this._services.set('audit_log_query', AuditLogQueryService) // 审计日志查询服务（静态类）
      ...
      this._services.set('reporting_stats', ReportingStatsService) // 系统/仪表盘统计（静态类）
      this._services.set('reporting_user_stats', ReportingUserStatsService) // 用户维度统计（V4.7.1 拆分，静态类）
      ...
      this._services.set('redemption_admin', RedemptionAdminService)
      this._services.set('backpack', BackpackService)

      // 注：C2C 交易市场服务（trade_order/trade_order_query/escrow_code/market_listing_*/orphan_frozen_cleanup）
      //     已随 C2C 下线删除（2026-06-05 阶段五，B2C 道具商城走 exchange 域）
      ...
      this._services.set('merchant_risk_alert', MerchantRiskAlertService) // 商家风控告警管理服务
      ...
      this._services.set('captcha', CaptchaService) // 人机验证服务（腾讯云天御，P1.5 用户端发码前置校验，静态类）
      this._services.set('admin_login_security', AdminLoginSecurityService) // 管理端登录安全（P0.6 失败锁定+IP白名单+审计，静态类）
      this._services.set('user_address', UserAddressService) // 用户收货地址服务（静态类）
      ...
      this._services.set('product_series_service', new ProductSeriesService(this.models)) // 产品系列管理（商品编码体系：系列 CRUD，序号由 SeriesSeqAllocator 事务内发号）
      this._services.set('supplier_service', new SupplierService(this.models)) // 供货商管理（商品编码体系：CRUD + 商品关联 + 货号辅助查询 + 健康统计）
      this._services.set('purchase_order_service', new PurchaseOrderService(this.models)) // S1 采购单（进货管理 + 收货入库）
      this._services.set('product_batch_service', new ProductBatchService(this.models)) // S2 产品批次（一批一码 + 召回）
      this._services.set('product_bundle_service', new ProductBundleService(this.models)) // S4 组合商品（BOM 管理）
      this._services.set('channel_mapping_service', new ExternalChannelMappingService(this.models)) // S5 外部渠道映射
      this._services.set('consignment_service', new ConsignmentService(this.models)) // S3 二手寄卖（状态机 + 所有权流转）
      ...
      this._services.set('crystal_multiplier', CrystalMultiplierService) // 水晶奖品倍率计算服务（静态类：判定/合并/夹紧/成本刹车 + C端合并倍率查询）
      this._services.set('reward_multiplier', require('./lottery/RewardMultiplierService')) // 水晶奖品倍率规则管理服务（静态类：CRUD + 成本看板 + 人群选项）
      this._services.set('event_budget', require('./lottery/EventBudgetService')) // 活动专属预算/活动积分生命周期服务（静态类：归集规则 CRUD + 归集判定 + 到期清零，防囤积套利 §12）
      ...
      this._services.set('lottery_alert_detector', LotteryAlertDetectorService) // 抽奖告警检测引擎（静态类）
      this._services.set('alert_silence', AlertSilenceService) // 告警静默规则管理服务（静态类）
      ...
      // 注：price_discovery/market_analytics（C2C 行情/价格发现）已随 C2C 下线删除（2026-06-05 阶段五）
       ...
       * （2026-06-04 合规改造：management_strategy per-user 暗箱干预已下线）
       */
   ...
   *
   * @example
   * const ExchangeAdminService = services.getService('exchange_admin') // 通过 snake_case 业务域键获取服务
   */
