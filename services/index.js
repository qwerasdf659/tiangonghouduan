...
// MarketListing 域子服务
const {
  CoreService: MarketListingCoreService,
  QueryService: MarketListingQueryService,
  AdminService: MarketListingAdminService,
  ValidationService: MarketListingValidationService,
  AdminListingService: MarketListingAdminListingService
...
const TradeOrderService = require('./TradeOrderService') // 交易订单服务（写操作核心）
const TradeOrderQueryService = require('./TradeOrderQueryService') // 交易订单查询服务（只读查询）
...
class ServiceManager {
  ...
  async initialize() {
    ...
    try {
      ...
      this._services.set('trade_order_query', TradeOrderQueryService) // 交易订单查询（2026-04-24 拆分）
      this._services.set('escrow_code', EscrowCodeService) // 交易市场担保码（Phase 4）

      // MarketListing 域子服务
      this._services.set('market_listing_core', MarketListingCoreService)
      this._services.set('market_listing_query', MarketListingQueryService)
      this._services.set('market_listing_admin', MarketListingAdminService)
      this._services.set('market_listing_validation', MarketListingValidationService)
      this._services.set('market_listing_admin_listing', MarketListingAdminListingService)
