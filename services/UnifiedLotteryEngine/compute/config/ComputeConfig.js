...
class DynamicConfigLoader {
  ...
  static async loadConfig(options = {}) {
    ...
    try {
      ...
      logger.info('[ComputeConfig] 动态配置加载成功', {
        lottery_campaign_id: lottery_campaign_id || 'global'
      })
      return merged_config
    } catch (error) {
      logger.error('[ComputeConfig] 动态配置加载失败:', error.message)
      return null
    } finally {
      cache.loading = false
    }
  ...
  static async loadMatrixConfig(options = {}) {
    ...
    try {
      ...
      logger.info('[ComputeConfig] 矩阵配置加载成功', {
        lottery_campaign_id: lottery_campaign_id || 'global'
      })
      return db_matrix
    } catch (error) {
      logger.error('[ComputeConfig] 矩阵配置加载失败:', error.message)
      return null
    }
  }
  ...
  static clearCache(lottery_campaign_id) {
    if (lottery_campaign_id) {
      ...
      logger.info('[ComputeConfig] 活动配置缓存已清除', { lottery_campaign_id })
    } else {
