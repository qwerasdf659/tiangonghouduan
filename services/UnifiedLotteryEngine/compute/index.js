 ...
 *
 * 导出内容：
 * - LotteryComputeEngine：抽奖计算引擎主类（Facade 模式）
 * - 独立计算器：可单独使用的计算组件
 * - 状态管理器：用户体验状态和全局状态管理
 * - 配置模块：静态配置和动态配置加载
 * - 工厂函数：单例模式获取引擎实例
...
// ========== 核心引擎 ==========
const LotteryComputeEngine = require('./LotteryComputeEngine')
 ...
 * @returns {LotteryComputeEngine} 计算引擎实例
 *
 * @example
 * // 获取默认实例
 ...
 * })
 */
function getComputeEngine(options = {}) {
  if (!_defaultInstance) {
    _defaultInstance = new LotteryComputeEngine(options)
  }
 ...
 * 重置默认实例（仅用于测试）
 *
 * 清除单例实例，下次调用 getComputeEngine 将创建新实例
 *
 * @returns {void}
 */
function resetComputeEngine() {
 ...
 * @returns {LotteryComputeEngine} 新的计算引擎实例
 *
 * @example
 * // 创建带有自定义配置的独立实例
...
function createComputeEngine(options = {}) {
  return new LotteryComputeEngine(options)
}
