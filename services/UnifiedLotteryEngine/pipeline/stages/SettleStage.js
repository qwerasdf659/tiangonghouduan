 ...
 *
 * 架构说明（2026-07-11 技术债务方案 7.4-5 拆分）：
 * - 本 Stage 只做编排与事务边界，Stage 接口（BaseStage 契约）与对外签名不变
 * - 奖品发放逻辑拆分至 settle/ 子目录，由本 Stage 持有并调用：
 *   - settle/PointsPrizeHandler：points 类奖品发放
 *   - settle/MaterialPrizeHandler：coupon/item/material 类奖品发放 + 星石配额扣减
 * - 事务仍由本 Stage 统一创建/提交/回滚，handler 只接收 transaction（调用链层级不变）
 *
 * @module services/UnifiedLotteryEngine/pipeline/stages/SettleStage
 * @author 统一抽奖架构重构
 * @since 2026
 * @updated 2026-01-19 - Phase 2 增强（积分扣减、派生幂等键）
 * @updated 2026-07-11 - 奖品发放逻辑拆分至 settle/ 子目录（纯搬移不改行为）
 */
...
// 奖品发放处理器（2026-07-11 拆分：SettleStage 只做编排与事务边界）
const MaterialPrizeHandler = require('./settle/MaterialPrizeHandler')
const PointsPrizeHandler = require('./settle/PointsPrizeHandler')
...
class SettleStage extends BaseStage {
  ...
  constructor() {
    ...
    /*
     * 奖品发放处理器（2026-07-11 拆分）：由 SettleStage 持有，
     * handler 通过 stage 引用复用本 Stage 的 log 日志上下文，不独立注册服务键
     */
    this.materialPrizeHandler = new MaterialPrizeHandler(this)
    this.pointsPrizeHandler = new PointsPrizeHandler(this)
  }
  ...
  async _distributePrize(user_id, prize, options) {
    ...
    try {
      ...
      /*
       * 按奖品类型委托对应发放处理器（2026-07-11 拆分，逻辑纯搬移）：
       * - points → PointsPrizeHandler（积分双录入账）
       * - coupon/item → MaterialPrizeHandler.distributeItemPrize（items + item_ledger 双录）
       * - material → MaterialPrizeHandler.distributeMaterialPrize（材料资产入账 + 星石配额扣减）
       * 事务边界不变：transaction 仍由本 Stage 创建并透传，handler 不创建/提交事务
       */
      switch (prize.prize_type) {
        case 'points':
          await this.pointsPrizeHandler.distributePointsPrize(user_id, prize, {
            idempotency_key,
            lottery_session_id,
            mint_account_id: mintAccount.account_id,
            transaction
          })
          break
        ...
        case 'coupon':
        case 'item':
          await this.materialPrizeHandler.distributeItemPrize(user_id, prize, {
            idempotency_key,
            lottery_session_id,
            lottery_draw_id,
            transaction
        ...
        case 'material':
          await this.materialPrizeHandler.distributeMaterialPrize(user_id, prize, {
            idempotency_key,
            mint_account_id: mintAccount.account_id,
            material_amount_override,
            crystal_multiplier,
            transaction
          })
          break
