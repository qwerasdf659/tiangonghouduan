...
class ImageResourceService {
  ...
  async createResource(resourceData, fileBuffer, uploadOptions = {}) {
    try {
      ...
      // 2. 生成存储路径
      const filePath = await this.storage.generateStoragePath(
        businessType,
        category,
        contextId,
        {
  ...
  async queryResources(queryParams) {
    try {
      ...
      return {
        resources: result.rows.map(row => row.toSafeJSON ? row.toSafeJSON() : row.get({ plain: true })),
        pagination: {
          total: result.count,
          page: parseInt(page),
          limit: parseInt(limit),
  ...
  async getResourceById(resourceId, options = {}) {
    try {
      ...
      const attributes = includeMetadata ? 
        undefined : 
  ...
  async batchReview(reviews, reviewerId) {
    try {
      ...
      console.log(`✅ 批量审核完成: 成功${results.success.length}条, 失败${results.failed.length}条`);
      return results;
