...
class ClaudeSupervisor {
  ...
  detectRepeatedStatusChecks(toolCalls) {
    const statusCheckCalls = toolCalls.filter(
      call =>
  ...
  detectSerialIndependentOps(toolCalls) {
    ...
    for (let i = 0; i < toolCalls.length - 1; i++) {
      ...
      if (
        this.isIndependentOperation(current) &&
   ...
   */
  hasTaskList(content) {
    const taskListPatterns = [/任务完成清单/, /- \[ \]/, /✅.*完成/, /❌.*未完成/, /📋.*清单/]
  ...
  monitorGitOperations(gitOps = []) {
    ...
    for (const op of gitOps) {
      // 检查是否需要用户授权
      if (config.require_user_authorization && this.isHighRiskGitOp(op) && !op.userAuthorized) {
        violations.push({
          type: 'CRITICAL',
          rule: 'Git操作用户授权控制规范',
      ...
      // 检查自动推送
      if (config.forbidden_auto_push && op.command.includes('push') && !op.userAuthorized) {
        violations.push({
          type: 'CRITICAL',
          rule: '禁止自动Git推送',
      ...
      // 检查提交信息质量
      if (
        config.require_meaningful_commits &&
  ...
  monitorCodeQuality(codeChanges = []) {
    ...
    for (const change of codeChanges) {
      // 检查文件存在性检查
      if (config.require_existence_check && !this.hasFileExistenceCheck(change)) {
        violations.push({
          type: 'HIGH',
          rule: '编写代码前先判断文件存在性',
      ...
      // 检查错误处理
      if (config.require_error_handling && !this.hasErrorHandling(change)) {
        violations.push({
          type: 'HIGH',
          rule: '错误处理和友好提示',
      ...
      // 检查中文注释
      if (config.require_chinese_comments && !this.hasChineseComments(change)) {
        violations.push({
          type: 'MEDIUM',
          rule: '代码要有详细中文注释',
      ...
      // 检查旧代码清理
      if (config.require_cleanup_old_code && !this.hasOldCodeCleanup(change)) {
        violations.push({
          type: 'MEDIUM',
          rule: '完成任务后删除旧功能代码',
  ...
  hasFileExistenceCheck(change) {
    const checks = ['fs.existsSync', 'file_search', 'list_dir', '文件是否存在', 'before editing']

    return checks.some(
      check => change.content.includes(check) || change.description.includes(check)
    )
  }
  ...
  hasErrorHandling(change) {
    const errorHandling = ['try-catch', 'catch', 'error', '错误处理', '友好提示', 'loading']

    return errorHandling.some(pattern => change.content.includes(pattern))
  }
  ...
  hasOldCodeCleanup(change) {
    const cleanupKeywords = ['删除', '清理', '移除', 'remove', 'delete', '不再需要']

    return cleanupKeywords.some(keyword => change.description.includes(keyword))
  }
  ...
  displayResults(report) {
    ...
    // 合规分数
    const scoreColor =
      report.compliance_score >= 90 ? '🟢' : report.compliance_score >= 70 ? '🟡' : '🔴'
