...
router.post('/', authenticateToken, upload.single('image'), async (req, res) => {
  try {
    ...
    if (!req.file) {
      return res.status(400).json(
        ApiResponse.error('未上传文件', 'NO_FILE')
      );
    }
    ...
    if (error.message.includes('文件大小超限')) {
      return res.status(413).json(
        ApiResponse.error(error.message, 'FILE_TOO_LARGE')
      );
    }
    
    if (error.message.includes('不支持的文件类型')) {
      return res.status(415).json(
        ApiResponse.error(error.message, 'UNSUPPORTED_FILE_TYPE')
      );
    }
    
    res.status(500).json(
      ApiResponse.error('创建资源失败', 'CREATE_FAILED', error.message)
    );
  }
});
...
router.post('/batch', authenticateToken, upload.array('images', 5), async (req, res) => {
  try {
    ...
    if (!req.files || req.files.length === 0) {
      return res.status(400).json(
        ApiResponse.error('未上传文件', 'NO_FILES')
      );
    }
    ...
    console.log(`✅ 批量上传完成: ${results.success.length}/${results.total} (${successRate}%), 耗时: ${duration}ms`);
    ...
    res.status(500).json(
      ApiResponse.error('批量上传失败', 'BATCH_UPLOAD_FAILED', error.message)
    );
  }
});
...
router.get('/', authenticateToken, async (req, res) => {
  try {
    ...
    res.status(500).json(
      ApiResponse.error('查询资源失败', 'QUERY_FAILED', error.message)
    );
  }
});
  ...
  try {
    ...
    if (!req.user.is_admin && resource.user_id !== req.user.id) {
      return res.status(403).json(
        ApiResponse.error('无权访问此资源', 'ACCESS_DENIED')
      );
    }
    ...
    if (error.message.includes('资源不存在')) {
      return res.status(404).json(
        ApiResponse.error('资源不存在', 'RESOURCE_NOT_FOUND')
      );
    }
    
    res.status(500).json(
      ApiResponse.error('获取资源详情失败', 'GET_FAILED', error.message)
    );
  }
});
...
router.put('/:resourceId', authenticateToken, async (req, res) => {
  try {
    ...
    if (!req.user.is_admin && resource.user_id !== req.user.id) {
      return res.status(403).json(
        ApiResponse.error('无权修改此资源', 'ACCESS_DENIED')
      );
    }
    ...
    if (updateData.reviewStatus && !req.user.is_admin) {
      return res.status(403).json(
        ApiResponse.error('无权进行审核操作', 'ADMIN_REQUIRED')
      );
    }
    ...
    res.status(500).json(
      ApiResponse.error('更新资源失败', 'UPDATE_FAILED', error.message)
    );
  }
});
...
router.delete('/:resourceId', authenticateToken, async (req, res) => {
  try {
    ...
    if (!req.user.is_admin && resource.user_id !== req.user.id) {
      return res.status(403).json(
        ApiResponse.error('无权删除此资源', 'ACCESS_DENIED')
      );
    }
    ...
    res.status(500).json(
      ApiResponse.error('删除资源失败', 'DELETE_FAILED', error.message)
    );
  }
});
...
router.get('/reviews/pending', requireAdmin, async (req, res) => {
  try {
    ...
    res.status(500).json(
      ApiResponse.error('获取待审核资源失败', 'GET_PENDING_FAILED', error.message)
    );
  }
});
...
router.post('/reviews/batch', requireAdmin, async (req, res) => {
  try {
    ...
    if (!Array.isArray(reviews) || reviews.length === 0) {
      return res.status(400).json(
        ApiResponse.error('审核数据格式错误', 'INVALID_REVIEWS_DATA')
      );
    }
    
    // 验证审核数据格式
    for (const review of reviews) {
      if (!review.resourceId || !review.action || !['approved', 'rejected'].includes(review.action)) {
        return res.status(400).json(
          ApiResponse.error('审核数据格式错误: 需要resourceId和action字段', 'INVALID_REVIEW_FORMAT')
        );
      }
    }
...
router.get('/stats/storage', requireAdmin, async (req, res) => {
  try {
    ...
    res.status(500).json(
      ApiResponse.error('获取存储统计失败', 'GET_STATS_FAILED', error.message)
    );
  }
});
...
router.get('/health', async (req, res) => {
  try {
    ...
    res.status(500).json(
      ApiResponse.error('服务异常', 'SERVICE_ERROR', error.message)
    );
  }
});
...
router.use((error, req, res, next) => {
  ...
  if (error instanceof multer.MulterError) {
    if (error.code === 'LIMIT_FILE_SIZE') {
      return res.status(413).json(
        ApiResponse.error('文件大小超过限制', 'FILE_SIZE_LIMIT')
      );
    }
    if (error.code === 'LIMIT_FILE_COUNT') {
      return res.status(400).json(
        ApiResponse.error('文件数量超过限制', 'FILE_COUNT_LIMIT')
      );
    }
  }
  
  res.status(500).json(
    ApiResponse.error('内部服务器错误', 'INTERNAL_ERROR', error.message)
  );
});
