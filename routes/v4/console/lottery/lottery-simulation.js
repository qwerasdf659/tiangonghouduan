 ...
 * 加载指定活动的模拟基线数据
 */
router.get('/baseline/:lottery_campaign_id', asyncHandler(async (req, res) => {
 ...
 * 模拟单个用户的连续抽奖旅程
 */
router.post('/user-journey', asyncHandler(async (req, res) => {
 ...
 * 一键应用模拟配置到线上
 */
router.post('/apply/:lottery_simulation_record_id', asyncHandler(async (req, res) => {
 ...
 * 计算模拟预测与实际数据的偏差
 */
router.post('/drift/:lottery_simulation_record_id', asyncHandler(async (req, res) => {
  const { lottery_simulation_record_id } = req.params
 ...
 * 定时应用模拟配置（指定未来生效时间）
 */
router.post('/schedule/:lottery_simulation_record_id', asyncHandler(async (req, res) => {
  const { lottery_simulation_record_id } = req.params
 ...
 * 获取策略配置的变更历史（含版本回滚信息）
 */
router.get('/version-history/:lottery_campaign_id', asyncHandler(async (req, res) => {
  const { lottery_campaign_id } = req.params
 ...
 * 回滚到指定版本的配置
 */
router.post('/rollback/:log_id', asyncHandler(async (req, res) => {
 ...
 * 获取预算消耗趋势和耗尽日期预测
 */
router.get('/budget-pacing/:lottery_campaign_id', asyncHandler(async (req, res) => {
  const { lottery_campaign_id } = req.params
 ...
 * 创建异常熔断监控规则
 */
router.post('/circuit-breaker/:lottery_simulation_record_id', asyncHandler(async (req, res) => {
  const { lottery_simulation_record_id } = req.params
 ...
 * 检查异常熔断状态
 */
router.get('/circuit-breaker-status/:lottery_campaign_id', asyncHandler(async (req, res) => {
  const { lottery_campaign_id } = req.params
 ...
 * 获取指定活动的模拟历史列表
 */
router.get('/history/:lottery_campaign_id', asyncHandler(async (req, res) => {
  const { lottery_campaign_id } = req.params
 ...
 * 获取单条模拟记录详情
 */
router.get('/record/:lottery_simulation_record_id', asyncHandler(async (req, res) => {
  const { lottery_simulation_record_id } = req.params
  const service = getSimulationService(req)
