 ...
 * 返回：规则详情（含优先级信息）
 */
router.get('/rules/:id', authenticateToken, requireRoleLevel(100), asyncHandler(async (req, res) => {
 ...
 * 返回：创建的规则信息
 */
router.post('/rules', authenticateToken, requireRoleLevel(100), asyncHandler(async (req, res) => {
  const {
    rule_type,
    lottery_campaign_id,
    role_uuid,
 ...
 * 返回：更新后的规则信息
 */
router.put('/rules/:id/disable', authenticateToken, requireRoleLevel(100), asyncHandler(async (req, res) => {
  const rule_id = parseInt(req.params.id, 10)
 ...
 */
router.get('/users/:user_id/status', authenticateToken, requireRoleLevel(100), asyncHandler(async (req, res) => {
 ...
 * 返回：更新后的配额状态
 */
router.post('/users/:user_id/bonus', authenticateToken, requireRoleLevel(100), asyncHandler(async (req, res) => {
  const { user_id } = req.params
 ...
 * 返回：配额规则和使用情况的汇总统计
 */
router.get('/statistics', authenticateToken, requireRoleLevel(100), asyncHandler(async (req, res) => {
  const { lottery_campaign_id } = req.query
 ...
 * 返回：配额充足性检查结果
 */
router.get('/users/:user_id/check', authenticateToken, requireRoleLevel(100), asyncHandler(async (req, res) => {
  const { user_id } = req.params
