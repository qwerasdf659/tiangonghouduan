 ...
 * @route POST /api/v4/console/lottery-management/probability-adjust
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    const {
      ...
      lottery_campaign_prize_id, // 特定奖品ID（对齐 lottery_campaign_prizes 表主键命名）
    ...
    // 判断是全局调整还是特定奖品调整
    const isSpecificPrize = !!lottery_campaign_prize_id
    let adjustmentData = {}

    if (isSpecificPrize) {
      // 特定奖品概率调整
      const validatedPrizeId = validators.validatePrizeId(lottery_campaign_prize_id)
      ...
      adjustmentData = {
        lottery_campaign_prize_id: validatedPrizeId,
 ...
 * @route POST /api/v4/console/lottery-management/user-specific-queue
 * @access Private (需要管理员权限)
 */
router.post(
  '/user-specific-queue',
