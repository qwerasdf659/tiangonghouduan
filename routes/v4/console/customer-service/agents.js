...
router.get('/', asyncHandler(async (req, res) => {
  ...
  return res.apiSuccess(result, '获取客服座席列表成功')
}))
 ...
 */
router.get('/lookup-user', asyncHandler(async (req, res) => {
  const { mobile } = req.query
  if (!mobile) {
    return res.apiError('请提供手机号', 'VALIDATION_ERROR', null, 400)
  }

  const service = req.app.locals.services.getService('cs_agent_management')
  const result = await service.lookupUserByMobile(mobile)
  return res.apiSuccess(result, '查询用户成功')
}))
...
router.get('/workload', asyncHandler(async (req, res) => {
  ...
  return res.apiSuccess(result, '获取工作负载概览成功')
}))
...
router.get('/:id', asyncHandler(async (req, res) => {
  ...
  return res.apiSuccess(result, '获取客服座席详情成功')
}))
...
router.post('/', asyncHandler(async (req, res) => {
  ...
  return res.apiSuccess(result, '客服座席注册成功')
}))
...
router.put('/:id', asyncHandler(async (req, res) => {
  ...
  return res.apiSuccess(result, '客服座席配置更新成功')
}))
...
router.delete('/:id', asyncHandler(async (req, res) => {
  ...
  return res.apiSuccess(result, '客服座席删除成功')
}))
