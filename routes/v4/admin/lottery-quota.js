...
/**
 * 查询配额规则列表
 * GET /api/v4/admin/lottery-quota/rules
 *
 * Query参数：
...
/**
 * 创建配额规则
 * POST /api/v4/admin/lottery-quota/rules
 *
 * Body参数：
...
/**
 * 禁用配额规则
 * PUT /api/v4/admin/lottery-quota/rules/:rule_id/disable
 *
 * 硬约束：
...
/**
 * 查询用户配额状态
 * GET /api/v4/admin/lottery-quota/users/:user_id/status
 *
 * Query参数：
 * - campaign_id: 活动ID（必填）
 *
...
/**
 * 为用户添加临时补偿次数（客服用）
 * POST /api/v4/admin/lottery-quota/users/:user_id/bonus
 *
 * Body参数：
 * - campaign_id: 活动ID（必填）
...
/**
 * 验证用户配额是否充足（预检查，不扣减）
 * GET /api/v4/admin/lottery-quota/users/:user_id/check
 *
 * Query参数：
 * - campaign_id: 活动ID（必填）
