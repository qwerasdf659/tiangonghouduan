 ...
 * - 数据脱敏处理（隐藏概率、库存等敏感信息）
 */
router.get('/:code/prizes', authenticateToken, dataAccessControl, asyncHandler(async (req, res) => {
 ...
 * - 管理员返回完整配置（含警告信息）
 */
router.get('/:code/config', authenticateToken, dataAccessControl, asyncHandler(async (req, res) => {
  const campaign_code = req.params.code
