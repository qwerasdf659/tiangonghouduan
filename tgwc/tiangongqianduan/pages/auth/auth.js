// pages/auth/auth.js - V4.0认证页面逻辑（完全符合V4.0统一引擎架构）
...
/**
 * 构建标准化的用户信息对象
 * @param {Object} rawUserInfo - 原始用户数据
 * @param {Object} jwtData - JWT解析数据
 * @returns {Object} 标准化用户信息
 *
 * 复杂度：~12
 */
...
Page({
   ...
   *
   * @returns {void}
   * @throws {Error} 初始化失败时抛出异常并调用handleInitError
   */
  safeInitPage() {
    // 设置基本状态
    this.setData({
    ...
    try {
      ...
      // 📌 环境配置可通过this.getEnvironmentConfig(appInstance)获取，但前端不进行Mock处理
  ...
  initAPIReferences() {
    try {
      ...
      this.apiClient = {
        sendVerificationCode: () => Promise.reject(new Error('API未初始化'))
      }
   ...
   * @param {string} formData.mobile - 手机号（11位，1开头）
   * @param {string} formData.verification_code - 验证码（6位数字）
   * @param {number} [retryCount=0] - 重试次数（用于网络失败重试）
   *
   * @description
   ...
   *
   * 优化策略：
   * 1. 提取JWT解析逻辑 → parseAndValidateJWT()
   * 2. 提取字段映射逻辑 → buildUserInfoObject()
   * 3. 提取存储逻辑 → saveAuthDataToStorage()
   * 4. 主函数保留流程控制
   */
  handleUnifiedLoginSuccess(loginData) {
    console.log('✅ 处理登录成功数据 - 方案B重构版')

    // ===== 步骤1：数据完整性检查 =====
    if (!loginData || !loginData.data) {
      console.error('❌ 登录数据为空或格式错误')
      this.handleLoginFailure(new Error('登录响应数据格式错误'))
      return
    }
    ...
    try {
      ...
      // 检查必要字段
      if (!accessToken || !rawUserInfo) {
        throw new Error('登录响应缺少必要数据（Token或用户信息）')
      }
