 ...
 * @version 2.0.0
 * @date 2026-02-22
 * @module jobs/ad-campaign-expiry-jobs
 */
...
if (!ENABLE_CONTENT_CRON_JOBS) {
  logger.info(
    '[AdCampaignExpiryCronJobs] 内容过期清理定时任务已禁用（设置 ENABLE_CONTENT_CRON_JOBS=true 启用）'
  )
  module.exports = {}
} else {
  /**
   * 内容过期清理定时任务调度器
   */
  class AdCampaignExpiryCronJobs {
    /**
     * 初始化所有定时任务
     * @returns {void} 无返回值
     */
    static init() {
      logger.info('[AdCampaignExpiryCronJobs] 初始化内容过期清理定时任务')

      cron.schedule('0 * * * *', async () => {
        await AdCampaignExpiryCronJobs.runExpiredContentCleanup()
      })

      logger.info('[AdCampaignExpiryCronJobs] 内容过期清理定时任务初始化完成（每小时执行）')
    }
    ...
    static async runExpiredContentCleanup() {
      ...
      logger.info('[AdCampaignExpiryCronJobs] 开始执行过期内容清理任务')
      ...
      try {
        ...
        if (completedCount > 0) {
          logger.info('[AdCampaignExpiryCronJobs] 过期内容清理完成', {
            completed_campaigns: completedCount,
            duration_ms: duration
          })
        } else {
          logger.debug('[AdCampaignExpiryCronJobs] 无过期内容需要清理', { duration_ms: duration })
        }
      } catch (error) {
        const duration = Date.now() - startTime
        logger.error('[AdCampaignExpiryCronJobs] 过期内容清理任务失败', {
          error: error.message,
          stack: error.stack,
          duration_ms: duration
        })
  ...
  AdCampaignExpiryCronJobs.init()
  module.exports = AdCampaignExpiryCronJobs
}
