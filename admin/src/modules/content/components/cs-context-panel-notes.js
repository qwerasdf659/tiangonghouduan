...
export async function loadNotes(userId) {
  const notesUrl =
    buildURL(CONTENT_ENDPOINTS.CS_USER_CONTEXT_NOTES, { userId }) +
    buildQueryString({ page_size: 50 })
  const issuesUrl =
    CONTENT_ENDPOINTS.CS_ISSUE_LIST + buildQueryString({ user_id: userId, page_size: 50 })
