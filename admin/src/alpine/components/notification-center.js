  ...
  return {
    ...
    async loadNotifications() {
      ...
      try {
        ...
        const response = await fetch('/api/v4/console/notifications?limit=20', {
        ...
        if (result.data) {
          ...
          this.notifications = result.data.items || result.data || []
        ...
        // 模拟数据
        this.notifications = [
          { id: 1, type: 'alert', title: '新的消耗审核', message: '有3条消耗记录待审核', is_read: false, created_at: new Date().toISOString() },
          { id: 2, type: 'warning', title: '预算告警', message: '活动A预算消耗已达85%', is_read: false, created_at: new Date(Date.now() - 3600000).toISOString() },
          { id: 3, type: 'info', title: '系统通知', message: '系统将于今晚进行维护', is_read: true, created_at: new Date(Date.now() - 86400000).toISOString() }
        ]
    ...
    async markAsRead(notification) {
      ...
      try {
        const token = localStorage.getItem('admin_token')
        await fetch(`/api/v4/console/notifications/${notification.id}/read`, {
          method: 'POST',
          headers: token ? { 'Authorization': `Bearer ${token}` } : {}
        })
    ...
    connectWebSocket() {
      ...
      try {
        ...
        this.socket.on('new_message', (data) => {
          this.handleWebSocketMessage({ type: 'notification', payload: {
    ...
    scheduleReconnect() {
      ...
      logger.debug(`[NotificationCenter] Socket.IO 将自动重连 (已尝试${this.wsReconnectAttempts}次)`)
    },
