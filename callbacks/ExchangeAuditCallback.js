...
module.exports = {
  ...
  async approved (exchangeId, auditRecord, transaction) {
    ...
    try {
      ...
      console.log(`[兑换审核回调] 库存已创建: exchange_id=${exchangeId}, 数量=${inventoryItems.length}`)
  ...
  async rejected (exchangeId, auditRecord, transaction) {
    ...
    try {
      ...
      await PointsService.addPoints(exchange.user_id, exchange.total_points, {
        ...
        business_id: `refund_exchange_${exchange.exchange_id}_${Date.now()}`,
      ...
      console.log(`[兑换审核回调] 积分已退回: user_id=${exchange.user_id}, points=${exchange.total_points}`)
      ...
      if (exchange.product_id) {
        ...
        console.log(`[兑换审核回调] 库存已恢复: product_id=${exchange.product_id}, quantity=${exchange.quantity}`)
      }
