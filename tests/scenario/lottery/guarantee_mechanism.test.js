 ...
 * 获取用户在某活动的抽奖次数
 */
async function getUserDrawCount(user_id, lottery_campaign_id) {
...
describe('【P1】保底机制集成测试', () => {
  ...
  beforeAll(async () => {
    ...
    testData = {
      ...
      lottery_campaign_id: realData.lottery_campaign_id
    }
    ...
    console.log(`   lottery_campaign_id: ${testData.lottery_campaign_id}`)
    console.log('='.repeat(80))
  })
  ...
  describe('4.1 保底计数器（数据库集成）', () => {
    test('能够正确查询用户抽奖次数', async () => {
      if (!testData.user_id || !testData.lottery_campaign_id) {
      ...
      const drawCount = await getUserDrawCount(testData.user_id, testData.lottery_campaign_id)

      expect(typeof drawCount).toBe('number')
      expect(drawCount).toBeGreaterThanOrEqual(0)

      console.log(
        `   用户${testData.user_id}在活动${testData.lottery_campaign_id}的抽奖次数: ${drawCount}`
      )
      console.log('   ✅ 数据库查询正常')
    })

    test('LotteryDraw.count 使用 user_id + lottery_campaign_id 条件', async () => {
      ...
      const count = await LotteryDraw.count({
        where: {
          ...
          lottery_campaign_id: testData.lottery_campaign_id
        }
      })
    ...
    test('计数使用双维度隔离', async () => {
      ...
      const [userCounts] = await sequelize.query(`
        SELECT 
          ...
          COUNT(*) as draw_count
        FROM lottery_draws
        GROUP BY user_id, lottery_campaign_id
        LIMIT 5
      `)
      ...
      if (userCounts.length > 0) {
        userCounts.forEach((row, index) => {
          console.log(
            `   ${index + 1}. user_id=${row.user_id}, lottery_campaign_id=${row.lottery_campaign_id}: ${row.draw_count}次`
          )
        })
      } else {
        console.log('   暂无抽奖数据')
      ...
      if (userCounts.length > 0) {
        const uniqueCombinations = new Set(
          userCounts.map(r => `${r.user_id}_${r.lottery_campaign_id}`)
        )
  ...
  describe('4.3 保底重置逻辑（数据库集成）', () => {
    test('取模运算不依赖额外状态字段', async () => {
      ...
      console.log(
        `   ${columnNames.filter(c => ['lottery_draw_id', 'user_id', 'lottery_campaign_id', 'lottery_prize_id'].includes(c)).join(', ')}`
      )
    ...
    test('累计次数可从 LotteryDraw 表直接计算', async () => {
      ...
      console.log(
        `   用户${testData.user_id}在活动${testData.lottery_campaign_id}: ${campaignCount}次`
      )
    ...
    test('不同活动的抽奖记录完全隔离', async () => {
      ...
      if (campaignStats.length > 0) {
        campaignStats.forEach(stat => {
          console.log(
            `   活动${stat.lottery_campaign_id}: ${stat.unique_users}人参与, ${stat.total_draws}次抽奖`
          )
        })
      } else {
        console.log('   暂无抽奖数据')
    ...
    test('同一用户在不同活动的计数独立', async () => {
      ...
      if (userCampaignStats.length > 0) {
        userCampaignStats.forEach(stat => {
          console.log(`   活动${stat.lottery_campaign_id}: ${stat.draw_count}次`)
        })
      } else {
        console.log('   暂无抽奖记录')
      }
  ...
  describe('集成测试报告', () => {
    test('生成保底机制集成测试报告', () => {
      ...
      console.log('   4.4 跨活动保底 - user_id + lottery_campaign_id 隔离 ✓')
      console.log('')
      console.log('📋 核心设计验证：')
      console.log('   - 计数来源：LotteryDraw 表记录数')
      console.log('   - 触发判断：next_draw_number % threshold === 0')
      console.log('   - 数据隔离：WHERE user_id=? AND lottery_campaign_id=?')
      console.log('='.repeat(80))
