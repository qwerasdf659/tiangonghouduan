...
describe('FeeCalculator - 手续费计算器测试', () => {
   ...
   *
   * 业务决策（交易市场多币种扩展）：
   * - star_stone：保持分档逻辑（基于 itemValue 分档 + ceil + 最低费 1）
   * - red_core_shard：单一费率 5%，最低手续费 1（从 system_settings 读取）
  ...
  describe('多币种手续费计算（calculateFeeByAsset）', () => {
     ...
     */
    test('star_stone 分档模式 - 高价值商品（itemValue=800, sellingPrice=750）', async () => {
    ...
    test('star_stone 分档模式 - 低价值商品（itemValue=100, sellingPrice=90）', async () => {
      const result = await FeeCalculator.calculateFeeByAsset('star_stone', 100, 90)
    ...
    test('star_stone 分档模式 - itemValue 为 null 时使用 sellingPrice 作为价值参考', async () => {
      const result = await FeeCalculator.calculateFeeByAsset('star_stone', null, 600)
    ...
    /**
     * red_core_shard 单一费率模式测试
     * - 使用 floor 向下取整（对用户更友好）
     * - calculation_mode 应为 'flat'
     * - 最低手续费 1
     */
    test('red_core_shard 单一费率模式 - 基础计算（sellingPrice=1000）', async () => {
    ...
    test('red_core_shard 单一费率模式 - 小额交易（sellingPrice=15）', async () => {
      const result = await FeeCalculator.calculateFeeByAsset('red_core_shard', null, 15)
    ...
    test('red_core_shard 单一费率模式 - floor 取整验证（sellingPrice=199）', async () => {
      const result = await FeeCalculator.calculateFeeByAsset('red_core_shard', null, 199)
    ...
    /**
     * star_stone vs red_core_shard 取整方式差异验证
     *
     * 业务决策（2026-01-14）：
     * - star_stone：ceil 向上取整
     * - red_core_shard：floor 向下取整（对用户更友好）
     */
    test('star_stone vs red_core_shard 取整方式差异验证', async () => {
      // 相同售价 199
      const diamondResult = await FeeCalculator.calculateFeeByAsset('star_stone', 199, 199)
      const redShardResult = await FeeCalculator.calculateFeeByAsset('red_core_shard', null, 199)

      // star_stone: 199 * 0.05 = 9.95 → ceil = 10
      expect(diamondResult.fee).toBe(10)
      expect(diamondResult.calculation_mode).toBe('tiered')

      // red_core_shard: 199 * 0.05 = 9.95 → floor = 9
      expect(redShardResult.fee).toBe(9)
      expect(redShardResult.calculation_mode).toBe('flat')

      // red_core_shard 对用户更友好（手续费少 1）
     ...
     * 业务决策：所有币种最低手续费为 1
     */
    test('最低手续费保底 - star_stone 极小额', async () => {
      const result = await FeeCalculator.calculateFeeByAsset('star_stone', 5, 5)
      expect(result.fee).toBeGreaterThanOrEqual(1) // 最低 1
    })

    test('最低手续费保底 - red_core_shard 极小额', async () => {
      const result = await FeeCalculator.calculateFeeByAsset('red_core_shard', null, 5)
      expect(result.fee).toBeGreaterThanOrEqual(1) // 最低 1
    })
     ...
     * 返回结构完整性验证
     */
    test('返回结构完整性验证 - star_stone', async () => {
      const result = await FeeCalculator.calculateFeeByAsset('star_stone', 500, 450)
    ...
    test('返回结构完整性验证 - red_core_shard', async () => {
      const result = await FeeCalculator.calculateFeeByAsset('red_core_shard', null, 500)
      ...
      // red_core_shard 单一费率模式，tier 应为 null
  ...
  describe('getFeeRateByAsset 辅助方法测试', () => {
    test('star_stone 返回分档费率信息', async () => {
    ...
    test('red_core_shard 返回单一费率信息', async () => {
