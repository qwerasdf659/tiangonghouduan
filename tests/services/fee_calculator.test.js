...
describe('FeeCalculator - 手续费计算器测试', () => {
   ...
   * ========================================
   */
  describe('低价值档（< 300积分）- 3%费率', () => {
    test('value=100, selling_points=90 → 手续费3积分', () => {
      const result = FeeCalculator.calculateItemFee(100, 90)
      expect(result.fee).toBe(3) // 90 * 0.03 = 2.7 → 向上取整 = 3
      expect(result.rate).toBe(0.03) // 费率3%
      expect(result.net_amount).toBe(87) // 卖家实收：90 - 3 = 87
      expect(result.tier).toBe('低价值档')
    })

    test('value=299, selling_points=280 → 手续费9积分', () => {
      const result = FeeCalculator.calculateItemFee(299, 280)
      expect(result.fee).toBe(9) // 280 * 0.03 = 8.4 → 向上取整 = 9
      expect(result.rate).toBe(0.03)
      expect(result.net_amount).toBe(271) // 280 - 9 = 271
    })

    test('极小值：value=10, selling_points=8 → 最小手续费1积分', () => {
      const result = FeeCalculator.calculateItemFee(10, 8)
      expect(result.fee).toBe(1) // 8 * 0.03 = 0.24 → 最小手续费1积分
   ...
   * ========================================
   */
  describe('高价值档（>= 600积分）- 10%费率', () => {
    test('value=600, selling_points=580 → 手续费58积分', () => {
      const result = FeeCalculator.calculateItemFee(600, 580)
      expect(result.fee).toBe(58) // 580 * 0.10 = 58
      expect(result.rate).toBe(0.10) // 费率10%
      expect(result.net_amount).toBe(522) // 580 - 58 = 522
      expect(result.tier).toBe('高价值档')
    })

    test('value=800, selling_points=750 → 手续费75积分', () => {
      const result = FeeCalculator.calculateItemFee(800, 750)
      expect(result.fee).toBe(75) // 750 * 0.10 = 75
      expect(result.net_amount).toBe(675) // 750 - 75 = 675
    })

    test('value=1000, selling_points=950 → 手续费95积分', () => {
      const result = FeeCalculator.calculateItemFee(1000, 950)
      expect(result.fee).toBe(95) // 950 * 0.10 = 95
      expect(result.net_amount).toBe(855) // 950 - 95 = 855
    })
  })
   ...
   */
  describe('边界值测试', () => {
    test('边界1：value=299（低价值档上限）→ 3%费率', () => {
      const result = FeeCalculator.calculateItemFee(299, 280)
      expect(result.rate).toBe(0.03)
      expect(result.tier).toBe('低价值档')
    })
    ...
    test('边界4：value=600（高价值档下限）→ 10%费率', () => {
      const result = FeeCalculator.calculateItemFee(600, 580)
  ...
  describe('特殊场景测试', () => {
    ...
    test('最小手续费规则：极小额交易', () => {
      ...
      expect(result.fee).toBe(1) // 3 * 0.03 = 0.09 → 最小手续费1积分
  ...
  describe('订单级别测试（支持混合商品）', () => {
    test('混合订单：不同档位商品分别计算', () => {
      const orderItems = [
        { inventory_id: 1, item_value: 200, selling_price: 180 }, // 低价值档3%
        { inventory_id: 2, item_value: 450, selling_price: 420 }, // 中价值档5%
        { inventory_id: 3, item_value: 800, selling_price: 750 } // 高价值档10%
      ]

      const result = FeeCalculator.calculateOrderFee(orderItems)

      // 验证总计（Verify Totals）
      expect(result.total_fee).toBe(6 + 21 + 75) // 总手续费：102积分
      expect(result.total_selling_price).toBe(180 + 420 + 750) // 总售价：1350积分
      expect(result.total_net_amount).toBe(174 + 399 + 675) // 卖家总实收：1248积分

      // 验证明细（Verify Breakdown）
      expect(result.breakdown).toHaveLength(3)
      expect(result.breakdown[0].fee).toBe(6) // 第1个商品手续费
      expect(result.breakdown[1].fee).toBe(21) // 第2个商品手续费
      expect(result.breakdown[2].fee).toBe(75) // 第3个商品手续费
    })
  })
  ...
  describe('辅助方法测试', () => {
    test('getRate() - 获取费率', () => {
      expect(FeeCalculator.getRate(200)).toBe(0.03) // 低价值档
    ...
    test('getFeeDescription() - 获取费率说明', () => {
      ...
      expect(desc1).toContain('3%')
      expect(desc1).toContain('低价值档')

      const desc2 = FeeCalculator.getFeeDescription(450)
      expect(desc2).toContain('5%')
      expect(desc2).toContain('中价值档')

      const desc3 = FeeCalculator.getFeeDescription(800)
      expect(desc3).toContain('10%')
      expect(desc3).toContain('高价值档')
    })
  })
})
