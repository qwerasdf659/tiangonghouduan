...
describe('DIY 素材展示字段与护栏（拍板决议 11.5-A/B）', () => {
  ...
  test('GET /api/v4/diy/templates/:id/beads — 用户端库存掩码（正数压成1）+ 隐藏对象存储字段', async () => {
    // 用库中真实 published 模板（不硬编码 ID，与真实数据状态对齐）
    const publishedTpl = await models.DiyTemplate.findOne({
      where: { status: 'published', is_enabled: true }
    })
    expect(publishedTpl).toBeTruthy()
    const res = await request(app).get(
      `/api/v4/diy/templates/${publishedTpl.diy_template_id}/beads`
    )
...
describe('DIY 手围驱动定制 — estimate 换算 / 发布护栏 / confirm 硬校验（手围驱动方案 §11）', () => {
  ...
  test('GET /api/v4/diy/templates/:id/estimate — 镶嵌模板拒绝估算（DIY_TEMPLATE_NOT_BEADING）', async () => {
    /* 找库中真实的 slots 镶嵌模板（layout 是 JSON，取回后按 shape 过滤，不硬编码 ID） */
    const templates = await models.DiyTemplate.findAll({ where: { status: 'published' } })
    const slotsTpl = templates.find(t => t.layout?.shape === 'slots')
    expect(slotsTpl).toBeTruthy()

    const res = await request(app).get(
      `/api/v4/diy/templates/${slotsTpl.diy_template_id}/estimate?wrist_size_mm=150&diameter=8`
    )
    expect(res.status).toBe(400)
    expect(res.body.success).toBe(false)
  ...
  test('GET /api/v4/diy/templates/:id — 手链模板手围档位毫米数据齐全（发布护栏落地验证）', async () => {
    /* 库中真实 published 手链模板（DIY_BRACELET=191），档位数据是长度驱动玩法的基础 */
    const braceletTpl = await models.DiyTemplate.findOne({
      where: { status: 'published', is_enabled: true, category_id: 191 }
    })
    expect(braceletTpl).toBeTruthy()

    const res = await request(app).get(`/api/v4/diy/templates/${braceletTpl.diy_template_id}`)

    expect(res.body.success).toBe(true)
    const sizing = res.body.data.sizing_rules
    expect(sizing.elastic_margin_mm).toBe(15)
    expect(sizing.size_options.length).toBeGreaterThan(0)
    /* 发布护栏口径：每档必须含数值型 wrist_size_mm 或 target_length_mm */
    for (const opt of sizing.size_options) {
      const hasMm =
        Number.isFinite(Number(opt.wrist_size_mm)) || Number.isFinite(Number(opt.target_length_mm))
...
describe('DIY 素材图上传 — trim_transparent 裁剪透明边距', () => {
  ...
  test('POST /api/v4/console/media/upload — pad_to_square=true 垫成正方形（拍板决议 D.7.1）', async () => {
    const sharp = require('sharp')
    // 生成一张 300×150 的非正方形 PNG（模拟运营传任意比例素材图）
    const testImage = await sharp({
      create: {
        width: 300,
        height: 150,
    ...
    const res = await request(app)
      ...
      .set('Authorization', `Bearer ${adminToken}`)
      .field('folder', 'diy-materials')
      .field('pad_to_square', 'true')
      .attach('image', testImage, 'test_bead_rect.png')

    expect(res.status).toBe(200)
    expect(res.body.success).toBe(true)

    // 垫方后输出统一正方形（512×512，透明底居中），填进正圆槽位不变形
    expect(res.body.data.width).toBe(res.body.data.height)
...
describe('DIY 材料分组接口 — 下发中文名与色值（拍板 16：小程序 Tab 不做本地映射）', () => {
  test('GET /api/v4/diy/material-groups — 每组含 group_code/count/display_name/color_hex', async () => {
    const res = await request(app).get('/api/v4/diy/material-groups')
    ...
    /* 以真实字典数据为基准核对（dict_type=diy_material_group，不硬编码期望中文名） */
    const dictRows = await models.SystemDictionary.findAll({
      where: { dict_type: 'diy_material_group', is_enabled: true },
      raw: true
    })
    const dictMap = new Map(dictRows.map(d => [d.dict_code, d]))

    for (const group of res.body.data) {
      expect(group).toHaveProperty('group_code')
      expect(typeof group.count).toBe('number')
      expect(group.count).toBeGreaterThan(0)

      const dict = dictMap.get(group.group_code)
      if (dict) {
        /* 字典有行：下发字典中文名 + 色值（与 system_dictionaries 一致） */
        expect(group.display_name).toBe(dict.dict_name)
        expect(group.color_hex).toBe(dict.dict_color)
      } else {
        /* 字典缺行：display_name 降级为裸 group_code（暴露问题由运营补字典） */
        expect(group.display_name).toBe(group.group_code)
      }
    }
  })
})

describe('DIY 槽位级素材过滤 — allowed_group_codes/allowed_shapes/allowed_diameters（拍板 15）', () => {
  let slotFilterTemplateId = null
  let targetMaterial = null

  beforeAll(async () => {
    /* 取库中真实启用素材作为约束基准（分组/形状/直径都来自真实数据，不硬编码） */
    targetMaterial = await models.DiyMaterial.findOne({ where: { is_enabled: true } })
    expect(targetMaterial).toBeTruthy()

    const media = await models.MediaFile.findOne({ order: [['media_id', 'ASC']] })
    expect(media).toBeTruthy()

    /* 建带槽位级三种约束的镶嵌模板（草稿即可，beads 接口不要求 published） */
    const res = await request(app)
      .post('/api/v4/console/diy/templates')
      .set('Authorization', `Bearer ${adminToken}`)
      .send({
        display_name: '测试模板_槽位级过滤',
        category_id: 192, // DIY_NECKLACE
        layout: {
          shape: 'slots',
          slot_definitions: [
            {
              ...
              rotation: 0,
              required: true,
              allowed_shapes: [targetMaterial.shape],
              allowed_group_codes: [targetMaterial.group_code],
              allowed_diameters: [Number(targetMaterial.diameter)]
            }
          ]
        },
        material_group_codes: [],
        preview_media_id: media.media_id,
        base_image_media_id: media.media_id
      })
    expect(res.body.success).toBe(true)
    slotFilterTemplateId = res.body.data.diy_template_id
  })

  afterAll(async () => {
    if (slotFilterTemplateId) {
      await models.DiyTemplate.destroy({
        where: { diy_template_id: slotFilterTemplateId }
      }).catch(() => {})
    }
  })

  test('GET /templates/:id/beads?slot_id=main — 返回素材全部满足槽位三种约束', async () => {
    const res = await request(app).get(
      `/api/v4/diy/templates/${slotFilterTemplateId}/beads?slot_id=main`
    )
    ...
    for (const bead of res.body.data) {
      expect(bead.group_code).toBe(targetMaterial.group_code)
      expect(bead.shape).toBe(targetMaterial.shape)
      expect(Number(bead.diameter)).toBe(Number(targetMaterial.diameter))
    }
  })

  test('GET /templates/:id/beads（不带 slot_id）— 槽位约束不生效，返回不限分组', async () => {
    const res = await request(app).get(`/api/v4/diy/templates/${slotFilterTemplateId}/beads`)

    expect(res.status).toBe(200)
    /* material_group_codes=[]（不限）时应返回全部启用素材（上限 200） */
    const enabledCount = await models.DiyMaterial.count({ where: { is_enabled: true } })
    expect(res.body.data.length).toBe(Math.min(enabledCount, 200))
  })
})

describe('DIY 镶嵌模板底图尺寸 — 自动回填 + 发布护栏（拍板决议 D.6/D.7.1）', () => {
  let bgTemplateId = null
  let baseMedia = null

  beforeAll(async () => {
    /* 取一张带真实 width/height 的媒体作底图（上传链路 sharp.metadata 落库值） */
    baseMedia = await models.MediaFile.findOne({
      where: { status: 'active', width: { [require('sequelize').Op.gt]: 0 } },
      order: [['media_id', 'ASC']]
    })
    expect(baseMedia).toBeTruthy()
  })

  afterAll(async () => {
    if (bgTemplateId) {
      await models.DiyTemplate.destroy({ where: { diy_template_id: bgTemplateId } }).catch(() => {})
    }
  })

  test('POST /console/diy/templates — slots 模板缺 background_width/height 时自动回填底图真实像素', async () => {
    const res = await request(app)
      .post('/api/v4/console/diy/templates')
      .set('Authorization', `Bearer ${adminToken}`)
      .send({
        display_name: '测试模板_底图尺寸回填',
        category_id: 194, // DIY_PENDANT
        layout: {
          shape: 'slots',
          /* 故意不传 background_width/height，验证从 media_files 回填 */
          slot_definitions: [
            {
              slot_id: 'main',
              label: '主石',
        ...
        preview_media_id: baseMedia.media_id,
        base_image_media_id: baseMedia.media_id
      })

    expect(res.body.success).toBe(true)
    bgTemplateId = res.body.data.diy_template_id
    expect(res.body.data.layout.background_width).toBe(baseMedia.width)
    expect(res.body.data.layout.background_height).toBe(baseMedia.height)
  })

  test('PUT /console/diy/templates/:id/status — 缺底图尺寸的 slots 模板拒绝发布（DIY_SLOT_BG_SIZE_MISSING）', async () => {
    expect(bgTemplateId).toBeTruthy()

    /* 模拟绕过回填直接入库的存量数据：剥掉 background_width/height（护栏兜底场景） */
    const template = await models.DiyTemplate.findByPk(bgTemplateId)
    const strippedLayout = { ...template.layout }
    delete strippedLayout.background_width
    delete strippedLayout.background_height
    await models.DiyTemplate.update(
      { layout: strippedLayout },
      { where: { diy_template_id: bgTemplateId } }
    )

    const res = await request(app)
      .put(`/api/v4/console/diy/templates/${bgTemplateId}/status`)
      .set('Authorization', `Bearer ${adminToken}`)
      .send({ status: 'published' })

    expect(res.status).toBe(400)
    expect(res.body.success).toBe(false)
    expect(res.body.code).toBe('DIY_SLOT_BG_SIZE_MISSING')
  })
})
