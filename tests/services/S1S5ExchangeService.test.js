...
describe('兑换订单逆向库存回补修复（2026-07-11：取消/拒绝/退款三条逆向路径统一回补库存）', () => {
  const TransactionManager = require('../../utils/TransactionManager')
  const ROLLBACK_SENTINEL = 'CANCEL_RESTORE_TEST_ROLLBACK'

  /**
   * 逆向回补测试公共夹具：事务内「扣库存计销量 + 直造订单 → 执行逆向动作 → 断言恢复 → 强制回滚」
   *
   * 与用户取消用例同一口径：所有写入随事务回滚，真实库零残留。
   *
   * @param {Object} params - 夹具参数
   * @param {string} params.order_status - 直造订单的初始状态（pending / approved）
   * @param {string} params.idempotency_prefix - 幂等键前缀（区分三条逆向路径）
   * @param {Function} params.reverse_action - 逆向动作执行器 async ({ coreService, order_no, user, transaction })
   * @returns {Promise<void>} 断言全部通过后由 ROLLBACK_SENTINEL 强制回滚
   */
  async function runRestoreScenario({ order_status, idempotency_prefix, reverse_action }) {
    ...
    try {
      await TransactionManager.execute(async transaction => {
        ...
        // 2. 直造一条兑换订单（真实表结构，随事务回滚不落库）
        const orderNo = `EMTEST${Date.now()}`
        await models.ExchangeRecord.create(
          {
            order_no: orderNo,
            idempotency_key: `${idempotency_prefix}_${Date.now()}`,
            business_id: `${idempotency_prefix}_biz_${Date.now()}`,
        ...
        // 3. 执行逆向动作（取消/拒绝/退款，修复点：三条路径都应回补库存）
        await reverse_action({ coreService, order_no: orderNo, user: senderUser, transaction })
  ...
  test('用户取消 pending 单 — 库存回补 + 销量回退（事务内验证后回滚）', async () => {
    await runRestoreScenario({
      order_status: 'pending',
      idempotency_prefix: 'cancel_restore_test',
      reverse_action: async ({ coreService, order_no, user, transaction }) => {
        const result = await coreService.cancelOrder(user.user_id, order_no, { transaction })
        expect(result.status).toBe('cancelled')
      }
    })
  })

  test('管理员拒绝 pending 单 — 库存回补 + 销量回退（事务内验证后回滚）', async () => {
    await runRestoreScenario({
      order_status: 'pending',
      idempotency_prefix: 'reject_restore_test',
      reverse_action: async ({ coreService, order_no, user, transaction }) => {
        const result = await coreService.rejectOrder(order_no, user.user_id, '回补测试拒绝', {
          transaction
        })
  ...
  test('管理员退款 approved 单 — 库存回补 + 销量回退（事务内验证后回滚）', async () => {
    await runRestoreScenario({
      order_status: 'approved',
      idempotency_prefix: 'refund_restore_test',
      reverse_action: async ({ coreService, order_no, user, transaction }) => {
        const result = await coreService.refundOrder(order_no, user.user_id, '回补测试退款', {
          transaction
        })
