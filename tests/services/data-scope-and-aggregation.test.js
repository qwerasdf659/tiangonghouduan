 ...
 * - 消费列表多店聚合（MerchantService.getMerchantRecords 的 store_scope/store_ids/is_manager 口径）
...
describe('数据范围隔离 + 多店聚合 + 低库存筛选（连真实库，无 mock）', () => {
  ...
  describe('消费列表多店聚合（MerchantService.getMerchantRecords）', () => {
    test('管理员（store_scope=all）查询不抛错且 query_scope=all', async () => {
    ...
    test('店长（is_manager=true）按可见门店集合聚合，query_scope=stores', async () => {
      const r = await MerchantService.getMerchantRecords({
        user_id: STAFF_USER_ID,
        store_scope: 'stores',
        store_ids: [STORE_ID],
    ...
    test('店员（is_manager=false）只看本人经手，query_scope=self', async () => {
      const r = await MerchantService.getMerchantRecords({
        user_id: STAFF_USER_ID,
        store_scope: 'stores',
        store_ids: [STORE_ID],
      ...
      expect(r.query_scope).toBe('self')
      // 店员模式：返回记录的经手人应都是本人
      r.records.forEach(rec => {
    ...
    test('可见门店为空集合时查不到任何记录（防越权兜底）', async () => {
      const r = await MerchantService.getMerchantRecords({
        user_id: STAFF_USER_ID,
        store_scope: 'stores',
        store_ids: [],
