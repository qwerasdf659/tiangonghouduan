 ...
 *
 * P1-1待办任务：兑换市场 `/api/v4/exchange` 的 business_id 策略
 *
 * 核心功能测试：
...
const _BeijingTimeHelper = require('../../../utils/timeHelper')
...
describe('兑换市场幂等性测试 (Exchange Market Idempotency - V4.5.0 材料资产支付)', () => {
  ...
  beforeAll(async () => {
    ...
    /*
     * 创建测试商品（V4.5.0 统一商品中心 ExchangeItem + ExchangeItemSku + ExchangeChannelPrice）
     * 商品编码体系落地后：item_code NOT NULL、sku_code 为 SK+12 位规范形（VARCHAR(14)），
     * 测试数据与业务同源，编码经 ProductCodeGenerator 生成（非硬编码）。
     */
    const testExchangeItem = await ExchangeItem.create({
    ...
    const testSku = await ExchangeItemSku.create({
      exchange_item_id: testExchangeItem.exchange_item_id,
    ...
    testItem = {
      exchange_item_id: testExchangeItem.exchange_item_id,
      sku_id: testSku.sku_id,
      item_name: testExchangeItem.item_name,
      cost_asset_code: 'red_core_shard',
      cost_amount: 100
    }

    /*
     * 初始化测试用户的材料资产账户（red_core_shard）
    ...
    console.log(`   - 测试商品ID (exchange_item_id): ${testItem.exchange_item_id}`)
  ...
  beforeEach(async () => {
    ...
    console.log(`🔍 测试前检查材料资产余额: ${currentBalance} red_core_shard`)
    ...
    if (currentBalance < 1000) {
      ...
      console.log(`✅ 已充值 ${1000 - currentBalance} red_core_shard`)
    } else {
  ...
  afterAll(async () => {
    try {
      if (testItem) {
        ...
        // 先删 SKU 渠道定价、SKU，再删商品（遵守 FK 约束顺序）
        ...
        if (skus.length > 0) {
          ...
          const { ExchangeChannelPrice: ECP } = require('../../../models')
          await ECP.destroy({ where: { sku_id: skuIds } })
          await ExchangeItemSku.destroy({ where: { exchange_item_id: testItem.exchange_item_id } })
        }
        await ExchangeItem.destroy({ where: { exchange_item_id: testItem.exchange_item_id } })
      }
      console.log('✅ 测试数据清理完成')
    } catch (cleanupError) {
      console.warn('⚠️ 测试数据清理部分失败（不影响测试结果）:', cleanupError.message)
    }
  }, 30000)
      ...
      expect(response.body).toMatchObject({
        ...
        data: {
          order: expect.objectContaining({
            ...
            // 测试商品为 virtual（虚拟即时履约），下单即 completed（无需邮寄/审核）
            status: 'completed'
          })
        }
      })
  ...
  describe('P1-1-2: 幂等性保护（相同Idempotency-Key只创建一笔订单）', () => {
    ...
    test('材料资产应该只扣除一次', async () => {
      ...
      const assetBalanceBefore = await AccountAssetBalance.findOne({
        where: { account_id: accountBefore.account_id, asset_code: 'red_core_shard' }
      })
      const balanceBefore = Number(assetBalanceBefore.available_amount)
      console.log(`🔍 初始材料资产余额: ${balanceBefore} red_core_shard`)
      ...
      console.log(`✅ 第一次兑换完成，扣除材料资产: ${deducted} red_core_shard`)
  ...
  describe('P1-1-3: 冲突保护（同一幂等键但不同参数返回409）', () => {
    test('同一 Idempotency-Key 但不同 exchange_item_id 应返回409', async () => {
      ...
      // 创建另一个测试商品（使用 ExchangeItem 模型；编码与业务同源经 ProductCodeGenerator 生成）
      const anotherExchangeItem = await ExchangeItem.create({
        item_name: '【测试】另一个商品',
      ...
      const anotherSku = await ExchangeItemSku.create({
        exchange_item_id: anotherExchangeItem.exchange_item_id,
        sku_code: ProductCodeGenerator.generate('SK'),
        stock: 1000,
        cost_price: 50,
        status: 'active',
      ...
      // 清理测试数据（先删子表再删主表，避免FK约束）
      await ExchangeChannelPrice.destroy({ where: { sku_id: anotherSku.sku_id } })
      await ExchangeItemSku.destroy({
        where: { exchange_item_id: anotherExchangeItem.exchange_item_id }
      })
      await anotherExchangeItem.destroy()
    })
  ...
  describe('P1-1-5: 并发幂等性保护', () => {
    test('并发请求相同 Idempotency-Key 应只创建一笔订单', async () => {
       ...
       * 业务验证：
       * 并发请求同一幂等键应保证数据一致性：
       * - 场景A（用户有余额）：至少1个成功(200)，其余409，数据库仅1条订单
       * - 场景B（用户余额不足）：所有请求返回相同业务错误，数据库0条订单（幂等键记录失败状态）
       */
      let businessErrorCount = 0
      responses.forEach(r => {
        if (r.status !== 200 && r.status !== 409) businessErrorCount++
      })
      ...
      if (successCount > 0) {
        ...
        console.log(`✅ 并发幂等性保护验证通过（场景A：有余额）- 订单数: ${orderCount}`)
      } else {
        // 所有请求因业务原因失败（如余额不足），幂等机制仍然生效（相同 key 返回一致结果）
        const statuses = new Set(responses.map(r => r.status))
        expect(statuses.size).toBeLessThanOrEqual(2)
        console.log(`✅ 并发幂等性保护验证通过（场景B：余额不足，一致性返回）`)
      }

      console.log(`   - 并发请求数: ${responses.length}`)
      console.log(
        `   - 成功: ${successCount}, 处理中: ${processingCount}, 业务错误: ${businessErrorCount}`
      )
    })
  })
})
