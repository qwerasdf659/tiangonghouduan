/**
 * 用户管理安全修复测试 - 风险1、2、3修复验证
 * 测试文件：routes/v4/unified-engine/admin/user_management.js
 * 创建时间：2025年11月07日
 *
 * 测试目标：
...
describe('🛡️ 用户管理安全修复测试 (风险1、2、3)', () => {
  ...
  beforeAll(async () => {
    ...
    // 获取真实测试用户
    const realUsers = await getRealTestUsers()
    adminUser = realUsers.adminUser
    regularUser1 = realUsers.regularUsers[0]
    regularUser2 = realUsers.regularUsers[1]
  ...
  describe('🔒 风险1修复：权限级别验证（防止权限提升攻击）', () => {
    test('管理员可以修改低级别用户的角色（正常情况）', async () => {
      ...
      const response = await request(app)
        .put(`/api/v4/unified-engine/admin/user-management/users/${regularUser1.user_id}/role`)
  ...
  describe('🚫 风险2修复：禁止自我状态修改（防止误操作）', () => {
    test('管理员可以修改其他用户的状态（正常情况）', async () => {
      ...
      const response = await request(app)
        .put(`/api/v4/unified-engine/admin/user-management/users/${regularUser1.user_id}/status`)
        .set('Authorization', `Bearer ${adminToken}`)
        .send({
          status: 'active',
    ...
    test('管理员无法修改自己的状态（安全保护）', async () => {
      ...
      const response = await request(app)
        .put(`/api/v4/unified-engine/admin/user-management/users/${adminUser.user_id}/status`)
        .set('Authorization', `Bearer ${adminToken}`)
        .send({
          status: 'inactive',
          reason: '测试自我状态修改-误操作'
  ...
  describe('🔄 风险3修复：事务回滚处理（数据一致性保护）', () => {
    ...
    test('用户不存在时事务正确回滚', async () => {
      ...
      const response = await request(app)
        .put(`/api/v4/unified-engine/admin/user-management/users/${nonexistentUserId}/role`)
        .set('Authorization', `Bearer ${adminToken}`)
        .send({
          role_name: 'user',
          reason: '测试事务回滚-用户不存在'
