/**
 * 获取用户积分API增强功能测试
 * 测试目标API: GET /api/v4/unified-engine/lottery/points/:user_id
 *
...
describe('GET /api/v4/unified-engine/lottery/points/:user_id - 增强功能测试', () => {
  ...
  describe('P0优化功能测试', () => {
    it('应拒绝无效的user_id参数', async () => {
      const res = await request(app)
        .get('/api/v4/unified-engine/lottery/points/abc')
    ...
    it('应拒绝查询不存在的用户', async () => {
      ...
      const res = await request(app)
        .get(`/api/v4/unified-engine/lottery/points/${nonExistentUserId}`)
        .set('Authorization', `Bearer ${adminToken}`)
    ...
    it('应返回明确错误：用户存在但无积分账户（不自动创建）', async () => {
      ...
      const res = await request(app)
        .get(`/api/v4/unified-engine/lottery/points/${userWithoutAccount.user_id}`)
        .set('Authorization', `Bearer ${adminToken}`)
    ...
    it('应拒绝查询冻结的积分账户', async () => {
      ...
      const res = await request(app)
        .get(`/api/v4/unified-engine/lottery/points/${frozenUser.user_id}`)
        .set('Authorization', `Bearer ${adminToken}`)
  ...
  describe('限流保护功能测试（60次/分钟）', () => {
    ...
    it('应允许正常请求（前60次）', async () => {
      ...
      for (let i = 0; i < 20; i++) {
        const res = await request(app)
          .get(`/api/v4/unified-engine/lottery/points/${normalUserId}`)
  ...
  describe('管理员审计日志测试', () => {
    ...
    it('管理员查询自己的积分不应记录审计日志', async () => {
      const res = await request(app)
        .get(`/api/v4/unified-engine/lottery/points/${adminUserId}`)
        .set('Authorization', `Bearer ${adminToken}`)
