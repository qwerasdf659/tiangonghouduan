...
describe('🎰 抽奖接口 sort_order 字段测试（方案3验证）', () => {
  let testUserToken
  let testUser
  ...
  beforeAll(async () => {
    ...
    if (!testUser) {
      throw new Error('测试账号不存在：13612227930')
    }
  ...
  describe('POST /api/v4/unified-engine/lottery/draw', () => {
  ...
  describe('GET /api/v4/unified-engine/lottery/prizes/:campaignCode', () => {
    test('✅ 奖品列表应该返回 sort_order 字段', async () => {
      const response = await request(app)
        .get('/api/v4/unified-engine/lottery/prizes/BASIC_LOTTERY')
        .set('Authorization', `Bearer ${testUserToken}`)
