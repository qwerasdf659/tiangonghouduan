...
describe('积分和用户系统API测试', () => {
  ...
  describe('积分系统API', () => {
     ...
     * API路径：GET /api/v4/unified-engine/points/trend
     * 参数：days（查询天数，7-90天）
     * 返回：labels（日期标签数组）、earn_data（每日获得积分）、consume_data（每日消费积分）
     */
    test('✅ 积分趋势查询（默认30天）- GET /api/v4/unified-engine/points/trend', async () => {
      const response = await tester.makeAuthenticatedRequest(
        'GET',
        '/api/v4/unified-engine/points/trend?days=30',
        null,
        'regular'
      )
    ...
    test('✅ 积分趋势查询（7天）- GET /api/v4/unified-engine/points/trend', async () => {
      const response = await tester.makeAuthenticatedRequest(
        'GET',
        '/api/v4/unified-engine/points/trend?days=7',
        null,
        'regular'
      )
    ...
    test('✅ 积分趋势查询（90天）- GET /api/v4/unified-engine/points/trend', async () => {
      const response = await tester.makeAuthenticatedRequest(
        'GET',
        '/api/v4/unified-engine/points/trend?days=90',
        null,
        'regular'
      )
    ...
    test('✅ 积分趋势参数边界测试（自动修正）- GET /api/v4/unified-engine/points/trend', async () => {
      // 测试days=5应自动修正为7
      const response1 = await tester.makeAuthenticatedRequest(
        'GET',
        '/api/v4/unified-engine/points/trend?days=5',
        null,
        'regular'
      )
      ...
      const response2 = await tester.makeAuthenticatedRequest(
        'GET',
        '/api/v4/unified-engine/points/trend?days=100',
        null,
        'regular'
      )
