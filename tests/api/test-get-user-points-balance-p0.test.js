 ...
 *
 * 测试目标API: GET /api/v4/unified-engine/points/balance/:user_id
 *
 * 测试内容（基于实施方案文档）:
...
describe('GET /api/v4/unified-engine/points/balance/:user_id - P0优化验证', () => {
  let adminToken
  let adminUserId
  let normalUserToken
  let normalUserId
  ...
  describe('P0优化1: 参数严格验证', () => {
    it('应拒绝无效的user_id参数（字符串abc）', async () => {
      const res = await request(app)
        .get('/api/v4/unified-engine/points/balance/abc')
    ...
    it('应拒绝负数user_id', async () => {
      const res = await request(app)
        .get('/api/v4/unified-engine/points/balance/-1')
        .set('Authorization', `Bearer ${adminToken}`)
    ...
    it('应拒绝零值user_id', async () => {
      const res = await request(app)
        .get('/api/v4/unified-engine/points/balance/0')
        .set('Authorization', `Bearer ${adminToken}`)
  ...
  describe('P0优化2: 用户存在性验证', () => {
    it('应拒绝查询不存在的用户（防止数据污染）', async () => {
      ...
      const res = await request(app)
        .get(`/api/v4/unified-engine/points/balance/${nonExistentUserId}`)
        .set('Authorization', `Bearer ${adminToken}`)
  ...
  describe('P0优化3: 账户存在性检查', () => {
    ...
    it('应返回明确错误：用户存在但无积分账户（不自动创建）', async () => {
      const res = await request(app)
        .get(`/api/v4/unified-engine/points/balance/${userWithoutAccount.user_id}`)
        .set('Authorization', `Bearer ${adminToken}`)
  ...
  describe('P0优化4: 账户状态检查', () => {
    ...
    it('应拒绝查询冻结的积分账户', async () => {
      const res = await request(app)
        .get(`/api/v4/unified-engine/points/balance/${frozenUser.user_id}`)
        .set('Authorization', `Bearer ${adminToken}`)
  ...
  describe('权限验证测试', () => {
    it('普通用户应只能查询自己的积分', async () => {
      const res = await request(app)
        .get(`/api/v4/unified-engine/points/balance/${normalUserId}`)
    ...
    it('普通用户应无权查询他人积分', async () => {
      const res = await request(app)
        .get(`/api/v4/unified-engine/points/balance/${adminUserId}`)
        .set('Authorization', `Bearer ${normalUserToken}`)
