...
describe('抽奖系统API测试', () => {
  ...
  describe('抽奖系统API', () => {
    ...
    test('✅ 获取用户抽奖历史 - GET /api/v4/lottery/history', async () => {
      const response = await tester.makeAuthenticatedRequest(
        'GET',
        `/api/v4/lottery/history/${test_user_id || test_account.user_id}`,
        null,
        'regular'
      )
    ...
    /**
     * ✅ 获取用户抽奖统计 - GET /api/v4/lottery/statistics/:user_id
     *
  ...
  describe('奖品分发系统API', () => {
    test('✅ 获取用户奖品分发历史 - GET /api/v4/prizeDistribution/user/:user_id/history', async () => {
      const response = await tester.makeAuthenticatedRequest(
        'GET',
        `/api/v4/prizeDistribution/user/${test_user_id || test_account.user_id}/history`,
        null,
        'regular'
      )
  ...
  describe('性能和集成测试', () => {
    test('🚀 API响应时间性能测试', async () => {
      ...
      const _response = await tester.makeRequest('GET', '/api/v4/lottery/health')
