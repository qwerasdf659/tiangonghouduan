...
async function test1_normalAccess (token) {
  ...
  try {
    const response = await axios.get(
      `${BASE_URL}/api/v4/lottery/config/BASIC_LOTTERY`,
...
async function test2_invalidCharacters (token) {
  ...
  try {
    const response = await axios.get(
      `${BASE_URL}/api/v4/lottery/config/INVALID@CODE`,
...
async function test3_tooLong (token) {
  ...
  try {
    ...
    const response = await axios.get(
      `${BASE_URL}/api/v4/lottery/config/${longCode}`,
      { headers: { Authorization: `Bearer ${token}` }, validateStatus: () => true }
    )
...
async function test4_notFound (token) {
  ...
  try {
    const response = await axios.get(
      `${BASE_URL}/api/v4/lottery/config/NOT_EXIST`,
      { headers: { Authorization: `Bearer ${token}` }, validateStatus: () => true }
    )
