...
- **DIY 数据大屏**（新增 `diy-dashboard.html` + `diy-dashboard.js`）：P0 完备度 + P1 转化漏斗/GMV 趋势/履约 + P2 素材热度 Top20/五行分布饼图，全部来自扩展后的 `GET /console/diy/stats`（新增 `completeness/funnel/gmv/fulfillment/material_ranking/five_elements_distribution` 字段），每个指标点击跳转对应管理页带筛选；已在 `vite.config.js` 与 `sidebar-nav.js` 注册。

### 13.3 前端仍需自行适配（改前端，见 11.7，后端不再改）

1. 图片降级链改 `image_media.thumbnails.w750 → public_url`（后端已下发 `thumbnails.{w375,w750,w1080}`）。
...
> 本节记录下方 40 项拍板清单的**实际落地状态**。全部改动在真实库 `restaurant_points_dev` 迁移落库 + Jest 24 用例全绿（`tests/services/S1S5ExchangeService.test.js`）+ 回归（ExchangeService 24 / DIY+幂等 63）全绿 + ESLint / `check:api-contract` / `validate:routes` / admin 构建通过 + PM2 健康。
...
- 幂等：全部写路径已登记 `CANONICAL_OPERATION_MAP`（含 submit/cancel/recall 与寄卖四个状态动作）。
...
验收：Jest 24 用例全绿 + 兑换/DIY/幂等回归全绿 + 质量门禁（ESLint / api-contract / routes / admin build）全过 + PM2 健康，真实库测试数据零残留。
