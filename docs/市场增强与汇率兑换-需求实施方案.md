...
**本次方案组合**：
1. **增强 C2C 挂牌交易**（模式 B，已实现）→ 补充价格发现 + 市场数据分析
...
**C2C 挂牌交易**（`market_listings` + `trade_orders`）：
...
│   └── ...
├── /api/v4/market        → routes/v4/market/            ← C2C用户端市场
...
│   │   ├── trade.js         ← TRADE_ENDPOINTS（C2C交易/挂牌/孤儿冻结/业务记录）
...
├── modules/
│   ├── market/
│   │   ├── pages/trade-management.js    ← C2C交易管理（Alpine.js Mixin v3.0）
...
| **市场定价**（基于成交均价） | Steam/Buff | 取近 7 天 C2C 市场成交均价作为汇率基准 | ⚠️ 仅 20 笔成交，数据不足，统计不可靠 |
...
```
/api/v4/market          ← C2C 用户交易市场（listings / sell / buy / manage / escrow）
