...
| **2. 提交前校验**   | 验证脚本                    | 扫描 `getService('camelCase')` 并阻断提交 |
...
| **service key 全量替换遗漏**     | 🟡 中  | 🔴 高 | 扫描脚本强制校验；提交前验证阻断；运行时报错含迁移提示     |
...
| **提交前验证**   | 提交前强制检测，发现旧 key 使用立即阻断                         | 🟡 被动防护 |
...
- [ ] 验证脚本已集成 blocking 模式检查（D2-Max + E2-Strict + G1-Unified）
...
# 2. 全仓替换（J2-RepoWide：routes/ + scripts/ + tests/ + services/）
...
> **评估日期**：2026年01月09日  
> **评估方式**：基于实际代码扫描和数据库连接分析  
> **评估结论**：项目现状优于预期，部分决策已自然达成

### 10.1 项目现状评估

| 维度                           | 当前状态                             | 决策对齐          | 需要改造                     |
| ------------------------------ | ------------------------------------ | ----------------- | ---------------------------- |
| **ServiceManager**             | ✅ 已存在，单例模式，注册 30+ 服务   | 符合              | 需改造 key 命名为 snake_case |
| **服务注入**                   | ✅ 已通过 `app.locals.services` 注入 | 符合 B1-Injected  | 基本符合，无需大改           |
| **启动时序**                   | ✅ 已是"初始化后监听"模式            | 符合 H1-InitFirst | 基本符合，无需大改           |
| **路由层直接 require Service** | ❌ 26 个文件，40 处                  | 不符合 G1-Unified | 需全部改造                   |
| **路由层直接 require Model**   | ✅ 0 处                              | 符合 D2-Max       | 已达标，无需处理             |
| **service key 命名**           | ❌ 全部 camelCase                    | 不符合 E2-Strict  | 需全量替换为 snake_case      |
| **C2-Lite 实例化**             | ✅ 按需实例化                        | 符合              | 无需额外改造                 |

#### 利好分析

1. **路由层已无直接 Model 调用**：D2-Max 中"禁止路由层 require Model"的部分已经达标（0 处违规），无需额外补充 Service 层读方法

2. **服务注入机制已就位**：`app.locals.services` 已配置，部分路由（如 `lottery/draw.js`）已在使用 `req.app.locals.services.getService()`

3. **启动时序基本符合**：`app.js` 已是"初始化完成才监听"的模式

4. **无需 C2-Lite 实例化改造**：当前 ServiceManager 中的服务已按需实例化（如 `UnifiedLotteryEngine` 是实例，其他静态类直接注册），无需额外改造

---

### 10.2 分模块执行清单

#### **模块 1：ServiceManager 核心改造**（预估 0.5 天）

**目标**：将 `services/index.js` 中的 service key 从 camelCase 改为 snake_case，并实现错误提示机制
...
1. **修改所有 `_services.set()` 调用**：将 30+ 个服务的 key 从 camelCase 改为 snake_case

2. **补充未注册的服务**：
   - 添加 `DataSanitizer` → `'data_sanitizer'`
   - 添加 `LotteryQuotaService` → `'lottery_quota'`（如存在）

3. **增强 `getService()` 错误提示**：
   - 当使用旧 key（camelCase）时，提供迁移建议
   - 输出可用的 snake_case key 列表

**完整 key 映射表**（基于当前 `services/index.js` 分析）：

| 旧 key (camelCase)       | 新 key (snake_case)                                                                                                                                |
| ------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| （单词类保持不变）       | `announcement`、`notification`、`consumption`、`user`、`feedback`、`image`、`activity`、`reporting`、`asset`、`idempotency`、`backpack`、`premium` |

---

#### **模块 2：高优先级路由改造（P0 写操作）**（预估 0.5 天）

**目标**：改造涉及关键业务写操作的路由，统一使用 `req.app.locals.services.getService()`

**范围**：6 个文件

| 文件路径                               | 直接 require 的 Service                      | 改造点 |
| -------------------------------------- | -------------------------------------------- | ------ |
| `routes/v4/market/sell.js`             | `IdempotencyService`、`MarketListingService` | 2 处   |
| `routes/v4/market/buy.js`              | `IdempotencyService` + 1                     | 2 处   |
| `routes/v4/lottery/draw.js`            | `DataSanitizer`、`IdempotencyService`        | 2 处   |
| `routes/v4/shop/exchange/exchange.js`  | `IdempotencyService`                         | 1 处   |
| `routes/v4/shop/consumption/submit.js` | `IdempotencyService`                         | 1 处   |
| `routes/v4/shop/assets/convert.js`     | `IdempotencyService`                         | 1 处   |
...
// 改造后（在路由处理函数内获取）
router.post('/xxx', authenticateToken, async (req, res) => {
  const services = req.app.locals.services
  const IdempotencyService = services.getService('idempotency')
  const MarketListingService = services.getService('market_listing')
  // ...
...
| 文件路径                                 | 直接 require 处数 |
| ---------------------------------------- | ----------------- |
| `routes/v4/console/marketplace.js`       | 4 处              |
| `routes/v4/console/campaign-budget.js`   | 4 处              |
| `routes/v4/console/merchant-points.js`   | 2 处              |
| `routes/v4/console/asset-adjustment.js`  | 2 处              |
| `routes/v4/console/shared/middleware.js` | 3 处              |
| `routes/v4/console/lottery-quota.js`     | 1 处              |
| `routes/v4/console/system/audit-logs.js` | 1 处              |
| `routes/v4/console/user_management.js`   | 1 处              |
| `routes/v4/console/user-hierarchy.js`    | 1 处              |

---

#### **模块 4：低优先级路由改造（读操作/工具服务）**（预估 0.5 天）
...
| 文件路径                               | 直接 require 处数 |
| -------------------------------------- | ----------------- |
| `routes/v4/market/manage.js`           | 1 处              |
| `routes/v4/market/listings.js`         | 1 处              |
| `routes/v4/merchant-points.js`         | 1 处              |
| `routes/v4/activities.js`              | 1 处              |
| `routes/v4/shop/exchange/items.js`     | 1 处              |
| `routes/v4/shop/exchange/orders.js`    | 1 处              |
| `routes/v4/shop/consumption/qrcode.js` | 1 处              |
| `routes/v4/system/chat.js`             | 1 处              |
| `routes/v4/system/feedback.js`         | 1 处              |
| `routes/v4/system/status.js`           | 1 处              |
| `routes/v4/lottery/prizes.js`          | 2 处              |

---

#### **模块 5：全仓统一（scripts/ + tests/）**（预估 1 天）

**目标**：按 J2-RepoWide 决策，统一 scripts/ 和 tests/ 中的服务调用方式

**5.1 scripts/ 目录处理**

需要先扫描 `scripts/` 目录下的服务调用：

- 识别哪些脚本使用了 `require('...services/...')`
- 评估每个脚本的改造价值（一次性脚本 vs 常用脚本）
- 对于需要 ServiceManager 的脚本，改为先初始化后获取

**特殊处理**：

- 一次性迁移脚本可保留直接 require（标记为例外）
- 验证脚本需要改造以检测新的调用方式

**5.2 tests/ 目录处理**

- 统一测试中的服务获取方式
- 更新 mock/stub 策略以适配新的获取方式
- 确保测试可以正常 mock ServiceManager

---

#### **模块 6：门禁与验证脚本**（预估 0.5 天）

**目标**：创建验证脚本，实施 I1-ImmediateBlock 策略

**6.1 创建验证脚本**

| 脚本                                                  | 功能                                  |
| ----------------------------------------------------- | ------------------------------------- |
| `scripts/validation/verify-service-manager-usage.js`  | 扫描路由层直接 require Service 的情况 |
| `scripts/validation/verify-all-keys-migrated.js`      | 检测是否有旧 key（camelCase）使用     |
| `scripts/validation/verify-route-layer-compliance.js` | 检测路由层是否直接操作 Model          |
...
| 风险项                     | 风险等级 | 应对措施                                                      |
| -------------------------- | -------- | ------------------------------------------------------------- |
| **E2-Strict 全量替换遗漏** | 🟡 中    | 一键替换脚本 + 验证脚本兜底 + 运行时错误提示                  |
| **scripts/ 目录复杂度**    | 🟡 中    | 该目录有 60+ 个脚本，需评估哪些需要改造，一次性脚本可标记例外 |
| **测试覆盖**               | 🟡 中    | 改造后需确保 479+ 测试全部通过                                |
| **停更窗口协调**           | 🟢 低    | 提前通知相关人员，暂停其他功能开发                            |

---

### 10.5 成功标准校验清单

基于 10.1 评估结果，更新后的成功标准：

**已达成（无需额外工作）**：

- [x] 路由层不再直接 `require('...models/XXX')`（D2-Max Model 部分）
- [x] `app.js` 启动时序已调整为"初始化完成才监听"（H1-InitFirst）
- [x] ServiceManager.initialize() 在 app.listen() 之前完成

**需要完成**：

- [ ] 所有路由文件不再直接 `require('...services/XXXService')`（26 个文件，40 处）
- [ ] 所有服务通过 `req.app.locals.services.getService()` 获取
- [ ] 全仓 service key 已全量替换为 snake_case（routes/ + scripts/ + tests/）
- [ ] ServiceManager 只注册 snake_case key（旧 key 运行时报错）
- [ ] 门禁已切到 blocking 模式（验证脚本 --strict）
...
**文档版本**：3.2.0（新增第 10 章：实际代码状态评估与分模块执行清单）  
**最后更新**：2026年01月09日  
**负责人**：后端架构组  
