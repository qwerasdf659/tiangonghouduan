...
> **状态**: ✅ 后端全部实施完成（2026-02-21），前端可直接对接
>
> **后端完成清单**:
> - ✅ P0: rarity 字典补录（5 条 system_dictionaries）— `rarity_display`/`rarity_color` 已恢复正常
> - ✅ P1: 级联删除 272 条测试数据（197 instances + 69 events + 6 trade_orders + 6 listings）
> - ✅ P1: item_type 加 NOT NULL 约束（严格模式）、source 字段回填 + NOT NULL 约束
> - ✅ P2: 新增 system_configs 背包配置（backpack_use_instructions + item_type_action_rules）
> - ✅ P2: `/use` 路由返回 `instructions` 字段
> - ✅ P3: 背包列表返回 `allowed_actions` 字段
> - ✅ P3: Web 管理后台新增 backpack 分类 tab + 模板编辑增加 use_instructions/allowed_actions
...
```json
{
  ...
  "data": {
    ...
    "status": "used",
    "is_duplicate": false,
    "instructions": "服务已激活，有效期内可享受对应权益。"
  }
}
```

> ✅ **2026-02-21 已实现**: `instructions` 字段已在 `/use` 路由中返回，优先级：模板级 meta.use_instructions > item_type 级配置 > null
