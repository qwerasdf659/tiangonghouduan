...
> **状态**: 后端已求证，发现 1 个现有 BUG（rarity 字典缺失），需用户拍板 5 项决策
...
**需要拍板**：是往 system_dictionaries 补录 5 条 `dict_type='rarity'` 记录（推荐，与现有 `attachDisplayNames` 机制一致），还是修改 displayNameHelper 逻辑去查 rarity_defs 表。
...
- 通过 ServiceManager 获取 `system_configs` 中 `backpack_use_instructions` 配置
...
### 步骤 1（P1）：数据清洗 — null item_type 测试数据

**问题**：197 条 `item_type` 为 NULL 的记录，经查全部属于用户 31（测试账号）和用户 135（测试账号），名称为 `生命周期测试物品_*` 和 `挂牌生命周期测试物品_*`。

**执行方式**：Sequelize migration

```sql
-- 将 197 条测试数据标记为 expired
UPDATE item_instances 
SET status = 'expired', item_type = 'tradable_item', updated_at = NOW()
WHERE item_type IS NULL;

-- 加 NOT NULL 约束（防止未来再出现）
ALTER TABLE item_instances MODIFY COLUMN item_type VARCHAR(50) NOT NULL DEFAULT 'product';
```

**需要拍板**：
- 测试数据标记 `expired` 还是直接 `DELETE`？（推荐 expired，保留审计痕迹）
- `item_type` 的 `NOT NULL DEFAULT 'product'`：默认值用 `product` 还是要求调用方必须显式传入？

**归属：后端数据库问题**

### 步骤 2（P2）：新增 system_configs 配置

通过 Sequelize migration 插入两条配置：
...
**影响范围**：
- 修改文件：同一个 migration 文件（与步骤 0、1 合并）
...
1. 通过 `req.app.locals.services.getService('xxx')` 获取 SystemConfig 服务（需确认 ServiceManager 中注册的服务名）
2. 查询 `config_key = 'backpack_use_instructions'` 配置
3. 用物品的 `item_type` 从配置 JSON 中取出对应 instructions
4. 如果物品有 `item_template_id` 且模板 `meta.use_instructions` 不为空，优先使用模板级文案
5. 附加到响应 data 中

**可复用**：
- ServiceManager 已注册所有服务，通过 `getService()` 获取即可
- SystemConfig 模型已有 `getValue()` 方法读取 JSON
...
1. 从 `system_configs` 读取 `item_type_action_rules`（可复用 SystemConfig.findOne + getValue）
2. 建议加内存缓存（5 分钟 TTL），避免每次请求都查数据库。参考项目中 `StrategyConfig.js` 的缓存模式（`STRATEGY_CONFIG_CACHE_TTL` 环境变量控制）
3. 为每个 item 根据 `item_type` 附加 `allowed_actions` 数组
4. 如果物品有 `item_template_id`，查询模板 meta.allowed_actions 做覆盖（当前所有模板 meta 为 NULL，此逻辑不会生效但为未来预留）

**可复用**：
- BackpackService._getItems() 已有类似的「批量查询 + Map 映射」模式（redemption_orders 的 has_redemption_code）
- 代码结构参考 `_getItems()` 第 3 步的 `hasCodeMap` 模式
...
- 现有分类通过 composable 动态加载，新分类只需数据库有对应记录即可被发现
- 如果分类列表是前端硬编码的，需在数组中增加 `{ key: 'backpack', label: '背包配置' }`
...
2. 直接将 197 条标记为 `expired`（保留审计痕迹，不删除）
3. 清洗完成后，对 `item_type` 字段加 NOT NULL 约束（Sequelize migration）
4. source 字段：6559 条全部为 NULL，本次暂不回填历史数据，仅在 `ItemService.mintItem()` 中确保新记录必填 source
...
| ServiceManager 模式 | 获取 system_configs 配置服务 | ✅ 完全复用 | 需确认 system_config 服务在 ServiceManager 中的注册名 |
| SystemConfig 模型 | getValue()/getProperty() 方法 | ✅ 完全复用 | 直接用 `SystemConfig.findOne({ where: { config_key } })` |

**不需要**：新建表、新建模型、新建服务类。
**需要**：1 个 migration 文件（合并步骤 0/1/2 的数据变更）+ 修改 2 个后端文件 + 修改 2 个前端文件。
...
| system-settings 管理页（396行） | 增加 backpack 分类 tab | ⚠️ 需扩展 | 现有 5 个分类，需增加第 6 个 |
| composable 模式（config.js, 337行） | 按分类动态加载配置 | ✅ 完全复用 | `loadCategoryConfig(category)` 已支持任意分类 |
...
| # | 决策项 | 选项 A（推荐） | 选项 B | 影响 |
|---|--------|--------------|--------|------|
| D1 | **rarity 字典缺失修复方式** | 往 system_dictionaries 补录 5 条 `dict_type='rarity'` 记录（推荐：与 attachDisplayNames 机制一致，零代码改动） | 修改 displayNameHelper 逻辑，查 rarity_defs 表 | 选 A 只需 migration 插数据；选 B 要改工具函数，影响所有使用 attachDisplayNames 的服务 |
| D2 | **197条 NULL item_type 测试数据处理** | 标记为 `expired`（推荐：保留审计痕迹） | 直接 DELETE | 选 A 安全可追溯；选 B 更干净但不可逆 |
| D3 | **item_type 的 NOT NULL 默认值** | 不设默认值，NOT NULL 且无 DEFAULT（推荐：强制调用方显式传入，防止再出现 NULL） | DEFAULT 'product' | 选 A 更严格但需确保所有 mint 路径都传 item_type；选 B 宽松但可能掩盖 bug |
| D4 | **source 字段（6559条全 NULL）处理** | 本次暂不回填，仅在 ItemService.mintItem() 中确保新记录必填 source（推荐：老数据量大，回填 ROI 低） | 通过 item_instance_events 的 business_type 回填历史 source | 选 A 工作量低，止损即可；选 B 数据完整但需要写回填脚本 |
| D5 | **system-settings 管理页 backpack 分类** | 在现有 system-settings.js 的分类数组中增加 `backpack` 选项（推荐） | 不新增分类，运营通过直接编辑 JSON 配置 | 选 A 用户友好；选 B 省前端改动但运营难用 |
...
| null item_type 处理 | 严格清洗 + NOT NULL 约束加固 | Oracle/AWS：未上线是清洗最佳时机 |
...
| 优先级 | 内容 | 归属 | 影响 | 修改文件 |
|--------|------|------|------|----------|
| **P0** | 修复 rarity 字典缺失（已有 BUG） | 后端数据库 | `rarity_display`/`rarity_color` 恢复正常 | 1 个 migration |
| **P1** | 数据清洗 197 条 NULL item_type + 加 NOT NULL 约束 | 后端数据库 | 数据质量基础 | 同上 migration |
...
| **P3** | system-settings 增加 backpack 分类 | Web管理后台前端 | 运营可管理背包配置 | `admin/src/modules/system/pages/system-settings.js` |
...
| 文件 | 改动类型 | 说明 |
|------|---------|------|
| `migrations/xxxxxx-fix-rarity-dict-and-backpack-configs.js` | **新建** | 合并：补录 rarity 字典 + 清洗 NULL item_type + 加 NOT NULL 约束 + 插入 system_configs |
