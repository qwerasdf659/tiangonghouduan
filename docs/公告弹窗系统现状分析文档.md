...
> **状态**: 7 项决策已拍板 + ✅ 系统性缺陷已全部修复（6 处代码 + 测试 + 旧版文件 + 定时任务 + 前端构建）
> **四次更新时间**: 2026-02-21（全量代码修复 + 测试验证通过 + ESLint/Prettier通过 + 前端构建完成 + 定时任务上线）
...
**响应格式**（经 `DataSanitizer.sanitizeAnnouncements()` 脱敏后，✅ 2026-02-21 已修复）:
```json
{
  "success": true,
  "data": {
...
**✅ 字段命名修复完成（2026-02-21 四次更新）**: 公开 API 和管理 API 统一输出 `announcement_id`（映射自数据库主键 `system_announcement_id`），与 `popup_banner_id`、`carousel_item_id` 命名模式一致。管理 API 同时保留原始字段 `system_announcement_id`（用于 Sequelize 内部操作）。
...
| **公告 `announcement_id` 幽灵字段** | **✅ 已修复（2026-02-21）** | **原问题：DataSanitizer、路由、Service 层共 6 处引用不存在的字段。已全部修复：DataSanitizer 统一输出 `announcement_id`（映射自 `system_announcement_id`）、Service 层 SQL 列名修正、console 路由日志修正、通知路由修正、旧版 utils/DataSanitizer.js 同步修正、测试文件修正、测试残留数据清理** |
...
#### ✅ 系统性缺陷已修复：`announcement_id` 幽灵字段（三次求证发现，四次更新修复完成 2026-02-21）
...
### 实施优先级总览（2026-02-21 四次更新：全部已完成）

| 优先级 | 步骤 | 归属 | 状态 | 验证结果 |
|--------|------|------|------|---------|
| **P0** | 步骤 1: DataSanitizer 主键映射（B1） | 后端 | ✅ 已完成 | 公开API返回 `announcement_id: 整数` |
| **P0** | 步骤 2: `/home` 路由 view_count（B2） | 后端 | ✅ 已完成 | DataSanitizer修复后自动生效 |
| **P0** | 步骤 3: Service 层 SQL 列名（B3+B4） | 后端 | ✅ 已完成 | 20/20测试全部通过 |
| **P1** | 步骤 4: console 字段名统一 + 日志（B5） | 后端 | ✅ 已完成 | console API返回 `announcement_id + system_announcement_id` |
| **P1** | 步骤 5: Web 前端字段名清理（W1） | Web 前端 | ✅ 已完成 | 前端构建成功 |
| **P1** | 步骤 6: 模型 JSDoc 注释（B6） | 后端 | ✅ 已完成 | ESLint通过 |
| **P2** | 步骤 7: 过期自动清理 cron | 后端 | ✅ 已完成 | `jobs/content-cron-jobs.js` 已创建并注册，环境变量 `ENABLE_CONTENT_CRON_JOBS=true` |
| **P2** | 步骤 8: 轮播图比例校验 | Web 前端 | ✅ 已完成 | `checkCarouselImageRatio()` 已添加到 carousel-items composable |

#### 额外修复（排查发现）

| 修复项 | 文件 | 说明 |
|--------|------|------|
| 通知路由创建日志修复 | `routes/v4/system/notifications.js` | `announcement.announcement_id` → `announcement.system_announcement_id`（createAnnouncement返回原始字段） |
| 旧版 DataSanitizer 同步修正 | `utils/DataSanitizer.js` | 与 `services/DataSanitizer.js` 保持一致的字段映射 |
| 测试文件幽灵字段全量修正 | `tests/services/AnnouncementService.test.js` | 5处 `announcement.announcement_id` → `announcement.system_announcement_id`（createAnnouncement返回） |
| 测试残留数据清理 | 数据库 `system_announcements` 表 | 清理5条因旧测试清理失败遗留的测试数据 |
...
## 十五、微信小程序前端对接指南（2026-02-21 四次更新后）

> 后端所有缺陷已修复，以下为小程序前端对接的接口说明和排查工作清单。

### 15.1 公告系统对接（修复后的接口）

**接口**: `GET /api/v4/system/announcements/home`

**说明**: 返回首页活跃公告（最多5条），按优先级降序+创建时间降序排列。每次请求自动累加 `view_count`。

**响应字段**（修复后）:

| 字段 | 类型 | 说明 |
|------|------|------|
| `announcement_id` | number | 公告ID（修复后正确输出，之前为 undefined） |
| `title` | string | 公告标题 |
| `content` | string | 公告内容 |
| `type` | string | 公告类型：system/activity/maintenance/notice |
| `type_display` | string | 类型中文名：系统公告/活动公告/维护公告/通知公告 |
...
| `creator` | object/null | 发布者信息 |
| `creator.user_id` | number | 发布者用户ID |
| `creator.nickname` | string | 发布者昵称 |

### 15.2 弹窗Banner系统对接（无缺陷，直接对接）

**接口**: `GET /api/v4/system/popup-banners?status=active&position=home`

**说明**: 返回指定位置的活跃弹窗列表，已按优先级排序，已合并广告竞价结果。

**响应字段**:

| 字段 | 类型 | 说明 |
|------|------|------|
| `popup_banner_id` | number | 弹窗ID |
| `title` | string | 弹窗标题（管理标识用） |
| `image_url` | string | 图片URL（Sealos对象存储key，需通过 `ImageUrlHelper.getImageUrl()` 生成完整CDN URL） |
| `display_mode` | string | 显示模式：wide(16:9)/horizontal(3:2)/square(1:1)/tall(3:4)/slim(9:16)/full_image |
| `image_width` | number | 原图宽度 |
| `image_height` | number | 原图高度 |
| `link_url` | string | 点击跳转URL |
| `link_type` | string | 跳转类型：none/page/miniprogram/webview |
| `banner_type` | string | 弹窗类型：notice/event/promo |
| `frequency_rule` | string | 频率规则：always/once/once_per_session/once_per_day/once_per_n_days/n_times_total |
| `frequency_value` | number | 频率参数（配合 N 天/N 次使用） |
| `force_show` | boolean | 是否强制弹出（true时不可点遮罩关闭） |
| `priority` | number | 弹出竞争优先级（数字越大越优先） |

**小程序端需实现的逻辑**:
1. 根据 `frequency_rule` + `frequency_value` 在本地存储中判断是否弹出（后端只下发规则，客户端执行）
2. `force_show=true` 时不允许点遮罩关闭，需点"我知道了"按钮
3. 弹窗按 `priority` 降序依次弹出
4. `link_type=page` 时使用 `wx.navigateTo({ url: link_url })` 跳转

### 15.3 弹窗展示上报接口

**接口**: `POST /api/v4/system/ad-events/popup-banners/show-log`（需登录）

**请求体**:

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `popup_banner_id` | number | 是 | 弹窗ID |
| `show_duration_ms` | number | 是 | 展示时长（毫秒） |
| `close_method` | string | 是 | 关闭方式：close_btn/overlay/auto_timeout |
| `queue_position` | number | 否 | 弹窗在队列中的位置（从0开始） |

### 15.4 小程序前端排查工作清单

| 编号 | 排查项 | 目的 | 优先级 | 后端状态 |
|------|--------|------|--------|---------|
| MP-1 | 确认是否调用 `GET /api/v4/system/announcements/home` | 文字公告功能 | P1 | ✅ 后端已修复，`announcement_id` 字段正确输出 |
| MP-2 | 确认是否调用 `GET /api/v4/system/popup-banners?status=active` | 弹窗功能 | P1 | ✅ 后端无缺陷，直接对接 |
| MP-3 | 确认弹窗 `frequency_rule` 客户端执行逻辑 | 频率控制 | P2 | ✅ 后端正确下发规则 |
| MP-4 | 确认弹窗展示上报是否实现 | 展示日志完整性 | P2 | ✅ popup_show_logs 表正常（33条数据） |
| MP-5 | 确认 `link_type=page` 跳转 | 弹窗点击跳转 | P3 | ✅ 后端正确返回 link_url |
| MP-6 | 确认文字公告展示形态 | 运营策略匹配 | P3 | ✅ 后端返回 type/priority/creator 完整数据 |
