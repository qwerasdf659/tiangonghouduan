...
- **路由 PR 必须检查**：
  ...
  - 是否使用统一响应方法（不出现 `res.json({ success: ... })` 手写结构）。  
...
- 被复用的底层方法（如 `PointsService.consumePoints`、`InventoryService.transferItem`）  
...
| 领域 | 负责表 | Service | 核心方法示例 |
|------|--------|---------|-------------|
| 积分 | user_points_accounts<br>points_transactions | PointsService | addPoints()<br>consumePoints()<br>refundPoints() |
| 库存 | user_inventory<br>trade_records | InventoryService | getUserInventory()<br>useItem()<br>transferItem() |
| 兑换市场 | exchange_items<br>exchange_market_records | ExchangeMarketService | getMarketItems()<br>exchangeItem() |
| 审核 | content_review_records<br>admin_operation_logs | ContentAuditEngine<br>AuditLogService | createAudit()<br>approve()<br>reject() |
...
- 每个模型注释里已经约定了业务含义，例如：
  ...
  - `ExchangeRecords` + `UserInventory` + `TradeRecord`：兑换订单 + 库存 + 转让记录。  
...
- **幂等**：
  ...
  - 审查时如果看到有新的扣分/加分逻辑，但没有幂等机制，视为 P0 问题。
...
**文档维护**：
- 最后更新：2025-12-11
- 版本：v5.1（新增规范分级与适用性说明）

**更新记录**：
- v5.1（2025-12-11）：
  - 新增"规范执行优先级与适用性说明"章节
  - 明确区分 P0（核心强制）/ P1（重要建议）/ P2（长期优化）三级规范
  - 新增规范适用性自查清单（快速判断应遵守的规范级别）
  - 新增与当前项目代码的对齐说明（强调基于实际代码，非历史报告）
  - 新增规范执行建议（新功能开发 / 存量代码维护 / Code Review）
- v5.0（2025-12-11）：
  - 合并 ABC 文档的规则部分，删除所有时间/阶段/计划性内容
  - 新增胖路由硬性指标（行数/models 调用次数/业务分支层数）
  - 新增实际案例作为反面教材（inventory.js 2933 行 / points.js 1361 行）
  - 统一事务/幂等/错误处理的标准模式（基于 PointsService）
  - 补充 Service 拆分与合并的具体建议（ReportingService / AdminSystemService）
  - 新增错误处理与日志规范章节
  - 删除所有"Week/Day/M1/验收标准/任务清单/时间表"等实施性内容
  - 文档定位：纯规则/标准文档，不含实施计划
