...
```javascript
// routes/v4/unified-engine/backpack.js（重构后）
router.get('/user/:user_id', authenticateToken, async (req, res) => {
  try {
    const { user_id } = req.params
    ...
    const backpack = await BackpackService.getUserBackpack(userId, {
    ...
    return res.apiError('获取背包失败', 'INTERNAL_ERROR', null, 500)
  }
})
```
...
router.post('/use/:item_id', authenticateToken, async (req, res) => {
  try {
    ...
    const result = await InventoryService.useItem(req.user.user_id, req.params.item_id, {
...
const EXCHANGE_ATTRIBUTES = {
  // 用户视图：用户查看自己的兑换订单
  userView: ['exchange_id', 'item_name', 'points_cost', 'status', 'created_at', 'updated_at'],
...
- v5.2 (2025-12-12)：
  ...
  - 新增"兑换市场专项检查清单"（第9.6节）
- v5.3 (2025-12-17)：
  - P0：明确“物品/资产单一真相”——不可叠加物品以 `ItemInstance` 为唯一真相、可叠加资产以统一账本为唯一真相；文档层面彻底移除任何“第二套库存真相”表的描述
  - P0：路由层禁止直连 models 写操作；仅允许极少数只读安全校验特例，并要求尽快下沉到 Service/中间件
  - P0：兑换市场“禁 points”升级为“业务逻辑 + 数据结构”双重禁止（枚举/校验/DB约束）
  - P0/P1：统一响应几乎全覆盖，明确导出/410 为允许特例并要求写清注释与迁移路径
