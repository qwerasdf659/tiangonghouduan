...
**清理**：用户资产余额/流水、抽奖记录、消费记录、交易记录、监控指标、操作日志
...
|------|------|-------|-------|
| `stores` | 门店配置 | 保留 | 清空 |
| `store_staff` | 门店员工关系 | 保留 | 清空 |
| `products` | 商品配置 | 保留 | 清空 |
| `user_hierarchy` | 用户层级关系 | 保留 | 清空 |
...
| `system_announcements` | 系统公告 | 保留 | 清空 |
...
| `popup_banners` | 弹窗Banner | 保留 | 清空 |
| `image_resources` | 图片资源 | 保留 | 清空 |
...
| `system_announcements` | 系统公告 | 全部保留 |
...
3. 对约37张业务表执行 `TRUNCATE TABLE`（但 `accounts` 只删 `account_type='user'` 的记录）
...
1. 连接 `.env` 指向的真实数据库
2. 执行 `SET FOREIGN_KEY_CHECKS = 0;`
3. 对约69张表执行 `TRUNCATE TABLE`（保留 `sequelizemeta` 和 `administrative_regions`）
4. 执行 `SET FOREIGN_KEY_CHECKS = 1;`
5. 重跑迁移重建系统账户
...
**清理对象**：约37张业务数据表  
**保留对象**：配置、用户、系统账户、活动定义、策略配置、功能开关、字典数据
