...
通过连接真实数据库验证，账户模块的问题**比奖品模块更严重**——不是"设计上有隐患"，而是**已经产生了实际的数据不一致**：

1. **余额与流水对不上** — 跑 `SUM(所有流水delta)` 对比 `当前余额`，有 **20 个账户不一致**。例如 account_id=6 的 DIAMOND 余额 345,700，但流水合计只有 110,400，凭空多出 235,300 钻石。
2. **15 个账户有余额但没有任何流水** — account_id=16,17,175,176 各有 DIAMOND 5000 + red_shard 2000 + red_crystal 200 + orange_shard 500 的余额，但 asset_transactions 里一条记录都没有。资产凭空出现。
3. **BIGINT 溢出** — account_id=7 有一条 `admin_data_fix` 流水的 delta_amount 为 -9223372036854775807（BIGINT 极限值），导致该账户和 DIAMOND 全局的 SUM 计算溢出，对账完全失效。
...
| 隐患 | 严重程度 | 说明 |
|---|---|---|
| **物品可以绕过追踪被创建** | 高 | BidService 竞价结算直接调用 `ItemInstance.create()`，绕过了 `ItemService.mintItem()`，导致竞价获得的物品没有铸造事件记录，在事件追踪中完全不可见 |
| **两份"真相"可以互相矛盾** | 高 | `item_instances.status` 和 `item_instance_events` 最后一条事件可以不一致，一旦发生，无法判断哪个是对的 |
| **52% 的物品来源不明** | 中 | 3,472 条物品实例的 source='unknown'，无法区分是抽奖、兑换还是测试产生的 |
| **锁定状态存在 JSON 里无法查询** | 中 | `item_instances.locks` 是 JSON 数组，不可索引。查"哪些物品的锁已过期"需要全表扫描并在应用层逐条解析 |
...
| 隐患 | 严重程度 | 实际数据 |
|---|---|---|
| **20 个账户余额与流水不一致** | 高 | account_id=6 DIAMOND 余额 345,700 vs 流水合计 110,400，差额 235,300 |
| **15 个账户有余额但无任何流水** | 高 | 4 个用户各有 DIAMOND+red_shard+red_crystal+orange_shard 余额，asset_transactions 中 0 条记录 |
| **BIGINT 溢出污染对账** | 高 | account_id=7 一条 admin_data_fix 流水 delta=-9223372036854775807，导致 DIAMOND 全局 SUM 溢出 |
| **单式记账无法验证全局守恒** | 中 | 当前只记每个账户的 delta，不记对手方。无法用 SUM(delta)==0 验证"钻石总量没有凭空增减" |
| **管理员调账无审批链** | 中 | admin_adjustment 只有文本 reason 和 admin_id，没有审批单号、审核状态、上级确认 |
| **资产可以绕过 BalanceService 写入** | 中 | 15 个无流水账户证明存在绕过正常流程直接写余额的路径 |
...
| 表 | 职责 | 真实数据量 |
|---|---|---|
| `item_instances` | 物品实例（当前状态） | 6,758 条 |
| `item_instance_events` | 物品事件记录（事件溯源） | 5,345 条 |
| `lottery_draws` | 抽奖记录 | 3,030 条 |
| `lottery_prizes` | 奖品定义（奖品池） | 按活动配置 |
| `redemption_orders` | 兑换订单 | 1,309 条 |
| `market_listings` | 交易市场挂牌 | 295 条 |
| `trade_orders` | 交易订单 | 134 条 |
| `asset_transactions` | 资产流水（积分/水晶） | 持续增长 |
| `accounts` | 账户（用户+系统） | 支持 SYSTEM_MINT/BURN/PLATFORM_FEE/ESCROW |
...
| available | unknown | 1,198 |
| used | unknown | 1,406 |
| locked | unknown | 588 |
| transferred | unknown | 280 |
| transferred | test | 144 |
| 其他 | 其他 | 243 |

**关键发现**：`source='unknown'` 的实例占 52%（3,472 条），这是早期数据缺乏来源标记的历史债务。
...
| G2 | `lottery_draws` → `item_instances` 断链 | 无法从抽奖记录追到具体哪个物品实例（meta 中没有 lottery_draw_id，真实数据验证 0 条关联） |
| G3 | `BidService` 竞价结算没写 event | 直接调用 `ItemInstance.create()` 绕过了 `ItemService.mintItem()`，没有 mint 事件 |
| G4 | 历史数据 source=unknown 占 52% | 过半物品来源不明 |
| G5 | 没有统一的全链路查询接口 | 无法一次性看到完整流转轨迹 |
| G6 | `item_instances` 和 `item_instance_events` 双真相 | 两份记录可能不一致，需要人工比对排查 |
| G7 | `locks` 用 JSON 存储 | 不可索引、不可查询、过期清理依赖应用层代码 |
...
**这就是本项目目前的状态**：item_instances 的 source=unknown 占 52%，20 个账户余额与流水不一致，15 个账户凭空有余额但无任何流水。
...
### 8.2 路径 B：从零三表模型（推倒重来）

| 维度 | 说明 |
|---|---|
| 改动量 | ItemService 重写 + ItemInstance 模型重构 + 新建 holds 和 ledger |
| 风险 | 中（改动面更大，但项目未上线） |
...
对真实数据库执行 `SUM(所有流水delta)` vs `当前余额(available+frozen)` 校验：

**不一致的典型案例**：

| 账户 | 资产 | 当前余额 | 流水合计 | 差额 | 问题 |
|---|---|---|---|---|---|
| account_id=5 | POINTS | 1,518,860 | 57,099,784 | +55,580,924 | 流水远多于余额（历史清洗余额但未清流水） |
| account_id=6 | DIAMOND | 345,700 | 110,400 | -235,300 | 余额多出23.5万（直接改库） |
| account_id=13 | DIAMOND | 245,600 | 37,000 | -208,600 | 余额多出20.8万 |
| account_id=7 | DIAMOND | 78,320 | 溢出 | BIGINT溢出 | admin_data_fix 写入极限值 |
| account_id=16 | DIAMOND | 5,000 | 0 | -5,000 | 有余额但0条流水 |

**有余额但完全无流水的 15 个余额记录**（涉及 account_id=16,17,175,176）：

每个账户都有 DIAMOND 5000 + red_shard 2000 + red_crystal 200 + orange_shard 500 的余额，但 asset_transactions 中无任何记录。这些资产是通过直接操作数据库或绕过 BalanceService 写入的。
...
2. 账户余额一致：每个账户的 `SUM(delta)` 是否等于 `available + frozen`
3. 无流水余额检测：是否有余额不为 0 但无流水的记录
...
| 可复用模式 | 现有代码示例 | 用于三表模型 |
|---|---|---|
| **Sequelize Model 定义** | `models/ItemInstance.js`（571 行，含 init/associate/索引/约束） | 直接用于新建 Item.js、ItemLedger.js、ItemHold.js |
| **Domain Service 模式** | `services/asset/ItemService.js`（934 行，static methods + transaction 参数） | ItemLifecycleService.js 沿用同一模式 |
...
| 文件 | 改动内容 | 影响范围 | 风险 |
|---|---|---|---|
| `services/asset/ItemService.js` | **核心重写**：mintItem → 同时写 items + item_ledger 双录；transferItem → 双录转移；consumeItem → 双录核销 | 高 | 中（所有物品操作入口） |
| `services/exchange/BidService.js` | settleBidProduct 中 **`ItemInstance.create()` → `ItemService.mintItem()`**（当前直接创建绕过服务层，无 mint 事件） | 中 | 低（bid_products 当前 0 条数据，功能未投产） |
| `services/UnifiedLotteryEngine/pipeline/stages/SettleStage.js` | `_distributePrize` 中 coupon/physical 分支：将 lottery_draw_id 传入 ItemService.mintItem 的 meta 参数 | 低 | 低（仅增加一个参数） |
...
| 资产双录升级 | BalanceService.changeBalance 增加对手方账户参数 | 1 天 |
...
| 旧表 | 替代为 | 数据迁移方式 |
|---|---|---|
| `item_instances`（6,758 条） | `items`（缓存层） | 逐条迁移，meta JSON 中的 name/value/prize_id 提取为正式列 |
| `item_instance_events`（5,345 条） | `item_ledger`（真相层） | 每条 mint/transfer/use 事件转为双录（出方 -1 + 入方 +1） |
...
项目未上线，没有用户影响。一次性切换到三表模型，不做新旧并行。代码中所有 `ItemInstance` 引用全部替换为 `Item`，不保留兼容分支。
...
### 决策 5：source=unknown 的 3,472 条物品 → **B. 有 mint 事件的回填，其余标 legacy**

- 2,992 条有 lottery mint 事件 → 回填 `source='lottery'`，`source_ref_id` 从事件 meta 中提取
- 其余 480 条无 mint 事件 → 标记 `source='legacy'`
...
| 4.7 | 回填 source 字段：2,992 条有 lottery mint 事件的回填 source='lottery'，其余标记 source='legacy' |
...
| 步骤 | 具体操作 |
|---|---|
| 5.1 | 修改 `services/asset/BalanceService.js` 的 `changeBalance` 方法：增加 `counterpart_account_id` 参数，每次变动写两条 asset_transactions（出方 + 入方） |
