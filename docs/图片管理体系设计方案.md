...
---这个项目不接入CDN  、P0：运营上传真实商品图片这一步先不用管  
...
### 2.4 category_defs 图标现状（2026-02-21 已完成）

> **2026-02-21 更新**：脏数据已清理（删除 chen112/auto/beauty），9 个分类图标已全部制作并上传 Sealos。

清理后保留 9 个分类，图标已全部就位：

| category_code | display_name | icon_url | 状态 |
|---|---|---|---|
| electronics | 电子产品 | `categories/1771673715022_...png` | **已完成** |
| food_drink | 餐饮美食 | `categories/1771673715054_...png` | **已完成** |
| voucher | 优惠券 | `categories/1771673715089_...png` | **已完成** |
| gift_card | 礼品卡 | `categories/1771673715103_...png` | **已完成** |
| home_life | 家居生活 | `categories/1771673715118_...png` | **已完成** |
| lifestyle | 生活日用 | `categories/1771673715133_...png` | **已完成** |
| food | 美食饮品 | `categories/1771673715146_...png` | **已完成** |
| collectible | 收藏品 | `categories/1771673715164_...png` | **已完成** |
| other | 其他 | `categories/1771673715176_...png` | **已完成** |

**前端静态资源**：`admin/public/assets/icons/categories/`（暗黑奢华风 256×256 PNG，~360KB 总计）
**前端映射表**：`admin/public/assets/icons/categories/_mapping.js`（`CATEGORY_ICONS` + `getCategoryIconUrl()`）
...
### 5.3 分类图标方案（已完成 2026-02-21）

**已生成 & 存放位置**：`admin/public/assets/icons/categories/`（暗黑奢华风 256×256 PNG）

```
admin/public/assets/icons/categories/
├── electronics.png     # 电子产品       42 KB
├── food-drink.png      # 餐饮美食       40 KB
├── voucher.png         # 优惠券         45 KB
├── gift-card.png       # 礼品卡         45 KB
├── home-life.png       # 家居生活       29 KB
├── lifestyle.png       # 生活日用       45 KB
├── food.png            # 美食饮品       44 KB
├── collectible.png     # 收藏品         45 KB
├── other.png           # 其他           28 KB
│                       ────────────────────
│                       总计: ~363 KB（9 个文件）
└── _mapping.js         # category_code → 文件路径映射表
```
...
**数据库 icon_url**：9 个分类的 `icon_url` 已全部上传到 Sealos `categories/` 目录并更新。管理台通过 Sealos URL 显示。
...
| ~~P0~~ | ~~图标上传 Sealos + 更新 material_asset_types.icon_url~~ | ~~决策 6~~ | **已完成（2026-02-21）** — 15 个图标已上传 Sealos `materials/` 目录，数据库已更新 |
| ~~P0~~ | ~~上线前数据清洗~~ | ~~无~~ | **已完成（2026-02-21）** — 迁移 `20260221100306`：exchange_items 89条清空、item_templates 3条垃圾删除、category_defs 3条脏数据删除 |
| P0 | 为现有商品和奖品上传真实图片 | 数据清洗完成 | **待运营操作** — exchange_items 已清空待重建；lottery_prizes 4条 physical 类型需上传图片 |
| ~~P1~~ | ~~商品上架强制图片校验逻辑~~ | ~~决策 3~~ | **已完成（2026-02-21）** — 路由层前置校验 + AdminService 状态变更校验 + PrizePoolService 补货激活校验 |
| ~~P1~~ | ~~多图支持（sort_order 字段 + API + 前端 UI）~~ | ~~十、多图~~ | **全部完成（2026-02-21）** — 后端 sort_order API + 前端多图上传/排序/删除 UI 均已实现 |
| ~~P1~~ | ~~图片删除保护（引用检查）~~ | ~~无~~ | **已完成（2026-02-21）** — ImageService.deleteImage() 删除前检查 ExchangeItem/LotteryPrize/ItemTemplate 引用 |
| ~~P2~~ | ~~稀有度 CSS 光效实现~~ | ~~无~~ | **已完成（2026-02-21）** — `admin/src/styles/components/rarity.css` 共享样式，含 5 级光效动画（pulse/glow/shine） |
| ~~P2~~ | ~~孤儿图片定时清理~~ | ~~无~~ | **已完成** — 每小时:30 自动清理 |
| ~~P2~~ | ~~category_defs 分类图标~~ | ~~决策 9~~ | **已完成（2026-02-21）** — 9 个暗黑奢华风图标已生成、上传 Sealos `categories/`、更新数据库、前端映射表已创建 |
| ~~P2~~ | ~~图片存储一致性 HEAD 检测定时任务~~ | ~~无~~ | **已完成（2026-02-21）** — `jobs/daily-image-storage-consistency-check.js` + 定时任务34（每天凌晨5点，分布式锁） |
| P3 | 加入 CDN | 决策 4 | 待实现（1 小时） |
...
| # | 问题 | 现状 | 影响 | 对应执行步骤 |
|---|---|---|---|---|
| B1 | ~~image_resources 表缺少 `sort_order` 字段~~ | ~~表中 15 个字段不含 sort_order~~ | ~~多图排序无法实现~~ | **已完成（2026-02-21）** — 迁移 `20260221100306` 新增字段，模型/路由/服务已同步 |
| B2 | ~~商品上架无图片校验逻辑~~ | ~~无图商品可直接上架~~ | ~~无图商品面向用户~~ | **已完成（2026-02-21）** — 路由前置校验 + AdminService + PrizePoolService |
| B3 | ~~material_asset_types.icon_url 指向不存在的路径~~ | ~~15 条 `test-seeds/icons/`~~ | ~~前端显示缺图~~ | **已完成（2026-02-21）** — 15 个图标已上传 Sealos `materials/` 目录 |
| B4 | ~~category_defs 图标不完整~~ | ~~9 条 NULL~~ | ~~分类图标缺失~~ | **已完成（2026-02-21）** — 9 个暗黑奢华风图标已生成并上传 Sealos `categories/` 目录 |
| B5 | popup_banners / carousel_items 独立管理 | 各自 `image_url` 字符串直存 | **已拍板保持现状** | 步骤 5（已决策，无需操作） |
| B6 | ~~item_templates 使用遗留字段~~ | ~~字符串直存~~ | ~~与统一图片管理不一致~~ | **已完成（2026-02-21）** — 迁移新增 `image_resource_id` FK，模型已更新关联 |
| B7 | ~~category_defs 存在脏数据~~ | ~~chen112/auto/beauty~~ | ~~生产数据不干净~~ | **已完成（2026-02-21）** — 迁移删除 3 条脏数据 |
| B8 | ~~图片删除保护未实现~~ | ~~FK ON DELETE SET NULL~~ | ~~可能删除使用中图片~~ | **已完成（2026-02-21）** — ImageService.deleteImage() 删除前引用检查 |
| B10 | ~~exchange_items 全量测试数据~~ | ~~89 条测试商品~~ | ~~用户看到测试商品~~ | **已完成（2026-02-21）** — 迁移清空 89 条 + exchange_records 5 条 |
| B11 | ~~item_templates 存在垃圾数据~~ | ~~id=18/19/20~~ | ~~生产数据不干净~~ | **已完成（2026-02-21）** — 迁移删除 3 条垃圾数据 |
| B9 | ~~孤儿图片清理~~ | ~~已调度~~ | ~~已自动清理~~ | **已完成** — 每小时:30 自动清理 |

### 12.2 Web 管理台前端项目的问题

| # | 问题 | 现状 | 影响 | 对应执行步骤 |
|---|---|---|---|---|
| W1 | ~~材料资产图标映射表未创建~~ | ~~无 `_mapping.js`~~ | ~~前端无法映射~~ | **已完成（2026-02-21）** — `admin/public/assets/icons/materials/_mapping.js` 已创建 |
| W1b | ~~分类图标映射表未创建~~ | ~~无分类图标~~ | ~~分类无图标展示~~ | **已完成（2026-02-21）** — 9 个图标 + `admin/public/assets/icons/categories/_mapping.js` 已创建 |
| W2 | ~~图片上传组件分散，未统一封装~~ | ~~各自实现上传逻辑~~ | ~~代码重复~~ | **已完成（2026-02-21）** — `admin/src/alpine/mixins/image-upload.js` 统一 mixin + `createPageMixin({ imageUpload: true })` |
| W3 | ~~商品编辑页无多图上传 UI~~ | ~~当前只支持单张主图~~ | ~~无法添加详情图~~ | **已完成（2026-02-21）** — `exchange-market.html` 详情图区域 + `exchange-items.js` 多图方法 |
| W4 | banner 上传走独立流程 | banners.js / carousel-items.js 各自 fetch 上传 | **已拍板保持现状** | 与 B5 联动（已决策） |
| W5 | ~~稀有度 CSS 光效未实现~~ | ~~稀有度无 CSS 效果~~ | ~~前端展示不完整~~ | **已完成（2026-02-21）** — `rarity.css` 共享样式文件，5 级光效（glow/pulse/shine） |
...
### ~~步骤 1：image_resources 表新增 sort_order 字段~~ [B] [已完成 2026-02-21]

**已完成操作**：迁移 `20260221100306-add-image-management-enhancements.js`
- 新增 `sort_order INT DEFAULT 0`
- `models/ImageResources.js` 字段定义已同步
- `ImageService` 上传/查询/更新方法已支持 sort_order
- `routes/v4/console/images.js` 新增 `PATCH /:id` 排序更新端点

### ~~步骤 2：商品上架强制图片校验~~ [B] [已完成 2026-02-21]

**已完成操作**：
1. `routes/v4/console/marketplace.js` 路由层前置校验（创建时 status=active 必须有图片）
2. `services/exchange/AdminService.js` 更新时 status → active 校验 + 图片记录存在性验证
3. `services/PrizePoolService.js` 奖品更新/补货激活时 physical 类型校验
4. 返回 `{ code: 'IMAGE_REQUIRED' }` 标准错误格式

### ~~步骤 3：材料资产图标上传 Sealos + 更新 icon_url~~ [B] [已完成 2026-02-21]

**已完成操作**：
1. 15 个 PNG 通过 `SealosStorageService.uploadImage()` 上传到 Sealos `materials/` 目录
2. `material_asset_types.icon_url` 已全部更新为真实对象 key
3. 验证通过：15/15 条记录 icon_url 指向有效 Sealos URL

### ~~步骤 4：category_defs 图标补充~~ [B] [已完成 2026-02-21]

**责任方**：后端 + Web 前端设计

**已完成操作**：
1. 为 9 个正式分类生成暗黑奢华风图标（256×256 PNG，~363KB 总计）
2. 前端静态资源：`admin/public/assets/icons/categories/`（9 个 PNG + `_mapping.js` 映射表）
3. 上传到 Sealos `categories/` 目录，更新 `category_defs.icon_url` 为真实对象 key
4. 前端映射：`CATEGORY_ICONS` 对象 + `getCategoryIconUrl()` 函数
...
### ~~步骤 6：item_templates 遗留字段处理~~ [B] [已完成 2026-02-21]

**已完成操作**：迁移 `20260221100306`
1. 新增 `image_resource_id INT` FK → `image_resources.image_resource_id`（ON DELETE SET NULL）
2. `models/ItemTemplate.js` 添加 belongsTo 关联 + 遗留字段标注废弃
3. 测试数据 `image_url='test/image.png'` 已清理为 NULL

### ~~步骤 7：上线前数据清洗~~ [B] [已完成 2026-02-21]

**已完成操作**：迁移 `20260221100306`（单事务执行）
1. category_defs：删除 3 条脏数据（chen112/auto/beauty），保留 9 个干净分类
2. item_templates：删除 3 条垃圾数据（id=18/19/20）
3. exchange_items：清空 89 条测试数据 + exchange_records 5 条 + 自增 ID 重置
4. lottery_prizes：保留 15 条真实奖品数据（4 条 physical 类型待运营上传图片）
5. Seeder 文件已同步清理 `test-seeds/icons/` 无效路径

### ~~步骤 8：图片删除引用保护~~ [B] [已完成 2026-02-21]

**已完成操作**：
1. `ImageService.deleteImage()` 删除前检查 `ExchangeItem` / `LotteryPrize` / `ItemTemplate` 引用
2. 有引用则抛出 `'图片正在被 N 个商品/奖品/模板 使用，无法删除。请先解除关联后再删除。'`
...
### ~~步骤 10：Web 管理台图标映射表~~ [W] [已完成 2026-02-21]

**责任方**：Web 管理台前端

**已完成操作**：
1. 材料图标映射：`admin/public/assets/icons/materials/_mapping.js` — `MATERIAL_ICONS` + `getMaterialIconUrl()`
2. 分类图标映射：`admin/public/assets/icons/categories/_mapping.js` — `CATEGORY_ICONS` + `getCategoryIconUrl()`
3. 前端使用方式：`import { getMaterialIconUrl } from '@/assets/icons/materials/_mapping.js'`

### ~~步骤 11：统一图片上传组件封装~~ [W] [已完成 2026-02-21]
...
### ~~步骤 12：商品编辑多图上传 UI~~ [W] [已完成 2026-02-21]
...
### ~~步骤 13：稀有度 CSS 光效~~ [W] [已完成 2026-02-21]
