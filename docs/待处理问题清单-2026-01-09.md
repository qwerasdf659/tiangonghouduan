...
## 已拍板决策汇总（2026-01-09 最终确认）

> 说明：所有架构/策略选择已完成最终确认，按优先级执行落地。

### ✅ D1（P2-健康检查契约）- 已确认为方案A（当前实施状态）

**决策**：不改接口，保持当前 `/health` 响应结构（`data.systems.database/redis`）

**理由**：

- 当前代码（`app.js:316-319`）已按此结构实现，无需改动
- 现有监控脚本（`scripts/verify-p0-fixes.js:75-76`、`scripts/monitoring/lightweight-monitor.js:77-80`）已按此解析
- 契约简洁稳定，避免冗余兼容字段长期共存
...
### ✅ D2（P1/P2-市场监控阈值与告警策略）- 已确认全走 DB + 默认值

**决策**：监控阈值/告警渠道/自动修复开关**全部以 DB `system_settings` 为真相源**

**配置清单**（`category='marketplace'`，所有 key 统一前缀 `monitor_`）：

| setting_key                        | value_type | 默认值          | 说明                                            | 业务风险级别 |
| ---------------------------------- | ---------- | --------------- | ----------------------------------------------- | ------------ |
| `monitor_threshold_days`           | number     | `2`             | 冻结异常检查阈值（天）                          | 中           |
| `monitor_long_listing_days`        | number     | `7`             | 长期挂牌告警阈值（天）                          | 低           |
| `monitor_price_low_ratio`          | number     | `0.1`           | 价格异常下限（市场均价的比例，0.1=10%）         | 中           |
| `monitor_price_high_ratio`         | number     | `3.0`           | 价格异常上限（市场均价的倍数，3.0=300%）        | 中           |
| `monitor_alert_channels`           | string     | `admin_message` | 告警渠道（逗号分隔：admin_message,webhook,sms） | 低           |
| `monitor_frozen_mismatch_auto_fix` | boolean    | `false`         | **冻结异常是否自动修复（高风险项）**            | **高**       |

**高风险项说明（frozen_mismatch_auto_fix）**：

- **默认关闭**（`false`）：监控发现冻结异常时**只告警不自动修复**，人工介入后处理
- **理由**：自动解冻/冻结涉及资产安全，误修复=资金风险；需监控规则跑稳、误报率可控后再灰度开启
- **后续灰度开启条件**：
  1. 监控运行 ≥ 1 个月，告警准确率 > 95%
  2. 建立误报复盘机制和快速回滚能力
  3. 优先在测试环境/小流量验证自动修复逻辑

**当前现状对齐**：

- ✅ DB 已有：`max_active_listings=10`（上架上限）、`listing_expiry_days=3`（过期天数）
- ❌ DB 缺失：监控阈值类配置（需新增 migration 创建）
- ❌ 代码硬编码：
  - `jobs/hourly-expire-fungible-asset-listings.js:42`：`LISTING_EXPIRY_DAYS = 3`（需改读 DB）
  - `jobs/hourly-market-listing-monitor.js:32-41`：`MONITOR_CONFIG` 硬编码（需改读 DB）
  - `routes/v4/market/sell.js:148`：`onSaleCount >= 10` 硬编码（需改调 `MarketListingService.getListingConfig`）

**实施步骤**：

1. 创建 migration 添加监控配置（`monitor_*` 系列 key）
2. 修改 `jobs/hourly-market-listing-monitor.js`：从 DB 读取 `MONITOR_CONFIG`（带缓存，TTL=60s）
3. 修改 `jobs/hourly-expire-fungible-asset-listings.js`：从 `MarketListingService.getListingConfig('listing_expiry_days')` 读取
4. 修改 `routes/v4/market/sell.js`：上架上限从 `MarketListingService.getListingConfig('max_active_listings')` 读取
5. 添加白名单校验（`config/system-settings-whitelist.js`）：确保 `monitor_frozen_mismatch_auto_fix` 默认 `false`
6. 测试验证：修改配置后 jobs/routes 立即生效（缓存 TTL 内）

---

### ✅ D3（P1-历史幂等 api_path 旧值残留）- 已确认方案A（删旧记录 + 统计走 canonical）

**决策**：删除旧路径记录 + 统计口径长期走 `canonical_operation`

**核心原则**：

- **不做历史迁移写回**（零数据改写风险）
- **旧路径记录在"功能验证通过后"统一删除**（按三步走策略执行）
- **统计口径长期走 canonical_operation**（避免再被 URL 绑架）

**已拍板"强制不兼容 + 删旧路径"前提下的执行策略**：

1. **代码删除（三步走第一步）**：
   - 删除旧路径代码（如 `/api/v4/exchange_market/exchange`）
   - 保留新路径（如 `/api/v4/shop/exchange/exchange`）
   - 确保所有引用/测试/文档都切换到新路径

2. **功能验证（三步走第二步）**：
   - 测试新路径功能完全正常（幂等/事务/异常回滚等）
   - 监控线上流量：确认旧路径流量已清零（nginx/access log）
   - 验证幂等表：旧路径未过期记录已自然过期（TTL=7天）或手动检查
...
- 旧路径 `/api/v4/exchange_market/exchange`：**414** 条（全部未过期）
- 新路径 `/api/v4/shop/exchange/exchange`：**54** 条（全部未过期）
- 执行时机：在 P0-1 canonical 化落地后、旧路径代码删除前统一清理

**统计口径迁移**：

- 现有统计脚本（如果有）需改为按 `canonical_operation` 聚合（通过 `IdempotencyService.getCanonicalOperation()` 反查）
- 示例：

  ```javascript
  // 旧统计逻辑（按 api_path）
  SELECT api_path, COUNT(*) FROM api_idempotency_requests GROUP BY api_path;

  // 新统计逻辑（按 canonical_operation，需在应用层聚合）
  const rows = await sequelize.query('SELECT api_path, COUNT(*) as cnt FROM api_idempotency_requests GROUP BY api_path');
  const grouped = rows.reduce((acc, row) => {
    const canonical = IdempotencyService.getCanonicalOperation(row.api_path);
    acc[canonical] = (acc[canonical] || 0) + row.cnt;
    return acc;
  }, {});
  ```
...
### ✅ D4（P2-路由层规范治理的强度）- 已确认强制拦截（Git Hooks本地门禁 + 存量治理）

**决策**：**Git Hooks（本地门禁）强制拦截违规**，按"增量门禁 + 存量治理计划"落地

**核心原则**：

- **增量门禁**：新增/修改路由若违规**直接 git commit 失败**（保证不新增技术债）
- **存量治理**：制定分阶段清理计划，逐步消化现有违规（不影响当前迭代）
- **本地验证**：使用 Git Hooks 在提交/推送前强制检查（无需CI/CD平台）

**路由层规范清单**（禁止项）：

1. ❌ 路由文件直接 `require('../models')` 或 `require('../models/User')` 等
2. ❌ 路由文件内调用 `sequelize.transaction()` 或 `await transaction = ...`
3. ❌ 路由文件内直接 `require` Service 但未通过 `TransactionManager` 包裹写操作
4. ❌ 路由文件直接操作 Model 的增删改（如 `User.create()`、`User.update()`）

**门禁实施步骤**（Git Hooks 本地门禁方案）：

1. **新增门禁脚本**（`scripts/validation/check-route-compliance.js`）：
   ...
   function checkRouteFile(filePath) {
     ...
     // 规则1：禁止直接 require models
     if (content.match(/require\(['"]\.\.\/.*models['"]\)/)) {
       violations.push({
         file: relativePath,
         rule: 'DIRECT_MODEL_REQUIRE',
         message: '路由层禁止直接 require models（应通过 Service 层）'
       })
     }

     // 规则2：禁止路由层开事务
     if (content.match(/sequelize\.transaction\(|await\s+.*transaction\s*=/)) {
       violations.push({
         file: relativePath,
         rule: 'DIRECT_TRANSACTION',
         message: '路由层禁止直接开事务（应通过 TransactionManager）'
       })
     }

     // 规则3：禁止直接调用 Model 写方法
     const writeMethodPattern =
       /\b(User|LotteryDraw|MarketListing|ItemInstance|Account)\.(create|update|destroy|bulkCreate)\(/g
     if (content.match(writeMethodPattern)) {
       violations.push({
         file: relativePath,
         rule: 'DIRECT_MODEL_WRITE',
         message: '路由层禁止直接调用 Model 写方法（应通过 Service 层）'
       })
     }
   }
   ...
   if (violations.length > 0) {
     console.error('❌ 路由层规范检查失败:')
     violations.forEach(v => {
       console.error(`   ${v.file}: [${v.rule}] ${v.message}`)
     })
     process.exit(1) // Git Hooks 强制拦截
   } else {
     console.log('✅ 路由层规范检查通过')
   }
   ```

2. **安装 Git Hooks 管理工具**（使用 husky）：
...
3. **配置 pre-commit Hook**（提交前检查）：

   ```bash
   # 创建 pre-commit hook
   npx husky add .husky/pre-commit "npm run validate:route-compliance"
   chmod +x .husky/pre-commit
   ```

4. **配置 pre-push Hook**（推送前二次检查，可选）：

   ```bash
   # 创建 pre-push hook（更严格）
   npx husky add .husky/pre-push "npm run lint && npm run validate:route-compliance"
...
5. **更新 package.json 脚本**：
   ...
   {
     "scripts": {
       "validate:route-compliance": "node scripts/validation/check-route-compliance.js",
...
6. **存量治理计划**（分阶段清理）：
   - **Phase 1（立即）**：扫描全量违规文件，生成清单（`scripts/validation/route-compliance-report.js`）
   - **Phase 2（1周内）**：针对高频违规路由（如市场/兑换）优先重构，收口到 Service
   - **Phase 3（2周内）**：清理中低频违规路由
   - **Phase 4（1个月内）**：移除门禁豁免白名单（如果有），实现 100% 合规
...
7. **白名单机制（过渡期）**：
   ...
   const LEGACY_WHITELIST = [
     'routes/v4/legacy-api/old-feature.js', // 待重构
     'routes/v4/admin/legacy-batch.js' // 待下线
   ]

   function isWhitelisted(filePath) {
     return LEGACY_WHITELIST.some(w => filePath.includes(w))
   }
   ```

**Git Hooks 优势**：

- ✅ **本地拦截**：`git commit` 前就失败，无需等 CI/CD 反馈（秒级检查）
- ✅ **零依赖**：无需 CI/CD 平台（GitHub Actions/GitLab CI），纯本地环境
- ✅ **实时反馈**：开发者立即看到违规，快速修复（缩短反馈周期）
- ✅ **团队统一**：通过 husky 配置共享，所有开发者自动启用
- ⚠️ **可绕过**：开发者可用 `git commit --no-verify` 跳过（需团队纪律约束）

**绕过机制控制**：

- 团队约定：禁止在非紧急情况下使用 `--no-verify`
- 代码审查：PR/MR 时人工检查是否符合规范
- 后续增强：服务端 Git Hooks（如 GitLab/GitHub Push Rules）二次拦截

**当前项目现状**：

- ✅ 已有 `npm run validate:routes`（检查路由文件存在性）
- ❌ 未有路由层规范门禁（需新增）
- ❌ 存量违规未统计（需扫描）
- ❌ 未配置 Git Hooks（需安装 husky）

**执行优先级**：P1（与 P0-1/P0-2 并行推进，但门禁可在存量清理完成后再强制开启）
...
const fs = require('fs');
const path = require('path');
fs.mkdirSync('scripts/validation', { recursive: true });
// ... （检查脚本内容见上方）
"
...
# 4. 测试 Hook（故意违规）
echo "const User = require('../models').User" >> routes/v4/test.js
git add routes/v4/test.js
git commit -m "test hook"
# 预期：❌ 路由层规范检查失败: [DIRECT_MODEL_REQUIRE] ...

# 5. 修复后再提交
rm routes/v4/test.js
git commit -m "test hook"
# 预期：✅ 路由层规范检查通过
```
...
### ✅ D5（P2-清理旧版本/兼容层代码的范围与时间点）- 已确认立即清理

**决策**：✅ **已拍板确认**（2026-01-09）

- ✅ **执行时间**：马上清理
- ✅ **当前仓库现状对齐**：
  - `routes/` 下仅有 `v4/`（未发现 `routes/v1|v2|v3`）
  - 未发现 `req.apiVersion` 等"按版本分支"的兼容代码（相关内容仅在本文档示例中出现）
- ✅ **已执行的清理**：
  - 已删除：`services/ThumbnailService.js`（文件已标注废弃且在代码中无引用）
- ✅ **保留不删除**：
  - `backups/` 目录（保留用于取证/回溯，不影响运行）
...
4. **创建数据清理脚本（已拍板：不备份，直接删除）**：
   ...
   async function deleteDeprecatedPathRecords() {
     ...
     console.log('⚠️ 已拍板决策（2026-01-09）：不备份，直接删除（不可逆）')
     console.log('⚠️ 前置条件：功能验证已完成且正常')
