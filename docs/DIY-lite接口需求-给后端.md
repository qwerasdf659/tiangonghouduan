...
3. 可选增强：素材列表按 `material_name` 批量填充五行/文案的小工具（降低运营录入成本，拍板②如选"按珠子录入"时推荐做）。
...
| # | 事项 | 选项 | 推荐 |
| --- | --- | --- | --- |
| ① | 手围模型 | A. 前端放弃手围，改用后端现行 S/M/L `size_options`；B. 后端扩展 `sizing_rules` JSON 增加手围/圈数/容量公式（4.3，零迁移） | **B**：保留 diy-lite 核心体验，且 JSON 扩展成本极低、schema 仍由后端定义 |
| ② | 新字段录入维度 | A. 逐颗珠子录入；B. 按 `material_name` 材质维度录入、服务端合并 | **A**（列在珠子行上，模型最简单、无合并逻辑技术债）+ 管理后台做"按材质批量填充"工具压运营成本 |
| ③ | 分享还原 | A. 新增脱敏只读接口 `/works/:id/shared`；B. 放开 `GET /works/:id` 非作者可读 | **A**：不破坏现有权限模型，脱敏可控 |
| ④ | 库存暴露口径 | A. 用户端掩码为 -1/0/1 三态；B. 下发精确库存 | **A**：符合数据最小化，改动只在序列化层 |
| ⑤ | `shape` 枚举是否扩展 `tube/oblate` | 是 / 否（仅靠 `bore_orientation`+尺寸表达） | **是**：一次迁移内顺带完成，管理后台下拉同步加两项 |
| ⑥ | `energy/pairing/五行` 是否上线必填 | 必填 / 可空（前端已做空态） | **可空**：不阻塞上线，运营分批补录 |
