...
> 最后更新：2026-02-23（二次审计验证 — P0资产守恒SUM=0全部通过(7/7)，P0核销扫码页已建已构建，P1多商家架构已建，余额一致性0差异）
...
| 现有页面 | 45个HTML页面（含 `redemption-management.html` 核销管理页、`store-management.html` 门店管理页、`dict-management.html` 字典管理页，均已存在） |
| 已有核销API封装 | `admin/src/api/redemption.js`（LIST/STATISTICS/EXPORT/BATCH_REDEEM/BATCH_CANCEL/BATCH_EXPIRE 六个端点已封装） |
...
| 管理后台核销管理 | `admin/redemption-management.html` | ✅ 已存在 | 订单列表 + 批量核销/取消/过期操作 |
...
| 现状 | 说明 |
|---|---|
| `redemption-management.html` 已存在 | 管理员视角的核销订单管理（列表/批量操作）已完成 |
| 缺少商家店员扫码核销页面 | 需要新增一个面向店员的扫码核销页（调用 `POST /api/v4/shop/redemption/scan`） |
| `admin/src/api/redemption.js` 已存在 | 后端核销 API 封装已完成 |
...
#### 2.3.2 Web管理后台（商家店员扫码核销）— 需 admin 前端实现
...
| 文件 | 功能 | 调用的后端接口（已存在） |
|---|---|---|
| `admin/redemption-scan.html`（新建） | 店员输入文本码 或 调用浏览器摄像头扫QR码 → 展示物品信息 → 确认核销 | `POST /api/v4/shop/redemption/scan`（QR码）或 `POST /api/v4/shop/redemption/fulfill`（文本码） |

技术实现：
- Alpine.js `x-data` 组件 + Tailwind CSS 布局（复用现有 admin 技术栈）
- QR扫码可用浏览器 `navigator.mediaDevices.getUserMedia` + JS QR解码库
- 核销前先展示物品信息（从扫码解析出 `redemption_order_id`，调用查询接口展示详情）
- 核销后展示成功状态和物品详情
...
| 步骤 | 内容 | 责任端 | 工作量 |
|---|---|---|---|
| 1 | 管理后台新增 `admin/redemption-scan.html` 扫码核销页面 | Web管理后台前端 | 1 天 |
