...
**文档版本**: v1.3（项目未上线一步到位版 - 2026年1月12日拍板修订）  
**创建时间**: 2025年1月11日  
**最后修订**: 2026年1月12日  
...
1. **二维码**：仅实现 v2 动态码（exp+nonce+签名），**v1 永久码相关字段/逻辑全部删除**（非仅"拒绝"，而是代码库/数据库彻底清理，2026年1月12日拍板）
...
- **门店数据来源**：✅ **后台录入，平台管理员通过 `/api/v4/console/stores` CRUD 维护**（2026年1月12日拍板）
...
### 域边界与角色等级决策（2026年1月12日拍板）

- **域边界（P0 强制）**：✅ **`/api/v4/shop/*` 商家域接口只给商家员工/店长**（必须在 `store_staff` 在职），**平台内部角色（`admin/ops/regional_manager/business_manager/sales_staff`）走 `/api/v4/console/*`**
- **角色等级以真实库为准**：✅ **`merchant_staff=20`、`merchant_manager=40`**（文档原写 50/55 已过期，权限主要靠 `permissions` 判断，避免 `role_level` 误伤其他逻辑）
- **v1 永久码处理**：✅ **按"未上线"口径，清空 v1 测试数据并删除所有 v1 相关字段/逻辑，只保留 v2**（非仅"拒绝 v1"，而是彻底移除 `qr_code_version/is_legacy_v1` 字段）
- **商家侧消费记录查询**：✅ **店员只看自己录入（`merchant_id=self`），店长看本店全部 + 管员工（`store_id=当前门店`）**
- **审计日志治理（P0 必选）**：✅ **上线前必须具备 180 天保留定时清理 + CSV 导出接口**
- **消费审核归属**：✅ **审核接口仅 `admin`（`role_level>=100`）可用，走 `/api/v4/console/*` 域**（不开放给 ops/regional_manager）
...
- [x] **AC1.3**: 新增角色 `merchant_staff`（**role_level=20，以真实库为准**），权限包含 `consumption:scan_user` 和 `consumption:create`；新增 `merchant_manager`（**role_level=40**），权限额外包含 `consumption:read_store`、`staff:read`
- [x] **AC1.4**: ✅ **域边界强制隔离（2026年1月12日完成实施）**：
  - **商家域 `/api/v4/shop/*`**：仅限 `merchant_staff/merchant_manager` 且 `store_staff` 在职链路可用
  - **平台管理员（admin/ops/regional_manager）走 `/api/v4/console/*`**，不直接调用商家域接口
  - ✅ **已实现** `requireMerchantDomainAccess` 中间件（`middleware/auth.js`），并在 `routes/v4/shop/index.js` 应用
...
- [x] **AC2.4**: ✅ **商家侧消费记录查询接口（2026年1月12日拍板）**：
  - **接口路径**：`GET /api/v4/shop/consumption/list`（商家域，需 `store_staff` 在职）
  - **店员（`role_in_store='staff'`）**：仅查自己录入（`WHERE merchant_id = self AND store_id IN (员工active门店)`）
  - **店长（`role_in_store='manager'`）**：查本店全部（`WHERE store_id = 当前门店`），可看非自己录入的记录
  - 必须接入 `requireMerchantPermission('consumption:read', { scope: 'store' })`
- [ ] **AC2.5**: ✅ **跨门店查询走 console 域（2026年1月12日拍板）**：区域经理/平台管理员不使用商家域 `/api/v4/shop/*`，走 `/api/v4/console/consumption/*` 跨门店查询
- [ ] **AC2.6**: 店长（`role_in_store='manager'`）可查看/管理本门店所有员工的记录 + 员工管理（查看/禁用本店员工）；普通员工（`role_in_store='staff'`）仅可查看自己录入的记录
...
- [x] **AC4.4**: ✅ **审计日志治理（P0 必选，2026年1月12日拍板）**：
  - **180 天保留定时清理**：增加定时任务，删除 `merchant_operation_logs.created_at` 超过 180 天的数据（利用 `idx_created_at` 索引）
  - **CSV 导出接口**：`GET /api/v4/shop/audit/export`（门店隔离 + 分页/时间范围限制 + 流式输出），需 `requireMerchantPermission('audit:export', { scope: 'store' })`
  - **上线前必须具备**这两项能力（不可推迟到 P2）
...
- [x] **AC6.4**: ✅ **v1 数据与逻辑全量清除（2026年1月12日拍板）**：
  - **数据清理**：清空 `consumption_records` 里 v1 测试数据（真实库当前有 9 条 v1 重复提交记录），处理关联外键约束后执行
  - **数据库字段删除**：新增迁移删除 `consumption_records.qr_code_version`、`consumption_records.is_legacy_v1` 字段及相关索引
  - **非 v2 格式直接返回** 400 `INVALID_QRCODE_FORMAT`（无需判断是否为 v1，任何非 `QRV2_` 前缀都拒绝）
- [x] **AC6.5**: ✅ **删除所有 v1 相关代码逻辑**：
  - `utils/QRCodeValidator.js`：删除 `v1_prefix`、`isV1QRCode()` 等分支，仅保留 v2 方法
  - `routes/v4/shop/consumption/*.js`：删除 v1 专门判断分支
  - `models/ConsumptionRecord.js`：删除 `qr_code_version`、`is_legacy_v1` 字段定义
  - `services/ConsumptionService.js`：删除对上述字段的读写
...
## 二、历史现状分析（2026年1月11日升级前快照）

> ⚠️ **注意**：本节描述的是 **升级实施前** 的历史状态。当前代码库已完成主要升级，以下问题大部分已解决。
>
> **验证时间**: 2026年1月11日真实数据库直连只读查询 + 代码静态分析（升级前）

### 2.1 升级前数据库现状（历史快照）

| 维度                 | 升级前查询结果                         | 升级后状态（2026年1月12日）                                    |
| -------------------- | -------------------------------------- | -------------------------------------------------------------- |
| **门店数据**         | `stores` 表 0 行                       | ✅ 已有数据，需通过 `/api/v4/console/stores` CRUD 维护         |
| **消费记录门店字段** | `consumption_records.store_id` 不存在  | ✅ **已添加**，NOT NULL，外键关联 `stores.store_id`            |
| **员工门店关系表**   | `store_staff` 表不存在                 | ✅ **已创建**，支持一人多店、sequence_no、触发器保证唯一active |
| **审计日志表**       | `merchant_operation_logs` 不存在       | ✅ **已创建**，独立商家域审计日志                              |
| **风控告警表**       | `risk_alerts` 不存在                   | ✅ **已创建**，支持频次/金额/关联告警                          |
| **二维码重放证据**   | v1 永久码被重放 9 次                   | ✅ **已修复**：v2 动态码 + nonce 防重放；v1 数据待清理         |
| **消费记录提交者**   | 主要是 admin 用户                      | ✅ **已增加** `merchant_staff/merchant_manager` 角色           |
| **商家角色**         | 无 `merchant_staff`                    | ✅ **已添加**：`merchant_staff=20`、`merchant_manager=40`      |
| **v1 字段**          | `qr_code_version`、`is_legacy_v1` 存在 | 🔴 **待删除**（2026年1月12日拍板：彻底移除）                   |
...
- [ ] **TS2.2**: ✅ **写操作统一事务边界（P0 待补齐，2026年1月12日拍板）**：
  - **已符合**：`routes/v4/shop/consumption/submit.js`、`routes/v4/shop/staff/index.js` 等已使用 `TransactionManager.execute()`
  - **待补齐**：`services/MerchantRiskControlService.updateAlertStatus()`、`services/ConsumptionService.softDeleteRecord()`/`restoreRecord()` 需纳入 `TransactionManager.execute()` 或显式传递 `transaction` 参数
...
- [ ] **TS2.5**: ✅ **统一响应格式强制执行（P0 待补齐，2026年1月12日拍板）**：
  - **已符合**：业务路由层已使用 `res.apiSuccess()`/`res.apiError()`
  - **待补齐**：`middleware/auth.js` 中 `requireAdmin`/`requirePermission`/`requireMerchantPermission`/`requireRole` 仍有 `res.status().json({success:false,error:...})` 分支，需统一为 `res.apiError()`/`res.apiUnauthorized()`/`res.apiForbidden()`，确保所有 `/api/*` 返回结构一致
...
```sql
-- 插入商家员工角色（role_level=20，以真实库为准，2026年1月12日拍板）
INSERT INTO `roles` (
  `role_name`,
  `role_level`,
  `permissions`,
  ...
  1,
  '商家员工角色：可扫码录入消费、查看自己录入的记录'
);

-- 插入商家店长角色（role_level=40，以真实库为准，2026年1月12日拍板）
INSERT INTO `roles` (
  `role_name`,
  `role_level`,
  `permissions`,
...
> - **域边界隔离**：商家域 `/api/v4/shop/*` 仅限商家员工/店长；平台内部角色走 `/api/v4/console/*`

| 角色                               | role_level | 使用域         | 扫码查用户 | 提交消费 | 查询消费记录        | 审核消费    | 员工管理    | 门店管理           | 数据范围                 |
| ---------------------------------- | ---------- | -------------- | ---------- | -------- | ------------------- | ----------- | ----------- | ------------------ | ------------------------ |
| **admin**                          | 100        | **console 域** | ✅ 可用    | ✅ 兜底  | ✅ 全部门店         | ✅ **唯一** | ✅ 全部     | ✅ 全部（含 CRUD） | **全局**                 |
| **ops**                            | 30         | **console 域** | ✅ 只读    | ❌ 不可  | ✅ 全部门店（只读） | ❌ 不可     | ✅ 只读     | ✅ 只读            | **全局（只读）**         |
| **regional_manager**               | 80         | **console 域** | ✅ 可用    | ❌ 不可  | ✅ 跨门店（区域内） | ❌ 不可     | ✅ 区域内   | ✅ 区域内          | **区域**                 |
| **merchant_manager**<br>（店长）   | **40**     | **shop 域**    | ✅ 可用    | ✅ 可用  | ✅ 本门店全部       | ❌ 不可     | ✅ 本店员工 | ✅ 本店信息        | **本门店**               |
| **merchant_staff**<br>（商家员工） | **20**     | **shop 域**    | ✅ 可用    | ✅ 可用  | ✅ 自己录入的       | ❌ 不可     | ❌ 不可     | ❌ 不可            | **本门店<br>（仅自己）** |

**关键说明（2026年1月12日拍板修订）**：

1. **域边界隔离（P0 强制）**：
   - ✅ **商家域 `/api/v4/shop/*`**：仅限 `merchant_staff`（role_level=20）/ `merchant_manager`（role_level=40），且必须在 `store_staff` 有在职记录
   - ✅ **平台域 `/api/v4/console/*`**：`admin/ops/regional_manager` 等平台内部角色使用，不走商家域
   - 在 `routes/v4/shop/index.js` 增加"商家域准入"中间件，拦截非商家角色

2. **提交消费（`/submit`）**：
   - ✅ **商家员工/店长日常录入**（shop 域）
   - ✅ **admin 兜底/应急**（console 域或 shop 域均可，但日常不走 shop）

3. **审核消费（`/review`）**：
   - ✅ **仅 admin 可审核**（role_level >= 100），走 `/api/v4/console/consumption/review/*`
   - ❌ **ops/regional_manager 不可审核**（真实库 ops=30 不满足 role_level>=100）

4. **商家侧消费记录查询**：
   - **店员**：仅看自己录入（`merchant_id = self`）
   - **店长**：看本店全部（`store_id = 当前门店`）+ 可管员工
   - 接口：`GET /api/v4/shop/consumption/list`

5. **role_level 以真实库为准**：
   - `merchant_staff=20`、`merchant_manager=40`（文档原写 50/55 已废弃）
   - `ops=30`（只读角色，不可审核/提交）
   - 权限主要靠 `permissions` JSON 判断，避免 `role_level` 误伤
...
### 附录A：API接口清单（2026年1月12日修订 - 域边界隔离版）

> **域边界原则**：商家域 `/api/v4/shop/*` 仅限商家员工/店长（需 `store_staff` 在职）；平台内部角色走 `/api/v4/console/*`

#### 商家域接口（`/api/v4/shop/*` - 商家员工/店长使用）

| 接口路径                             | 方法 | 权限要求                                             | 说明                         |
| ------------------------------------ | ---- | ---------------------------------------------------- | ---------------------------- |
| `/api/v4/shop/consumption/user-info` | GET  | `requireMerchantPermission('consumption:scan_user')` | 扫码获取用户信息             |
| `/api/v4/shop/consumption/submit`    | POST | `requireMerchantPermission('consumption:create')`    | 提交消费记录                 |
| `/api/v4/shop/consumption/list`      | GET  | `requireMerchantPermission('consumption:read')`      | **新增**：商家侧消费记录查询 |
| `/api/v4/shop/staff/list`            | GET  | `requireMerchantPermission('staff:read')`            | 查询本店员工列表（店长）     |
| `/api/v4/shop/staff/add`             | POST | `requireMerchantPermission('staff:manage')`          | 添加员工到本店（店长）       |
| `/api/v4/shop/staff/:id/disable`     | PUT  | `requireMerchantPermission('staff:manage')`          | 禁用本店员工（店长）         |
| `/api/v4/shop/audit/logs`            | GET  | `requireMerchantPermission('audit:read')`            | 查询本店操作日志             |
| `/api/v4/shop/audit/export`          | GET  | `requireMerchantPermission('audit:export')`          | **新增**：导出本店日志 CSV   |
| `/api/v4/shop/risk/alerts`           | GET  | `requireMerchantPermission('risk:read')`             | 查询本店风控告警             |

#### 平台域接口（`/api/v4/console/*` - admin/ops/regional_manager 使用）

| 接口路径                                     | 方法 | 权限要求        | 说明                               |
| -------------------------------------------- | ---- | --------------- | ---------------------------------- |
| `/api/v4/console/stores`                     | GET  | `stores:read`   | **新增**：门店列表查询             |
| `/api/v4/console/stores`                     | POST | `stores:create` | **新增**：创建门店                 |
| `/api/v4/console/stores/:id`                 | PUT  | `stores:update` | **新增**：更新门店信息             |
| `/api/v4/console/stores/:id`                 | DEL  | `stores:delete` | **新增**：删除门店                 |
| `/api/v4/console/consumption/list`           | GET  | `consumption:*` | 跨门店消费记录查询                 |
| `/api/v4/console/consumption/review/pending` | GET  | `requireAdmin`  | **迁移**：待审核消费列表           |
| `/api/v4/console/consumption/review/:id`     | PUT  | `requireAdmin`  | **迁移**：审核消费记录（仅 admin） |
| `/api/v4/console/staff`                      | GET  | `staff:read`    | 全局员工列表查询                   |
| `/api/v4/console/staff/:id/transfer`         | POST | `staff:update`  | 员工调店                           |
| `/api/v4/console/staff/user/:id/disable`     | PUT  | `staff:update`  | 禁用员工（所有门店）               |
| `/api/v4/console/merchant-logs`              | GET  | `logs:read`     | 全局商家操作日志查询               |
| `/api/v4/console/merchant-logs/export`       | GET  | `logs:export`   | 全局日志导出 CSV                   |
| `/api/v4/console/risk-alerts`                | GET  | `risk:read`     | 全局风控告警查询                   |
| `/api/v4/console/risk-alerts/:id`            | PUT  | `risk:review`   | 复核/忽略告警                      |

### 附录B：数据库迁移脚本清单（2026年1月12日修订版）

> **修订说明**：基于 2026年1月12日拍板决策，新增 v1 数据/字段清理迁移、门店 CRUD 支持、审计日志清理任务

| 迁移文件                                     | 说明                                                      | 优先级 | 前置依赖        | 备注                                                                                    |
| -------------------------------------------- | --------------------------------------------------------- | ------ | --------------- | --------------------------------------------------------------------------------------- |
| `20260112_update_merchant_role_levels.js`    | **修正商家角色 role_level**（20/40 以真实库为准）         | **P0** | 无              | ✅ 2026年1月12日拍板：`merchant_staff=20`、`merchant_manager=40`                        |
| `20260112_cleanup_v1_qrcode_data.js`         | **清空 v1 测试数据**（真实库当前有 9 条 v1 重复记录）     | **P0** | 无              | ✅ 2026年1月12日拍板：按"未上线"口径，删除/重置 v1 数据                                 |
| `20260112_remove_v1_qrcode_fields.js`        | **删除 v1 相关字段**（`qr_code_version`、`is_legacy_v1`） | **P0** | v1 数据清理完成 | ✅ 2026年1月12日拍板：彻底移除字段及相关索引                                            |
| `20250111_insert_merchant_roles.js`          | 插入商家角色配置（含权限 JSON）                           | **P0** | 无              | `merchant_staff`/`merchant_manager`（role_level 已修正）                                |
| ~~`20250111_add_mobile_masking.js`~~         | ~~已废弃~~                                                | ~~--~~ | --              | ✅ 已拍板：不脱敏                                                                       |
| `20250111_upgrade_qrcode_v2_only.js`         | 二维码 v2 升级（仅支持 v2）                               | **P0** | 无              | ✅ 使用独立密钥 `CONSUMPTION_QR_SECRET`                                                 |
| `20250111_import_stores_data.js`             | 导入门店主数据                                            | **P1** | 无              | ✅ 维护方式：`/api/v4/console/stores` CRUD                                              |
| `20250111_create_store_staff.js`             | 创建门店员工多对多关系表                                  | **P1** | 门店数据存在    | ✅ 方案B：支持一人多店、sequence_no、触发器保证唯一active                               |
| `20250111_alter_consumption_add_store_id.js` | 消费记录增加门店字段（**NOT NULL**）                      | **P1** | 门店数据存在    | ✅ 项目未上线，直接 NOT NULL                                                            |
| `20250111_create_merchant_operation_logs.js` | 创建商家域审计日志表                                      | **P1** | 无              | ✅ 独立 `merchant_operation_logs` 表                                                    |
| `20260112_add_audit_log_cleanup_job.js`      | **新增审计日志定时清理任务**（180天保留）                 | **P1** | 审计日志表存在  | ✅ 2026年1月12日拍板：上线前必须具备                                                    |
| `20260112_init_store_staff_active.js`        | **上线前初始化 store_staff active 数据**                  | **P1** | 门店/员工存在   | ⚠️ 当前真实库 `store_staff` 全是 inactive，需初始化每店至少一个 `merchant_manager` 在职 |
| `20250111_create_risk_alerts.js`             | 创建风控告警表                                            | **P2** | 无              | 阈值：10次/60秒、5000/50000、3店/10分钟                                                 |

**关键修订点（2026年1月12日拍板决策）**：

1. **✅ v1 全量清除**：清空 v1 测试数据 + 删除 `qr_code_version`/`is_legacy_v1` 字段（非仅"拒绝"，而是彻底移除）
2. **✅ role_level 修正**：`merchant_staff=20`、`merchant_manager=40`（以真实库为准）
3. **✅ 域边界隔离**：商家域 `/api/v4/shop/*` 仅限商家员工/店长，平台内部角色走 `/api/v4/console/*`
4. **✅ 门店 CRUD**：新增 `/api/v4/console/stores` 接口，平台管理员维护
5. **✅ 审计日志治理**：180天保留定时清理 + CSV 导出（P0 必选，上线前必须具备）
6. **✅ 审核接口归属**：消费审核接口迁移到 `/api/v4/console/consumption/review/*`，仅 admin 可用
7. **✅ 商家侧查询**：新增 `/api/v4/shop/consumption/list`（店员看自己、店长看本店）
8. **✅ 上线前初始化**：`store_staff` 需初始化每店至少一个 `merchant_manager`（当前全是 inactive 会导致商家接口 403）
