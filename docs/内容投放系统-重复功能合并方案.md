...
| `status` 流程 | draft→pending_review→approved→active | draft→**直接 active** | draft→**直接 active** |
...
### 3.5 carousel_show_logs → ad_impression_logs（11 条数据，D2 定论：扩展现有表）

carousel_show_logs 有 11 条记录。D2 定论：扩展 ad_impression_logs 加 4 个 nullable 字段。
字段映射：`exposure_duration_ms` → `show_duration_ms`，`is_clicked` → `is_clicked`，`is_manual_swipe` 丢弃（使用频率极低）。

### 3.6 popup_show_logs → ad_impression_logs（36 条数据，D2 定论：扩展现有表）

popup_show_logs 有 36 条记录。D2 定论：扩展 ad_impression_logs 加 4 个 nullable 字段。
字段映射：`show_duration_ms` → `show_duration_ms`，`close_method` → `close_method`，`queue_position` → `queue_position`。
...
- 新增 `createOperationalCampaign()` 方法 — 简化创建流程，自动设置 billing_mode='free'、status='active'、review_status='approved'
...
- ~~新增 ad_impression_logs 扩展 4 个字段（如果 D2 选方案 A）~~
...
| B13 | 扩展 system ad-events 路由 — 统一前端内容获取 | 改 1 个文件 | 替代 popup-banners/carousel-items/announcements 3 个路由 |
| B14 | 删除 3 个 Service + 5 个 Model + 6 个 Route + 2 个 Test | 删 16 个文件 | 最后执行 |
| B15 | DROP 5 张废弃表 | 迁移脚本 | 确认数据迁移完成后 |
| B16 | ALTER `ad_impression_logs` 加 4 个 nullable 字段 | 1 条 migration | D2 定论：show_duration_ms, close_method, queue_position, is_clicked |
...
| 编号 | 变更项 | 工作量 | 说明 |
|---|---|---|---|
| M1 | 弹窗接口路径变更 | 改 1 个请求 | `/api/v4/system/popup-banners` → `/api/v4/system/ad-events?slot_type=popup&position=home` |
| M2 | 轮播接口路径变更 | 改 1 个请求 | `/api/v4/system/carousel-items` → `/api/v4/system/ad-events?slot_type=carousel&position=home` |
| M3 | 公告接口路径变更 | 改 1 个请求 | `/api/v4/system/announcements` → `/api/v4/system/ad-events?slot_type=announcement` |
| M4 | 响应字段名变更 — 直接使用后端新字段名 | 改 3 个页面 | `popup_banner_id` → `ad_campaign_id`，`banner_type` → `campaign_category`，`display_mode` 从 creative 取 |
| M5 | 展示日志上报接口变更 | 改 1 个请求 | 原 popup_show_log / carousel_show_log → 统一 ad_impression_log |
...
**定论：方案 A — 跳过审核，直接 active**

理由：
1. 行业共识：运营内容和系统公告由授权运营人员直发是主流做法（美团/阿里/字节/Google 均如此）
2. 只有银行/金融因合规要求才全审核，本项目无此需求
3. 项目未上线，运营团队极小，审核流程增加无意义的操作步骤
4. 安全保障：后端 `requireAdmin` 中间件已控制权限，只有管理员能操作
...
**定论：方案 A — 扩展 ad_impression_logs 表加 4 个 nullable 字段**

理由：
1. 美团、字节跳动、Google 均采用"统一表 + nullable 扩展列"方案，这是中小规模系统的业界标准
2. 腾讯的分表方案适合日均亿级数据量，本项目总共 47 条，不需要
3. 阿里的 JSON 方案虽灵活，但 MySQL JSON 字段查询性能和索引支持不如独立列
4. 47 条数据加 4 个 nullable 字段，冗余完全可忽略
5. 未来所有类型的展示日志统一一张表，运维最简单
...
-- 5. ad_impression_logs 加 4 个字段（D2 定论：方案 A）
ALTER TABLE ad_impression_logs ADD COLUMN show_duration_ms INT UNSIGNED NULL;
ALTER TABLE ad_impression_logs ADD COLUMN close_method VARCHAR(20) NULL;
ALTER TABLE ad_impression_logs ADD COLUMN queue_position TINYINT UNSIGNED NULL;
ALTER TABLE ad_impression_logs ADD COLUMN is_clicked TINYINT(1) NULL DEFAULT 0;
...
4. 读取 36 条 popup_show_logs → 插入 ad_impression_logs（附带 show_duration_ms, close_method, queue_position）
5. 读取 11 条 carousel_show_logs → 插入 ad_impression_logs（附带 show_duration_ms=exposure_duration_ms, is_clicked）
...
4. **修改 AdImpressionLog Model**：新增 4 个 nullable 字段（D2 定论：方案 A）
5. **扩展 AdCampaignService**：新增 `createOperationalCampaign()` + `createSystemCampaign()` 方法
6. **修改 AdBiddingService.selectWinners()**：三层优先级出队逻辑
7. **扩展 console ad-campaigns 路由**：支持 `?category=` 筛选
8. **新增/扩展 system ad-events 路由**：统一前端内容获取接口（替代 3 个独立路由）
...
3. 展示日志上报改为 ad_impression_log 接口
