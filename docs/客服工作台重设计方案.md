...
**Service 注册与访问方式**（对齐现有 ServiceManager 模式）：
```javascript
// services/index.js 中注册
serviceManager.register('cs_user_context', CustomerServiceUserContextService)
serviceManager.register('cs_issue', CustomerServiceIssueService)
serviceManager.register('cs_diagnose', CustomerServiceDiagnoseService)
serviceManager.register('cs_compensate', CustomerServiceCompensateService)
...
`CustomerServiceUserContextService`（聚合查询，全部只读）：
- `getSummary(userId)` → 调用各 Service 的 count 方法聚合
- `getAssets(userId)` → 调用 BalanceService 查余额 + AssetTransactionService 查流水
- `getBackpack(userId)` → 调用 ItemService 查物品列表
- `getLottery(userId)` → 直接查 lottery_draws 表
- `getTrades(userId)` → 直接查 trade_orders + market_listings
- `getTimeline(userId)` → 多表 UNION 按时间排序
- `getRisk(userId)` → 查 user_risk_profiles + risk_alerts
- `getHistory(userId)` → 查历史 customer_service_sessions
...
| 总表数 | 96张 | **96张** | ✅ 一致 |
| 用户数 | 66 | **66** | ✅ 一致 |
| 物品实例 | 6758 | **6758**（available:4196 locked:624 transferred:424 used:1511 expired:3）| ✅ 一致 |
| 抽奖记录 | 3030 | **3030** | ✅ 一致 |
...
| 资产流水 | （文档未提及） | **28,685条** | 📌 数据量大，timeline接口需分页 |
| 操作日志 | （文档未提及） | **5,822条** | 📌 审计日志已有大量数据可复用 |
| 物品事件 | （文档未提及） | **5,345条** | 📌 item_instance_events 已有数据 |
| 消费记录 | （文档未提及） | **48条** | 📌 数据量少 |
| 风控告警 | （文档未提及） | **9条** | 📌 数据量少 |
...
| B7 | 新建 CustomerServiceUserContextService | Service | 聚合查询服务，全部只读，调用现有 BalanceService/ItemService 等 |
...
| M3 | 字段名对齐后端 | 代码修改 | 直接使用后端返回的字段名（如 `customer_service_session_id` 非 `sessionId`） |
...
| 模式 | 已有范例 | 扩展方式 |
|------|---------|---------|
| 路由文件拆分 | `routes/v4/console/customer-service/` 下已有 5 个文件 | 新增 user-context.js / issues.js / gm-tools.js |
| Service 静态类 | `BalanceService`、`ItemService` 等 | 新增 Service 遵循同样的静态方法 + options.transaction 模式 |
...
| Facade 模式 | `AdminCustomerServiceService` 包装 `CustomerServiceSessionService` | 可选择是否为新 Service 建 Facade |
...
| 决策 | 推荐选项 | 核心理由 | 参考行业 |
|------|---------|---------|---------|
| 1. 前端目录结构 | B: content/ 内 cs- 前缀 | 1-2人团队不需要模块拆分 | 米哈游/中小公司 |
| 2. Schema 变更 | A: 标准 Migration | 全行业唯一正确做法 | 全行业共识 |
| 3. 补偿权限 | A: admin-only + 预留阈值扩展 | 1坐席4admin，先简单后扩展 | 游戏公司初期 |
| 4. 工单类型 | B: 8种（加 consumption + feedback） | 覆盖所有活跃业务模块 | 美团/5173混合 |
| 5. 资产隔离 | A: GLOBAL 默认 + 活动折叠展开 | 主货币突出，活动按需展开 | 米哈游 |
| 6. 冻结阈值 | B: 30分钟告警 + 2小时严重 | C2C自动撮合，30分钟足够判定异常 | 交易猫/美团混合 |
| 7. 满意度触发 | C: 自动内嵌卡片 + 手动补发 | 不打扰用户，评分率20-35% | 企业微信/抖音 |
...
### Phase 1：后端 user-context API（预计2天）

**文件清单**：

| 文件 | 动作 | 行数估算 |
|------|------|---------|
| `services/CustomerServiceUserContextService.js` | 新建 | ~300行 |
| `routes/v4/console/customer-service/user-context.js` | 新建 | ~150行 |
| `routes/v4/console/customer-service/index.js` | 修改 | +2行挂载 |
| `services/index.js` | 修改 | +1行注册 |

**核心实现逻辑**：

- `getSummary(userId)` → 并行 COUNT：
  ```javascript
  const [assetCount, itemCount, lotteryCount, tradeCount, feedbackCount] = await Promise.all([
    AccountAssetBalance.count({ where: { account_id }, ... }),
    ItemInstance.count({ where: { owner_user_id: userId } }),
    // ... 复用现有 Model 的 count
  ])
  ```

- `getAssets(userId)` → 调用 `BalanceService.getAllBalances()` + 查 `asset_transactions` 最近5条
  - 注意：需要先通过 `accounts` 表查 `account_id`（`accounts.user_id = userId`）
  - 只查 `lottery_campaign_key = 'GLOBAL'` 的余额（取决于决策5）

- `getBackpack(userId)` → 调用 `ItemService.getUserItemInstances()` 已有方法

- `getLottery(userId)` → 直接查 `LotteryDraw.findAndCountAll({ where: { user_id } })`

- `getTrades(userId)` → 调用 `TradeOrderService.getUserOrders()` + 查 `MarketListing`

- `getTimeline(userId)` → 多表 UNION ALL（Sequelize raw query），按 created_at DESC

- `getRisk(userId)` → 查 `UserRiskProfile` + `RiskAlert`

- `getHistory(userId)` → 查 `CustomerServiceSession` + 聚合每个session的消息摘要
...
```
            第1周                    第2周                    第3周
后端  ┤ Phase 0(0.5d) + Phase 1(2d) ┤ Phase 2(2d)           ┤ Phase 5后端部分(0.5d)
Web   ┤ Phase 3 前半(mock开发)       ┤ Phase 3对接 + Phase 4  ┤ Phase 5前端部分(1.5d)
小程序 ┤                              ┤                        ┤ M1 满意度(0.5d)
```

**第2周末**即可达到核心可用状态（A区重构 + B区增强 + C区8Tab + 诊断功能）。
