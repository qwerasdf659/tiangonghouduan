...
- [ ] 更新服务依赖图
- [ ] 更新本文档状态
- [ ] 子服务入口文件（index.js）有使用说明

### 12.4 命名一致性验收

- [ ] 后端路由文件名与服务名一致（如 `lottery-realtime.js` → `RealtimeService.js`）
- [ ] ServiceManager 服务键与服务类名一致（如 `lottery_realtime` → `RealtimeService`）
- [ ] 全局搜索确认无遗漏的旧 URL 引用
- [ ] 后端 API 功能正常（实时监控、统计、报表、用户分析、活动分析）
...
| ExchangeService.js | 1,873 | `exchange/` 目录（3个子服务） | 1处 |
| ConsumptionService.js | 1,825 | `consumption/` 目录（3个子服务） | 1处 |
...
**执行计划**：
- 第1批：AssetService（1-2天）→ 验证（0.5天）
- 第2批：其余6个服务（3-4天）→ 验证（1天）
- 总计：5.5-7.5天
