...
- **后端问题**：`allowedFields` 确实缺少 `win_weight` 和 `reward_tier`，代码第 531-558 行核实。
...
#### 任务十一~十四（活动级奖品管理、增删、排序、库存）— 需路径纠偏

- **后端可复用能力**：
  - 查询：`GET /prize-pool/list?lottery_campaign_id=X` 已支持按活动筛选
  - 添加：`POST /prize-pool/batch-add` 已支持
...
| 已有能力 | 位置 | 可复用于 |
|---------|------|---------|
| `PrizePoolService.getAllPrizes(filters)` | `services/PrizePoolService.js` | 任务 11 活动级查询（已支持 `lottery_campaign_id` 筛选） |
| `PrizePoolService.batchAddPrizes()` | 同上 | 任务 12 新增奖品 |
...
理由：大厂做数据库驱动是因为日活百万、活动日换，你的项目 1 个活动 5 个内置版本足够。硬编码的 `SegmentResolver` 已经能 `simulateResolve()` 做测试，不影响运营。等上线跑起来运营确实频繁改规则时，再建 `segment_rule_configs` 表不迟——你已经有 `SegmentResolver` 类可以无缝扩展。
...
理由：你的项目属于"餐饮 SaaS"而非游戏公司，不需要预实现所有算法。扩展点（ENUM 值）已经预留，等运营真正需要百分比模式时再实现，1 天可以加完。现在做是浪费工期，因为没有数据可以验证正确性。
...
| # | 决定 | 结论 | 理由关键词 |
|---|------|------|-----------|
| 1 | 分群动态管理 | **推迟到第三轮** | 1 个活动 5 个内置版本足够，`SegmentResolver` 已可扩展 |
| 2 | 删除策略 | **软删除（Sequelize paranoid）** | 行业无例外共识，改动仅 5 行，Sequelize 自动过滤 |
| 3 | 排序 UI | **拖拽（SortableJS CDN）** | 效率差 10 倍，所有现代后台标配 |
| 4 | 分组查询 | **新增独立 API** | 响应结构完全不同，职责分离 |
| 5 | normalize 引擎 | **推迟** | 扩展点已预留，无实际需求，等运营要了 1 天加完 |
...
填写：奖品名称、类型、选奖权重、所属档位、库存数量、图片、描述等
    ↓
保存 → POST /api/v4/console/lottery/:campaignId/prizes
    ↓
奖品列表自动刷新，新奖品出现在列表末尾
```
...
弹出确认对话框：
  ...
  └────────────────────────────────────────┘
    ↓
确认 → DELETE /api/v4/console/lottery/:campaignId/prizes/:prizeId
    ↓
奖品列表自动刷新
```
...
| `routes/v4/console/lottery.js` | POST 新增奖品 + DELETE 删除奖品路由 |
...
```
PUT /api/v4/console/lottery/:campaignId/prizes/reorder
Body:
{
  "prize_orders": [
...
| `services/PrizePoolService.js` | `batchUpdateSortOrder(campaignId, prizeOrders)` 事务批量更新方法 |
| `routes/v4/console/lottery.js` | PUT `/prizes/reorder` 路由 |
...
```
PUT /api/v4/console/lottery/:campaignId/prizes/:prizeId/stock
...
```
PUT /api/v4/console/lottery/:campaignId/prizes/batch-stock
Body: {
  "updates": [
...
| `routes/v4/console/lottery.js` | PUT `/prizes/:prizeId/stock` + PUT `/prizes/batch-stock` 路由 |
...
| 方法 | 路径 | 功能 |
|------|------|------|
| GET | `/api/v4/console/campaigns/:id/prizes` | 获取指定活动的奖品列表（按档位分组） |
| POST | `/api/v4/console/campaigns/:id/prizes` | 为指定活动新增奖品 |
| DELETE | `/api/v4/console/campaigns/:id/prizes/:prize_id` | 从指定活动中删除奖品（软删除） |
| PUT | `/api/v4/console/campaigns/:id/prizes/:prize_id/stock` | 快捷更新单个奖品库存 |
| PUT | `/api/v4/console/campaigns/:id/prizes/sort-order` | 批量更新奖品排序 |
...
| `routes/v4/console/campaigns.js` 或新增路由文件 | 修改/新增 | 5 个活动级奖品管理 API |
...
#### 步骤 1：任务一 — 加白名单（后端 2 行 + Web 前端 ~25 行）

**后端改动**：`services/PrizePoolService.js` 第 531-558 行 `allowedFields` 对象

```javascript
// 在 allowedFields 对象中加入：
win_weight: 'win_weight',
reward_tier: 'reward_tier'
```

改完后，`PUT /api/v4/console/prize-pool/prize/:id` 传入 `{ win_weight: 350000, reward_tier: 'high' }` 即可更新。审计日志会自动记录变更（`admin_operation_logs` 的 `changed_fields` 字段）。
...
#### 步骤 3：任务十二（增删奖品）— 复用已有 API + Web 前端 ~70 行

**后端**：无需新增 API。

- 新增奖品：复用 `POST /api/v4/console/prize-pool/batch-add`，传入 `lottery_campaign_id` + 单个奖品数组
- 删除奖品：复用 `DELETE /api/v4/console/prize-pool/prize/:id`

如需单个奖品添加的简化接口，可在 `prize_pool.js` 追加：

```
POST /api/v4/console/prize-pool/:code/add-prize
```

逻辑：从 URL 的 `campaign_code` 解析出 `lottery_campaign_id`，自动分配 `sort_order`，调用 `batchAddPrizes` 传入单奖品。
...
#### 步骤 10：任务九+十（normalize 引擎 + 选奖方式选择）

根据决定 5 的拍板结果执行。如推迟则跳过。

#### 步骤 11：任务三（分群动态管理）

根据决定 1 的拍板结果执行。如推迟则跳过。

### 后端新增代码量汇总

| 改动类型 | 文件 | 行数 |
|---------|------|------|
| `allowedFields` 加 2 字段 | `services/PrizePoolService.js` | +2 |
| 分组查询方法 | `services/PrizePoolService.js` | +40 |
| 批量排序方法 | `services/PrizePoolService.js` | +30 |
| 设置绝对库存方法 | `services/PrizePoolService.js` | +20 |
| 批量库存方法 | `services/PrizePoolService.js` | +25 |
| 风险提示检测 | `services/PrizePoolService.js` | +10 |
| 兜底保护检查 | `services/PrizePoolService.js` | +5 |
| 4 个新路由 | `routes/v4/console/prize_pool.js` | +80 |
| 迁移脚本 | `migrations/YYYYMMDD-remove-fallback.js` | +20 |
| **第一二轮后端总计** | | **~230 行** |
...
| 改动类型 | 文件 | 行数 |
|---------|------|------|
| prizeForm + 提交逻辑 | `composables/prizes.js` | +15 |
| 编辑 Modal 权重/档位 UI | `lottery-management.html` | +15 |
| 活动级管理面板 | `lottery-management.html` | +120 |
| 管理面板 JS 逻辑 | `composables/prizes.js` | +80 |
| 概率可视化 | `lottery-management.html` | +60 |
| 排序 UI（SortableJS） | `lottery-management.html` | +40 |
| 库存 inline 编辑 | `lottery-management.html` | +50 |
| **第一二轮 Web 前端总计** | | **~380 行** |
