...
| # | 任务 | 改动量 | 状态 | 详细方案 |
|---|------|--------|------|---------|
| 1 | 后端加 `win_weight`/`reward_tier` 到白名单 + 前端加编辑入口 | 3 个文件，+25 行 | ✅ 已完成 | [任务一](#问题说明) |
| 11 | 活动管理页新增「奖品管理」入口（选择指定活动进入专属奖品管理界面） | 前端 ~60 行，后端 ~20 行 | ✅ 后端已完成 | [任务十一](#任务十一活动管理页新增奖品管理入口) |
| 12 | 指定活动新增与删除奖品（在活动奖品管理界面中增删奖品） | 前端 ~70 行，后端 ~60 行 | ✅ 后端已完成 | [任务十二](#任务十二指定活动新增与删除奖品) |
| 14 | 活动奖品库存管理（行内编辑 + 快捷操作 + 批量调库存） | 前端 ~50 行，后端 ~25 行 | ✅ 后端已完成 | [任务十四](#任务十四活动奖品库存管理) |
...
| # | 任务 | 改动量 | 状态 | 详细方案 |
|---|------|--------|------|---------|
| 4 | 奖品列表显示档内实际占比（当前只显示原始权重数字，运营看不出实际概率） | 前端 ~20 行 | ✅ 后端已完成（grouped API返回tier_percentage） | [任务四](#任务四奖品列表显示档内实际占比) |
| 5 | 编辑活动弹窗增加奖品概览面板（一眼看到所有奖品的档位、权重、概率分布） | 前端 ~80 行 | 🔶 待前端开发（后端grouped API已就绪） | [任务五](#任务五编辑活动弹窗增加奖品概览面板) |
| 6 | 编辑奖品时显示同档位其他奖品（方便对比权重大小） | 前端 ~40 行 | 🔶 待前端开发（后端grouped API已就绪） | [任务六](#任务六编辑奖品时显示同档位其他奖品) |
| 7 | 库存=0 但权重>0 的风险提示（避免抽中没货的奖品） | 前后端各 ~10 行 | ✅ 已完成（后端warnings+前端标记） | [任务七](#任务七库存0-但权重0-的风险提示) |
| 13 | 活动奖品位置排序（拖拽或按钮调整奖品展示顺序 sort_order） | 前端 ~80 行，后端 ~30 行 | ✅ 后端已完成（batchUpdateSortOrder API就绪） | [任务十三](#任务十三活动奖品位置排序) |
...
| 3 | 用户分群规则动态管理（运营可视化搭建分群条件） | 8 个文件，新功能 | ✅ 后端已完成（模型+6个API+字段注册表） | [任务三](#任务三用户分群规则动态管理) |
...
| 10 | 创建活动时选择选奖方式（让运营可以选 tier_first 或 normalize） | 前后端各 ~30 行 | ✅ 后端已支持（pick_method字段，前端待实现选择UI） | [任务十](#任务十创建活动时选择选奖方式) |
