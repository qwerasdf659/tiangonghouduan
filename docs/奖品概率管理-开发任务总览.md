...
- `win_weight`、`reward_tier`、`is_fallback` 字段**数据库中已有数据**，且 `PrizePoolService.updatePrize()` 的 `allowedFields` **已包含**这三个字段（2026-02-22 已修复）。
- `500积分券`（win_weight=610000）和 `九八折券`（win_weight=200000）库存为 0 但权重 > 0，`updatePrize()` 现已正确返回 `warnings` 数组（2026-02-22 已修复）。
...
| 6 | 编辑奖品时显示同档位其他奖品（方便对比权重大小） | 前端 ~40 行 | ✅ 后端已完成（grouped API 返回档内所有奖品）❌ 前端未完成（编辑 Modal 中无同档位参照区） | [任务六](#任务六编辑奖品时显示同档位其他奖品) |
| 7 | 库存=0 但权重>0 的风险提示（避免抽中没货的奖品） | 前后端各 ~10 行 | ✅ 后端已完成（updatePrize 返回 warnings）⚠️ 前端部分完成（编辑弹窗有提示，奖品列表缺黄色警告标记） | [任务七](#任务七库存0-但权重0-的风险提示) |
| 13 | 活动奖品位置排序（拖拽或按钮调整奖品展示顺序 sort_order） | 前端 ~80 行，后端 ~30 行 | ✅ 后端已完成 ⚠️ 前端使用按钮排序（未引入 SortableJS 拖拽） | [任务十三](#任务十三活动奖品位置排序) |
...
| 3 | 用户分群规则动态管理（运营可视化搭建分群条件） | 8 个文件，新功能 | ✅ 后端已完成（模型+6个API+字段注册表+SEGMENT_FIELD_REGISTRY导出已修复）⚠️ 前端部分完成（JS逻辑有，条件构建器/策略列表/测试面板 HTML 缺失） | [任务三](#任务三用户分群规则动态管理) |
