...
async function testHealthCheck() {
  ...
  try {
    ...
    const isHealthy = (result.status === 200) && 
      (result.data.status === 'healthy' || result.data.data?.status === 'healthy' || result.data.success)
...
async function testStatisticsAPI() {
  console.log('\n🔍 测试 3: 弹窗统计 API...')
  try {
    const result = await makeRequest('GET', '/api/v4/console/popup-banners/statistics', null, adminToken)
...
async function testListAPI() {
  console.log('\n🔍 测试 4: 弹窗列表 API...')
  try {
    const result = await makeRequest('GET', '/api/v4/console/popup-banners?page=1&limit=10', null, adminToken)
    ...
    if (result.status === 200 && result.data.success) {
      ...
      if (data.banners && data.banners.length > 0) {
        ...
        data.banners.forEach((banner, index) => {
          console.log(`   ${index + 1}. [${banner.banner_id}] ${banner.title} - ${banner.is_active ? '启用' : '禁用'}`)
        })
      } else {
        console.log('   (暂无弹窗数据)')
      }
...
async function main() {
  ...
  if (await testAdminLogin()) {
    ...
    console.log('\n🔍 尝试测试公开的弹窗 API...')
    try {
      const publicResult = await makeRequest('GET', '/api/v4/system/popup-banners?position=home&limit=5')
