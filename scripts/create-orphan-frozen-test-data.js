...
async function createTestData() {
  ...
  try {
    ...
    for (let i = 0; i < users.length; i++) {
      ...
      log(`   ✅ 创建测试数据: user_id=${user.user_id}, asset_code=${assetCode}, frozen=${frozenAmount}`, 'green')
    }
    ...
    if (createdRecords.length > 0) {
      ...
      createdRecords.forEach((r, i) => {
        log(`   ${i+1}. balance_id=${r.balance_id}, user_id=${r.user_id}, ${r.asset_code}`, 'yellow')
  ...
  try {
    ...
    if (orphanList.length > 0) {
      orphanList.forEach((item, i) => {
        ...
        log(`   ${i+1}. user_id=${item.user_id}, asset_code=${item.asset_code}${marker}`, 'yellow')
    ...
    if (stats.by_asset && stats.by_asset.length > 0) {
      ...
      stats.by_asset.forEach(asset => {
        log(`   - ${asset.asset_code}: count=${asset.count}, amount=${asset.total_orphan_amount}`, 'yellow')
      })
    }
