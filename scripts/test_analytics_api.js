...
async function testTodayStats() {
  ...
  try {
    ...
    if (result.statusCode === 200 && result.data.success) {
      ...
      console.log(
        `     - user_stats.active_users_today: ${data.user_stats?.active_users_today || '-'}`
      )
      ...
      console.log(
        `     - lottery_stats.high_tier_draws_today: ${data.lottery_stats?.high_tier_draws_today || '-'}`
      )
      console.log(
        `     - points_stats.points_earned_today: ${data.points_stats?.points_earned_today || '-'}`
      )
      console.log(
        `     - points_stats.points_spent_today: ${data.points_stats?.points_spent_today || '-'}`
      )
      console.log(
        `     - inventory_stats.new_items_today: ${data.inventory_stats?.new_items_today || '-'}`
      )
      console.log(
        `     - inventory_stats.used_items_today: ${data.inventory_stats?.used_items_today || '-'}`
      )
...
async function testPerformanceReport() {
  ...
  try {
    ...
    if (result.statusCode === 200 && result.data.success) {
      ...
      console.log(
        `     - database.total_lottery_draws: ${data.database?.total_lottery_draws || '-'}`
      )
      console.log(
        `     - health_indicators.database_responsive: ${data.health_indicators?.database_responsive}`
      )
