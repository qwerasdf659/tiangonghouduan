  ...
  try {
    ...
    if (frozenBalances.length > 0) {
      ...
      frozenBalances.slice(0, 3).forEach((b, i) => {
        log(
          `     ${i + 1}. user_id=${b.account?.user_id}, asset_code=${b.asset_code}, frozen=${b.frozen_amount}, available=${b.available_amount}`,
          'yellow'
        )
      })
    }
    ...
    if (stats.by_asset && stats.by_asset.length > 0) {
      ...
      stats.by_asset.forEach(assetStat => {
        log(
          `     ${assetStat.asset_code}: count=${assetStat.count}, amount=${assetStat.total_orphan_amount}, users=${assetStat.affected_user_count}`,
          'yellow'
        )
      })
    }
    ...
    log(
      JSON.stringify(detectApiResponse, null, 2)
        .split('\n')
        .map(l => '     ' + l)
        .join('\n'),
      'yellow'
