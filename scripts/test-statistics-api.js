    ...
    if (response.data.success) {
      ...
      Object.keys(data).forEach(key => {
        ...
        if (Array.isArray(value)) {
          ...
          console.log(`   • ${key}: Object with keys [${Object.keys(value).slice(0, 5).join(', ')}...]`)
        } else {
          console.log(`   • ${key}: ${value}`)
        }
      })
...
function generateAdaptationSuggestion(chartsData, reportData) {
  ...
  console.log(`   • customer_service: { total_sessions, closed_sessions, avg_response_time, satisfaction }`)
