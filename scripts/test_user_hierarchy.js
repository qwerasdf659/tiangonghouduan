...
async function testUserHierarchy() {
  ...
  try {
    ...
    roles.forEach(r => {
      console.log(
        `   - ID: ${r.role_id}, 名称: ${r.role_name}, 级别: ${r.role_level}, 激活: ${r.is_active}`
      )
    })
    ...
    hierarchies.forEach(h => {
      ...
      console.log(
        `     superior_user_id: ${h.superior_user_id || '无'}, 上级: ${h.superior?.nickname || '-'}`
      )
    ...
    users.forEach(u => {
      console.log(
        `   - ID: ${u.user_id}, 手机: ${u.mobile}, 昵称: ${u.nickname || '-'}, 状态: ${u.status}`
      )
    })
