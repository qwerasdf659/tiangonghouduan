...
async function checkDatabaseData() {
  ...
  try {
    ...
    if (healthResult.status === 200) {
      ...
      console.log(
        `   数据库状态: ${healthResult.data.database?.status || healthResult.data.checks?.database?.status || '未知'}`
      )
    }
...
async function diagnoseFrontendIssues(batchResult) {
  ...
  if (!batchResult.success) {
    ...
    // 检查数据完整性
    const campaignsWithZeroBudget = batchResult.campaigns.filter(
      c => (c.pool_budget?.total || 0) === 0
    )
    ...
    if (activitiesResult.data.success && activitiesResult.data.data) {
      ...
      if (activities.length > 0) {
        ...
        const updateResult = await httpRequest(
          `/api/v4/console/campaign-budget/campaigns/${campaignId}`,
          {
            method: 'PUT',
            body: {
              budget_mode: 'pool',
...
async function runTests() {
  ...
  // 步骤7: 如果没有数据，尝试创建测试数据
  if (
    !batchResult.campaigns ||
    batchResult.campaigns.length === 0 ||
