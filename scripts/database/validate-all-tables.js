...
function generateReport(validationResults) {
  ...
  const errorIssues = validationResults.reduce((sum, r) =>
    sum + r.issues.filter(i => i.severity === 'error').length, 0)
  const warningIssues = validationResults.reduce((sum, r) =>
  ...
  log(`   发现问题: ${totalIssues} (错误: ${errorIssues}, 警告: ${warningIssues})`,
  ...
  for (const result of validationResults) {
    ...
    const issueColor = issueCount > 0 ? (result.issues.some(i => i.severity === 'error') ? 'red' : 'yellow') : 'reset'

    log(`   ${tableName} | ${rowCount} | ${colCount} | ${idxCount} | ${fkCount} | ${colors[issueColor]}${issueStr}${colors.reset}`)
  }
  ...
  return {
    ...
    high_freq_tables: highFreqTables.map(t => ({ name: t.table_name, rows: parseInt(t.row_count) })),
