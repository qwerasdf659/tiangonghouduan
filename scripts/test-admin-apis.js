  ...
  for (const category of categories) {
    try {
      ...
      if (res.status === 200 && res.data.success) {
        ...
        log('success', `${category}设置: ${count}项配置`, settings.slice(0, 3).map(s => `${s.setting_key}=${s.parsed_value}`))
      } else {
...
async function testUserManagementAPI(token) {
  ...
  try {
    const res = await request(`${BASE_URL}/api/v4/console/user-management/users?page=1&limit=10`, { headers })
    ...
    if (res.status === 200 && res.data.success) {
      ...
      log('success', `用户列表获取成功: ${users.length}/${total}用户`, 
...
async function testRolesAPI(token) {
  ...
  try {
    ...
    if (res.status === 200 && res.data.success) {
      ...
      log('success', `角色列表获取成功: ${roles.length}个角色`, 
...
async function testPrizePoolAPI(token) {
  ...
  try {
    ...
    if (res.status === 200 && res.data.success) {
      ...
      log('success', `奖品列表获取成功: ${prizes.length}个奖品`, 
        prizes.slice(0, 5).map(p => ({ id: p.prize_id, name: p.prize_name, prob: p.win_probability })))
...
async function initializeDefaultSettings() {
  ...
  try {
    ...
    const defaultSettings = [
      // 基础设置
      { category: 'basic', setting_key: 'system_name', setting_value: '餐厅抽奖系统', value_type: 'string', description: '系统名称' },
      { category: 'basic', setting_key: 'system_version', setting_value: 'v4.0.0', value_type: 'string', description: '系统版本' },
      { category: 'basic', setting_key: 'customer_phone', setting_value: '400-999-8888', value_type: 'string', description: '客服电话' },
      { category: 'basic', setting_key: 'customer_email', setting_value: 'support@example.com', value_type: 'string', description: '客服邮箱' },
      
      // 积分设置
      { category: 'points', setting_key: 'sign_in_points', setting_value: '10', value_type: 'number', description: '每日签到积分' },
      { category: 'points', setting_key: 'initial_points', setting_value: '100', value_type: 'number', description: '新用户初始积分' },
      { category: 'points', setting_key: 'points_expire_days', setting_value: '365', value_type: 'number', description: '积分有效期（天）' },
      { category: 'points', setting_key: 'budget_allocation_ratio', setting_value: '0.3', value_type: 'number', description: '预算分配系数' },
      
      // 通知设置
      { category: 'notification', setting_key: 'sms_enabled', setting_value: 'false', value_type: 'boolean', description: '短信通知开关' },
      { category: 'notification', setting_key: 'email_enabled', setting_value: 'false', value_type: 'boolean', description: '邮件通知开关' },
      { category: 'notification', setting_key: 'app_notification_enabled', setting_value: 'true', value_type: 'boolean', description: '应用内通知开关' },
      
      // 安全设置
      { category: 'security', setting_key: 'max_login_attempts', setting_value: '5', value_type: 'number', description: '最大登录尝试次数' },
      { category: 'security', setting_key: 'lockout_duration', setting_value: '30', value_type: 'number', description: '锁定时长（分钟）' },
      { category: 'security', setting_key: 'password_min_length', setting_value: '6', value_type: 'number', description: '密码最小长度' },
      { category: 'security', setting_key: 'api_rate_limit', setting_value: '100', value_type: 'number', description: 'API速率限制（次/分钟）' }
    ]
    ...
    for (const setting of defaultSettings) {
      ...
      if (wasCreated) {
        created++
        log('success', `创建设置: ${setting.category}/${setting.setting_key} = ${setting.setting_value}`)
      } else {
        skipped++
      }
    }
