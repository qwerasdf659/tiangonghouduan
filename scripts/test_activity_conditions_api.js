  ...
  try {
    ...
    if (campaigns.length === 0) {
      ...
      campaigns.forEach((c, i) => {
        const hasConditions =
          c.participation_conditions && Object.keys(c.participation_conditions).length > 0
    ...
    try {
      ...
      console.log(
        `   - participation_conditions: ${JSON.stringify(conditionConfig.participation_conditions || {})}`
      )
      console.log(
        `   - condition_error_messages: ${JSON.stringify(conditionConfig.condition_error_messages || {})}`
      )
    } catch (e) {
    ...
    if (updatedCampaign.participation_conditions) {
      ...
      console.log(
        `   - participation_conditions: ${JSON.stringify(updatedCampaign.participation_conditions)}`
      )
      console.log(
        `   - condition_error_messages: ${JSON.stringify(updatedCampaign.condition_error_messages)}`
      )
    } else {
      console.log('❌ 条件配置未保存')
    }
