/**
 * 修复user_inventory表的重复外键约束
 * 问题：user_inventory表有两个重复的user_id外键，且UPDATE规则都是NO ACTION
 * 解决：删除重复外键，保留一个并修改为正确的规则
 *
 * 创建时间：2025年10月10日
 */
...
async function main () {
  console.log('🔧 修复user_inventory表的重复外键约束')
  ...
  try {
    ...
    const [currentFK] = await sequelize.query(`
      ...
      WHERE rc.CONSTRAINT_SCHEMA = DATABASE()
        AND kcu.TABLE_NAME = 'user_inventory'
        AND kcu.COLUMN_NAME = 'user_id'
      ORDER BY rc.CONSTRAINT_NAME
    `)

    console.log('📋 当前user_inventory.user_id的外键约束:')
    ...
    if (currentFK.length > 1) {
      ...
      for (const fk of currentFK) {
        ...
        await sequelize.query(`ALTER TABLE user_inventory DROP FOREIGN KEY ${fk.CONSTRAINT_NAME}`)
      }
    ...
    if (newFK.length === 1 && newFK[0].DELETE_RULE === 'CASCADE' && newFK[0].UPDATE_RULE === 'CASCADE') {
      console.log('\n✅ 修复成功！user_inventory.user_id外键规则已正确配置')
    } else {
      console.log('\n⚠️  修复可能未完全成功，请检查')
    }
