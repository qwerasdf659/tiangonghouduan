...
const sequelize = new Sequelize(
  ...
  process.env.DB_PASSWORD,
  {
    ...
    dialect: 'mysql',
    timezone: '+08:00', // 北京时间（Beijing Time）
    logging: false
  }
)
...
async function repairOrphanConsumption (transaction) {
  ...
  for (const record of orphanRecords) {
    try {
      ...
      console.log(`✅ 修复 record_id=${record.record_id}, user_id=${record.user_id}, points=${record.points_to_award}`)
      repairedCount++
    } catch (error) {
...
async function repairApprovedWithoutTransaction (transaction) {
  ...
  for (const record of approvedRecords) {
    try {
      ...
      console.log(`✅ 修复 record_id=${record.record_id}, user_id=${record.user_id}, points=${pointsToAward} (approved)`)
      repairedCount++
    } catch (error) {
      console.error(`❌ 修复 record_id=${record.record_id} 失败:`, error.message)
    }
