...
async function step2_getAssetTypes() {
  ...
  try {
    const response = await httpRequest('GET', '/api/v4/console/material/asset-types?is_enabled=true')
  ...
  try {
    ...
    if (response.data.success) {
      ...
      if (items.length > 0) {
        ...
        console.log(`   - cost_asset_code字段: ${firstItem.cost_asset_code !== undefined ? '✅' : '❌'}`)
