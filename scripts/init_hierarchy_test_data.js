...
async function initHierarchyTestData() {
  ...
  try {
    ...
    console.log(
      `   区域负责人角色: ${regionalManagerRole.role_name} (ID: ${regionalManagerRole.role_id})`
    )
    console.log(
      `   业务经理角色: ${businessManagerRole.role_name} (ID: ${businessManagerRole.role_id})`
    )
    ...
    console.log(
      `   创建区域负责人: ${users[0].nickname || users[0].mobile} (ID: ${users[0].user_id})`
    )
    ...
    for (let i = 1; i <= 2 && i < users.length; i++) {
      ...
      console.log(
        `   创建业务经理: ${users[i].nickname || users[i].mobile} (ID: ${users[i].user_id}) → 上级: ${users[0].user_id}`
      )
    }
    ...
    for (let i = 3; i < users.length && i < 8; i++) {
      ...
      console.log(
        `   创建业务员: ${users[i].nickname || users[i].mobile} (ID: ${users[i].user_id}) → 上级: ${users[superiorIndex].user_id}, 激活: ${i < 6}`
      )
    }
    ...
    createdHierarchies.forEach(h => {
      const superiorInfo = h.superior
        ...
        : '(顶级)'
      console.log(
        `   [${h.hierarchy_id}] ${h.user?.nickname || h.user?.mobile} | ${h.role?.role_name} ${superiorInfo} | 激活: ${h.is_active}`
      )
    })
