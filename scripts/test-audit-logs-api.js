...
async function runTests() {
  ...
  if (ADMIN_TOKEN) {
    ...
    console.log('═'.repeat(60))
    try {
      const res = await makeRequest('GET', '/api/v4/console/system/audit-logs?page=1&page_size=5', ADMIN_TOKEN)
      console.log(`   状态码: ${res.status}`)
    ...
    console.log('═'.repeat(60))
    try {
      const res = await makeRequest('GET', '/api/v4/console/system/audit-logs/statistics', ADMIN_TOKEN)
      console.log(`   状态码: ${res.status}`)
    ...
    try {
      // 先获取列表，取第一条日志的ID
      const listRes = await makeRequest('GET', '/api/v4/console/system/audit-logs?page=1&page_size=1', ADMIN_TOKEN)
      ...
      if (listRes.status === 200 && listRes.data.success && listRes.data.data?.logs?.length > 0) {
        ...
        const res = await makeRequest('GET', `/api/v4/console/system/audit-logs/${logId}`, ADMIN_TOKEN)
        console.log(`   状态码: ${res.status}`)
