...
async function testGetImageList() {
  ...
  if (result.data?.success) {
    ...
    console.log(
      `      - statistics.total_size: ${result.data.data?.statistics?.total_size || 0} bytes`
    )
    console.log(
      `      - statistics.weekly_uploads: ${result.data.data?.statistics?.weekly_uploads || 0}`
    )
    console.log(
      `      - statistics.orphan_count: ${result.data.data?.statistics?.orphan_count || 0}`
    )
...
async function testFilterByBusinessType() {
  ...
  for (const type of businessTypes) {
    ...
    const result = await request(
      'GET',
      `/api/v4/console/images?business_type=${type}&page=1&page_size=5`
    )
...
async function testGetImagesByBusiness() {
  ...
  const result = await request(
    'GET',
    '/api/v4/console/images/by-business?business_type=lottery&context_id=1'
  )
