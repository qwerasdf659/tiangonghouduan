...
- ✅ **用户体验优化** - 🔴 v2.1.4核心改进：简化用户操作流程，提升用户体验，减少用户输入步骤
...
const ACTUAL_SERVICE_ARCHITECTURE = {
  ...
  routes: {
    structure: 'routes/',
    modules: [
      'auth.js - 认证授权（377行）：🚧 开发阶段简化版 - 手机号登录跳过短信验证、管理员隐藏登录入口（连续点击标题5次触发）、JWT双Token机制、新用户1000积分奖励、管理员二次验证暂时禁用',
      ...
      'photo.js - 拍照上传（363行）：🔴 v2.1.4重大简化 - 用户仅上传照片无需输入金额、商家审核时直接设置消费金额、Sealos云存储集成、人工审核流程优化、积分奖励自动计算（金额×10）',
