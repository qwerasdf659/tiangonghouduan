...
> **⚠️ 重要提醒**：当前为开发阶段，为便于前后端数据库对接开发，以下接口功能**暂停开发**：

- 📱 **短信验证接口**：`POST /api/auth/send-sms` - 开发阶段直接返回成功，不发送短信
- 🔐 **管理员二次验证接口**：`POST /api/auth/admin-verify-sms` - 开发阶段跳过短信验证
- 📞 **所有短信相关API**：暂时不接入短信服务商，但保留接口结构

**接口开发指导**：
- 💡 **用户登录接口**：`POST /api/auth/login` - 接受任意6位验证码（如：123456）
- 💡 **管理员登录接口**：`POST /api/auth/admin-login` - 仅验证账号密码，跳过短信验证
- 💡 **前端适配**：前端隐藏管理员登录入口，通过连续点击标题5次触发
- 💡 **数据预留**：接口返回结构预留短信验证字段，便于后续集成

### 1.7 🔴 v2.1.4版本重要接口变更

**📋 v2.1.4核心变更：拍照上传系统简化，移除用户手动输入消费金额功能**

#### 1.7.1 已删除的接口功能
- ❌ **用户输入金额功能** - v2.1.4版本移除用户手动输入消费金额的步骤
- ❌ **前端金额输入界面** - 用户上传照片后直接提交审核，无需输入金额
- ❌ **amount参数验证** - `POST /api/photo/upload`接口移除amount必需参数

#### 1.7.2 修改的接口功能
- 🔄 **拍照上传接口** - `POST /api/photo/upload`
  - 🔴 **重大简化**：移除必需参数`amount`（用户不再需要输入消费金额）
  - ✅ **仅需参数**：`file`（照片文件）
  - ✅ **用户体验**：一键上传，无需额外输入
  - 改为等待人工审核模式，商家审核时设置消费金额

- 🔄 **商家审核接口** - `POST /api/merchant/review/:id/approve`
  - 🔴 **新增必需参数**：`consumption_amount`（商家根据照片确认的实际消费金额）
  - ✅ **审核流程**：商家查看照片 → 设置消费金额 → 确认审核通过
  - ✅ **积分计算**：系统根据商家设置的金额自动计算积分奖励（金额×10）
...
#### 1.7.3 新增的接口功能
- ✅ **管理员隐藏登录接口** - `POST /api/auth/admin-login`（开发阶段跳过短信验证）
- ✅ **简化拍照上传接口** - `POST /api/photo/upload`（v2.1.4简化版，仅需上传照片）
- ✅ **商家金额确认接口** - `POST /api/merchant/review/:id/set-amount`（商家设置消费金额）
- ✅ **用户体验优化** - 所有接口响应时间优化，减少用户等待时间
