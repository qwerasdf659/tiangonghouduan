...
- **更新时间**：2025年01月17日 - 基于当前运行代码的完整验证和更新，已确认商品展示API正常
...
Body: {
  "draw_type": "single"        // 支持英文和中文类型，详见下方映射规范
}

**🔴 抽奖类型映射规范（2025年1月12日新增）：**
```javascript
// 🚨 重要：前端可以发送中文或英文抽奖类型，后端已支持完整映射
const SUPPORTED_DRAW_TYPES = {
  // 英文类型（原有支持）
  'single': 1,     // 单次抽奖
  'triple': 3,     // 三连抽  
  'quintuple': 5,  // 五连抽
...
// ✅ 前端发送示例：
// { "draw_type": "十连抽" }     → 10次抽奖，扣除1000积分
// { "draw_type": "five" }      → 5次抽奖，扣除500积分
// { "draw_type": "single" }    → 1次抽奖，扣除100积分
```
...
// 接口状态：正常运行，返回状态码200
// 返回数据：成功返回1个商品"玉石1"，数据结构完整
// 认证机制：JWT token认证正常，使用user_id字段
```
