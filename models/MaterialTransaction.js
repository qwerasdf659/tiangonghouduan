 ...
 * 关联关系：
 * - User：多对一（多条流水属于一个用户）
 * - MaterialAssetType：多对一（多条流水对应一个资产类型）
 */
...
class MaterialTransaction extends Model {
  ...
  static associate (models) {
    // 多对一：多条流水属于一个用户
    MaterialTransaction.belongsTo(models.User, {
      foreignKey: 'user_id',
      as: 'user',
      comment: '关联用户信息'
    })
    ...
    // 多对一：多条流水对应一个资产类型
    MaterialTransaction.belongsTo(models.MaterialAssetType, {
 ...
 * @returns {typeof MaterialTransaction} 初始化后的模型类
 */
  ...
  MaterialTransaction.init(
    {
      ...
      // 交易类型
      tx_type: {
        type: DataTypes.ENUM('earn', 'consume', 'convert_in', 'convert_out', 'admin_adjust'),
        allowNull: false,
