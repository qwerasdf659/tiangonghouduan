...
/**
 * 添加 asset_orphan_cleanup 到 admin_operation_logs 的 operation_type 枚举
 *
 * 业务背景（P0-2）：
 * - 孤儿冻结清理需要统一入口（OrphanFrozenCleanupService）
 * - 清理操作需要记录审计日志
 * - operation_type 需要新增 asset_orphan_cleanup 值
 *
 * 解决方案：
 * - 修改 operation_type 枚举，添加 asset_orphan_cleanup 值
 * - 从统一枚举定义获取所有值，确保代码与数据库一致
 *
 * 决策时间：2026-01-09
...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    try {
      console.log('🔧 开始执行：添加 asset_orphan_cleanup 到 operation_type 枚举（P0-2）')
      ...
      // 检查是否已包含 asset_orphan_cleanup
      if (currentType.includes('asset_orphan_cleanup')) {
        console.log('   ⏭️ asset_orphan_cleanup 已存在于枚举中，跳过修改')
      ...
      // 3. 提交事务
      await transaction.commit()
      console.log('✅ 迁移完成：asset_orphan_cleanup 操作类型已添加（P0-2）')
    } catch (error) {
  ...
  async down(queryInterface, Sequelize) {
    ...
    // 删除枚举值需要确保没有数据使用该值
    console.log('⚠️ 回滚操作：不删除 asset_orphan_cleanup，保持现有枚举')
