...
module.exports = {
  ...
  async up (queryInterface, Sequelize) {
    ...
    try {
       ...
       * ========================================
       */
      const tableExists = await queryInterface.tableExists('material_conversion_rules', { transaction })
      if (tableExists) {
      ...
      await queryInterface.createTable('material_conversion_rules', {
        ...
        to_amount: {
          type: Sequelize.BIGINT,
          allowNull: false,
          comment: '目标材料数量（转换后获得的目标材料数量），例如：合成规则为1、分解规则为9（9折下行）'
        },
        ...
        effective_at: {
          type: Sequelize.DATE,
          allowNull: false,
          comment: '生效时间（北京时间），用于版本化管理。改比例时新增未来生效规则，不覆盖历史规则'
        },
      ...
      await queryInterface.addIndex('material_conversion_rules', ['from_asset_code', 'to_asset_code', 'is_enabled', 'effective_at'], {
  ...
  async down (queryInterface, Sequelize) {
    ...
    try {
      ...
      // 删除外键约束
      await queryInterface.removeConstraint('material_conversion_rules', 'fk_material_conversion_rules_to_asset', { transaction })
      console.log('✓ 外键约束 to_asset_code 删除成功')

      await queryInterface.removeConstraint('material_conversion_rules', 'fk_material_conversion_rules_from_asset', { transaction })
