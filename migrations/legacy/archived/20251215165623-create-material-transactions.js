      ...
      await queryInterface.createTable(
        'material_transactions',
        {
          ...
          tx_type: {
            ...
            allowNull: false,
            comment:
              '交易类型：earn（获得）、consume（消耗）、convert_in（转换收入）、convert_out（转换支出）、admin_adjust（管理员调整）'
          },
          ...
          amount: {
            ...
            allowNull: false,
            comment:
              '金额（统一正数，方向由tx_type决定）。earn/convert_in为收入，consume/convert_out为支出'
          },
          ...
          business_type: {
            ...
            allowNull: false,
            comment:
              '业务类型（用于分类统计和追溯），如：lottery_reward（抽奖发放）、exchange（兑换扣减）、material_convert（材料转换）、admin_adjust（管理员调整）'
          },
          ...
          business_id: {
            ...
            unique: true,
            comment:
              '业务ID（幂等键，必填，唯一约束），如：lottery_draw_{draw_id}_{asset_code}、exchange_{item_id}_{user_id}_{timestamp}、material_convert_{rule_id}_{user_id}_{timestamp}'
          },
          ...
          title: {
            ...
            allowNull: true,
            comment:
              '标题（用于前端展示和审计），如：抽奖获得碎红水晶、兑换商品消耗完整红水晶、合成完整红水晶'
          },
          ...
          meta: {
            ...
            allowNull: true,
            comment:
              '元数据（JSON格式，可选），可记录：rule_id（转换规则ID）、from_amount/to_amount（比例快照）、order_id（订单ID）、draw_id（抽奖ID）等'
          },
  ...
  async down(queryInterface, Sequelize) {
    ...
    try {
      ...
      await queryInterface.removeConstraint(
        'material_transactions',
        'fk_material_transactions_asset_code',
        { transaction }
      )
      ...
      await queryInterface.removeConstraint(
        'material_transactions',
        'fk_material_transactions_user_id',
        { transaction }
      )
