/**
 * 迁移文件：为lottery_draws表添加business_id字段
 *
 * 目的：实现抽奖记录的幂等控制，符合P0-3规范要求（P0-6任务）
 *
 * 变更内容：
 * - 在lottery_draws表添加business_id字段（VARCHAR(100)，可为空，有唯一索引）
 *
 * 业务场景：
 * 1. 防止重复提交时创建多条抽奖记录
 * 2. 通过business_id实现永久幂等保护
 * 3. 支持业务操作追溯（通过business_id查询原始记录）
 *
 * 幂等规则：
 * - business_id格式: `lottery_draw_${userId}_${campaignId}_${timestamp}`
 * - 同一business_id只能创建一条记录
 * - 重复提交返回已有记录（幂等）
 *
 * 技术规范参考：
 * - 规范 P0-3（所有资产变动必须有business_id幂等控制）
 * - 待办清单 P0-6（LotteryDrawService.draw抽奖结果未使用business_id幂等控制）
 *
 * 创建时间：2025-12-11
 * 使用模型：Claude Sonnet 4.5
 */

'use strict'

module.exports = {
  /**
   * 执行迁移：添加business_id字段
   * @param {Object} queryInterface - Sequelize QueryInterface
   * @param {Object} Sequelize - Sequelize类型定义
   * @returns {Promise<void>} 无返回值
   */
  up: async (queryInterface, Sequelize) => {
    console.log('📝 开始迁移：为lottery_draws表添加business_id字段')

    // 步骤1：检查字段是否已存在
    const [columns] = await queryInterface.sequelize.query(`
      SELECT COLUMN_NAME
      FROM INFORMATION_SCHEMA.COLUMNS
      WHERE TABLE_SCHEMA = DATABASE()
        AND TABLE_NAME = 'lottery_draws'
        AND COLUMN_NAME = 'business_id'
    `)

    if (columns.length > 0) {
      console.log('✅ business_id字段已存在，跳过迁移')
      return
    }

    // 步骤2：添加business_id字段
    console.log('正在添加business_id字段...')
    await queryInterface.addColumn('lottery_draws', 'business_id', {
      type: Sequelize.STRING(100),
      allowNull: true, // 允许为空，兼容历史数据
      // 注释：业务关联ID，用于幂等控制（格式：lottery_draw_用户ID_活动ID_时间戳）
      comment: '业务关联ID，用于幂等控制',
      after: 'draw_id' // 放在draw_id字段后面
    })

    console.log('✅ 成功添加business_id字段')

    // 步骤3：为business_id字段添加唯一索引（提高查询性能，确保幂等）
    console.log('正在为business_id字段创建唯一索引...')

    // 先检查索引是否已存在
    const [indexes] = await queryInterface.sequelize.query(`
      SELECT INDEX_NAME
      FROM INFORMATION_SCHEMA.STATISTICS
      WHERE TABLE_SCHEMA = DATABASE()
        AND TABLE_NAME = 'lottery_draws'
        AND INDEX_NAME = 'idx_lottery_draw_business_id'
    `)

    if (indexes.length === 0) {
      await queryInterface.addIndex('lottery_draws', ['business_id'], {
        name: 'idx_lottery_draw_business_id',
        unique: false, // 不使用唯一索引，因为历史数据可能为NULL
        comment: '业务ID索引，用于幂等查询'
      })
      console.log('✅ 成功创建business_id索引')
    } else {
      console.log('✅ business_id索引已存在，跳过创建')
    }

    // 步骤4：验证修改
    const [verifyColumns] = await queryInterface.sequelize.query(`
      SELECT COLUMN_NAME, COLUMN_TYPE, IS_NULLABLE, COLUMN_COMMENT
      FROM INFORMATION_SCHEMA.COLUMNS
      WHERE TABLE_SCHEMA = DATABASE()
        AND TABLE_NAME = 'lottery_draws'
        AND COLUMN_NAME = 'business_id'
    `)

    if (verifyColumns.length > 0) {
      const column = verifyColumns[0]
      console.log('修改后的字段信息:', {
        name: column.COLUMN_NAME,
        type: column.COLUMN_TYPE,
        nullable: column.IS_NULLABLE,
        comment: column.COLUMN_COMMENT
      })
      console.log('✅ 验证通过：business_id字段已成功添加')
    } else {
      throw new Error('验证失败：business_id字段未正确添加')
    }

    // 步骤5：验证索引
    const [verifyIndexes] = await queryInterface.sequelize.query(`
      SELECT INDEX_NAME, COLUMN_NAME, NON_UNIQUE
      FROM INFORMATION_SCHEMA.STATISTICS
      WHERE TABLE_SCHEMA = DATABASE()
        AND TABLE_NAME = 'lottery_draws'
        AND INDEX_NAME = 'idx_lottery_draw_business_id'
    `)

    if (verifyIndexes.length > 0) {
      const index = verifyIndexes[0]
      console.log('索引信息:', {
        name: index.INDEX_NAME,
        column: index.COLUMN_NAME,
        unique: index.NON_UNIQUE === 0 ? '是' : '否'
      })
      console.log('✅ 验证通过：business_id索引已成功创建')
    } else {
      throw new Error('验证失败：business_id索引未正确创建')
    }

    console.log('✅ 迁移完成')
  },

  /**
   * 回滚迁移：移除business_id字段和索引
   * @param {Object} queryInterface - Sequelize QueryInterface
   * @param {Object} _Sequelize - Sequelize类型定义（未使用）
   * @returns {Promise<void>} 无返回值
   */
  down: async (queryInterface, _Sequelize) => {
    console.log('🔄 开始回滚：移除lottery_draws表的business_id字段')

    // 步骤1：检查是否有使用business_id的记录
    const [records] = await queryInterface.sequelize.query(`
      SELECT COUNT(*) as count
      FROM lottery_draws
      WHERE business_id IS NOT NULL
    `)

    const count = records[0].count

    if (count > 0) {
      console.warn(
        `⚠️ 警告：存在${count}条包含business_id的抽奖记录。` +
          '回滚后这些记录的business_id信息将丢失。'
      )
    }

    // 步骤2：移除索引
    console.log('正在移除business_id索引...')

    const [indexes] = await queryInterface.sequelize.query(`
      SELECT INDEX_NAME
      FROM INFORMATION_SCHEMA.STATISTICS
      WHERE TABLE_SCHEMA = DATABASE()
        AND TABLE_NAME = 'lottery_draws'
        AND INDEX_NAME = 'idx_lottery_draw_business_id'
    `)

    if (indexes.length > 0) {
      await queryInterface.removeIndex('lottery_draws', 'idx_lottery_draw_business_id')
      console.log('✅ 成功移除business_id索引')
    } else {
      console.log('✅ business_id索引不存在，跳过移除')
    }

    // 步骤3：移除business_id字段
    console.log('正在移除business_id字段...')

    const [columns] = await queryInterface.sequelize.query(`
      SELECT COLUMN_NAME
      FROM INFORMATION_SCHEMA.COLUMNS
      WHERE TABLE_SCHEMA = DATABASE()
        AND TABLE_NAME = 'lottery_draws'
        AND COLUMN_NAME = 'business_id'
    `)

    if (columns.length > 0) {
      await queryInterface.removeColumn('lottery_draws', 'business_id')
      console.log('✅ 成功移除business_id字段')
    } else {
      console.log('✅ business_id字段不存在，跳过移除')
    }

    // 步骤4：验证回滚
    const [verifyColumns] = await queryInterface.sequelize.query(`
      SELECT COLUMN_NAME
      FROM INFORMATION_SCHEMA.COLUMNS
      WHERE TABLE_SCHEMA = DATABASE()
        AND TABLE_NAME = 'lottery_draws'
        AND COLUMN_NAME = 'business_id'
    `)

    if (verifyColumns.length === 0) {
      console.log('✅ 验证通过：business_id字段已成功移除')
    } else {
      throw new Error('验证失败：business_id字段未正确移除')
    }

    console.log('✅ 回滚完成')
  }
}
