...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    try {
      ...
      await queryInterface.createTable('account_asset_balances', {
        balance_id: {
        ...
        asset_code: {
          ...
          allowNull: false,
          comment: '资产代码（Asset Code）：如 DIAMOND、red_shard、red_crystal 等；唯一约束：(account_id, asset_code)'
        },
        available_amount: {
          type: Sequelize.BIGINT,
          allowNull: false,
          defaultValue: 0,
          comment: '可用余额（Available Amount）：可直接支付、转让、挂牌的余额；业务规则：不可为负数，所有扣减操作必须验证余额充足；单位：整数（BIGINT避免浮点精度问题）'
        },
        frozen_amount: {
          type: Sequelize.BIGINT,
          allowNull: false,
          defaultValue: 0,
          comment: '冻结余额（Frozen Amount）：下单冻结、挂牌冻结的余额；业务规则：交易市场购买时冻结买家DIAMOND，挂牌时冻结卖家标的资产；成交后从冻结转为扣减或入账；取消/超时时解冻回到 available_amount；不可为负数'
        },
        created_at: {
          type: Sequelize.DATE,
          allowNull: false,
      ...
      await queryInterface.addIndex('account_asset_balances', ['account_id', 'asset_code', 'campaign_id'], {
      ...
      await queryInterface.createTable('accounts', {
        ...
        account_type: {
          ...
          allowNull: false,
          comment: '账户类型（Account Type）：user-用户账户（关联真实用户，user_id必填）| system-系统账户（平台运营账户，system_code必填）'
        },
        user_id: {
          type: Sequelize.INTEGER,
          allowNull: true,
          comment: '用户ID（User ID）：当 account_type=user 时必填且唯一；当 account_type=system 时为NULL；外键关联 users.user_id'
        },
        system_code: {
          type: Sequelize.STRING(50),
          allowNull: true,
          comment: '系统账户代码（System Code）：当 account_type=system 时必填且唯一；预定义系统账户：SYSTEM_PLATFORM_FEE（平台手续费）、SYSTEM_MINT（系统发放）、SYSTEM_BURN（系统销毁）、SYSTEM_ESCROW（托管/争议）'
        },
        status: {
          type: Sequelize.ENUM('active','disabled'),
          allowNull: false,
          defaultValue: 'active',
          comment: '账户状态（Account Status）：active-活跃（可正常交易）| disabled-禁用（冻结状态，禁止任何交易）'
        },
        created_at: {
          type: Sequelize.DATE,
          allowNull: false,
      ...
      await queryInterface.createTable('admin_operation_logs', {
        ...
        operation_type: {
          type: Sequelize.ENUM('points_adjust','exchange_audit','product_update','product_create','product_delete','user_status_change','prize_config','prize_create','prize_delete','prize_stock_adjust','campaign_config','role_assign','role_change','system_config','session_assign','inventory_operation','inventory_transfer','consumption_audit'),
          allowNull: false,
          comment: '操作类型'
        },
        target_type: {
      ...
      await queryInterface.createTable('api_idempotency_requests', {
        request_id: {
          type: Sequelize.BIGINT,
          allowNull: false,
          primaryKey: true,
      ...
      await queryInterface.createTable('asset_transactions', {
        ...
        delta_amount: {
          type: Sequelize.BIGINT,
          allowNull: false,
          comment: '变动金额（Delta Amount - 资产变动数量，正数表示增加，负数表示扣减，单位：1个资产单位）'
        },
        balance_after: {
          type: Sequelize.BIGINT,
          allowNull: false,
        ...
        business_type: {
          type: Sequelize.STRING(50),
          allowNull: false,
          comment: '业务类型（Business Type - 业务场景分类）：market_purchase_buyer_debit-市场购买买家扣减, market_purchase_seller_credit-市场购买卖家入账, market_purchase_platform_fee_credit-市场购买平台手续费, exchange_debit-兑换扣减, material_convert_debit-材料转换扣减, material_convert_credit-材料转换入账'
        },
        meta: {
          type: Sequelize.JSON,
          allowNull: true,
          comment: '扩展信息（Meta - JSON格式存储业务扩展信息）：如order_no, item_id, conversion_rule等，用于业务追溯和审计'
        },
        created_at: {
          type: Sequelize.DATE,
          allowNull: false,
        ...
        idempotency_key: {
          type: Sequelize.STRING(100),
          allowNull: false,
          comment: '幂等键（每条流水唯一）：抽奖格式 {request_key}:consume/{request_key}:reward，其他格式 {type}_{account}_{ts}_{random}'
        }
      }, {
        transaction,
        charset: 'utf8mb4',
      ...
      await queryInterface.addIndex('asset_transactions', ['account_id', 'asset_code', 'created_at'], {
      ...
      await queryInterface.createTable('audit_records', {
        ...
        rejection_reason: {
          type: Sequelize.ENUM('amount_mismatch','invalid_receipt','duplicate_submission','fraud_suspected','merchant_error','other'),
          allowNull: true,
      ...
      await queryInterface.createTable('authentication_sessions', {
        user_session_id: {
          type: Sequelize.BIGINT,
          allowNull: false,
          primaryKey: true,
      ...
      await queryInterface.createTable('consumption_records', {
        record_id: {
          type: Sequelize.BIGINT,
          allowNull: false,
          primaryKey: true,
      ...
      await queryInterface.createTable('content_review_records', {
        audit_id: {
          type: Sequelize.BIGINT,
          allowNull: false,
          primaryKey: true,
      ...
      await queryInterface.createTable('customer_service_sessions', {
        user_id: {
          type: Sequelize.INTEGER,
          allowNull: true,
          comment: '外键引用（允许NULL）'
      ...
      await queryInterface.createTable('item_instance_events', {
        event_id: {
          type: Sequelize.BIGINT,
          allowNull: false,
          primaryKey: true,
      ...
      await queryInterface.addIndex('item_instance_events', ['item_instance_id', 'idempotency_key'], {
      ...
      await queryInterface.createTable('item_instances', {
        item_instance_id: {
          type: Sequelize.BIGINT,
          allowNull: false,
          primaryKey: true,
        ...
        status: {
          ...
          comment: '物品状态（available=可用/locked=锁定中/transferred=已转移/used=已使用/expired=已过期）'
        },
        meta: {
          type: Sequelize.JSON,
          allowNull: true,
          comment: '物品元数据（JSON格式，包含：name/description/icon/value/attributes/serial_number等）'
        },
        created_at: {
          type: Sequelize.DATE,
          allowNull: false,
        ...
        locks: {
          type: Sequelize.JSON,
          allowNull: true,
          comment: '锁定记录数组。格式: [{lock_type, lock_id, locked_at, expires_at, auto_release, reason}]。lock_type: trade/redemption/security'
        }
      }, {
        transaction,
        charset: 'utf8mb4',
      ...
      await queryInterface.createTable('lottery_campaigns', {
        ...
        campaign_type: {
          type: Sequelize.ENUM('daily','weekly','event','permanent','pool_basic','pool_advanced','pool_vip','pool_newbie'),
          allowNull: false,
        ...
        participation_conditions: {
          type: Sequelize.JSON,
          allowNull: true,
          comment: '参与条件配置（JSON格式，用途：存储活动参与条件规则，如用户积分≥100、用户类型=VIP等，业务场景：管理员在Web后台配置，用户端API自动验证，NULL表示无条件限制所有用户可参与）'
        },
        condition_error_messages: {
          type: Sequelize.JSON,
          allowNull: true,
          comment: '条件不满足时的提示语（JSON格式，用途：存储每个条件对应的用户友好错误提示，业务场景：用户不满足条件时显示具体原因，如"您的积分不足100分，快去消费获取积分吧！"）'
        }
      }, {
        transaction,
        charset: 'utf8mb4',
      ...
      await queryInterface.createTable('lottery_draw_quota_rules', {
        rule_id: {
          type: Sequelize.BIGINT,
          allowNull: false,
          primaryKey: true,
        ...
        scope_id: {
          type: Sequelize.STRING(100),
          allowNull: false,
          comment: '作用域ID：global固定为"global"，campaign存campaign_id，role存role_uuid，user存user_id'
        },
        window_type: {
      ...
      await queryInterface.addIndex('lottery_draw_quota_rules', ['scope_type', 'scope_id', 'status', 'effective_from', 'effective_to'], {
      ...
      await queryInterface.createTable('lottery_draws', {
        ...
        prize_type: {
          type: Sequelize.ENUM('points','coupon','physical','virtual','service','product','special'),
          allowNull: true,
      ...
      await queryInterface.createTable('lottery_management_settings', {
        setting_id: {
          type: Sequelize.STRING(50),
          allowNull: false,
          primaryKey: true,
      ...
      await queryInterface.addIndex('lottery_management_settings', ['user_id', 'setting_type', 'status'], {
      ...
      await queryInterface.createTable('lottery_user_daily_draw_quota', {
        quota_id: {
          type: Sequelize.BIGINT,
          allowNull: false,
          primaryKey: true,
      ...
      await queryInterface.addIndex('lottery_user_daily_draw_quota', ['quota_date', 'campaign_id'], {
        name: 'idx_date_campaign',
        unique: false,
        transaction
      })
      await queryInterface.addIndex('lottery_user_daily_draw_quota', ['user_id', 'campaign_id', 'quota_date'], {
      ...
      await queryInterface.createTable('market_listings', {
        ...
        listing_kind: {
          ...
          allowNull: false,
          comment: '挂牌类型（Listing Kind）：item_instance-不可叠加物品实例（如装备、卡牌）| fungible_asset-可叠加资产（如材料、钻石）；业务规则：决定标的资产字段的填充规则'
        },
        seller_user_id: {
          type: Sequelize.INTEGER,
          allowNull: false,
        ...
        offer_asset_code: {
          type: Sequelize.STRING(50),
          allowNull: true,
          comment: '标的资产代码（Offer Asset Code）：当 listing_kind=fungible_asset 时必填，如 red_shard、DIAMOND；业务规则：挂牌时必须冻结卖家该资产的 offer_amount 数量'
        },
        offer_amount: {
          type: Sequelize.BIGINT,
          allowNull: true,
          comment: '标的资产数量（Offer Amount）：当 listing_kind=fungible_asset 时必填，单位为 offer_asset_code 的最小单位；业务规则：必须 >0，挂牌时冻结该数量'
        },
        price_asset_code: {
          type: Sequelize.STRING(50),
          allowNull: false,
          defaultValue: 'DIAMOND',
          comment: '结算资产代码（Price Asset Code）：交易市场结算币种，固定为 DIAMOND；业务规则：前端和后端都强制校验只允许 DIAMOND'
        },
        price_amount: {
          type: Sequelize.BIGINT,
          allowNull: false,
          comment: '挂牌价格（Price Amount）：卖家设定的总价，单位为 price_asset_code（DIAMOND）；业务规则：必须 >0，成交时买家支付该金额（含手续费）'
        },
        seller_offer_frozen: {
          type: Sequelize.BOOLEAN,
          allowNull: false,
          defaultValue: 0,
          comment: '卖家标的是否已冻结（Seller Offer Frozen）：标记卖家标的资产是否已冻结；业务规则：listing_kind=fungible_asset 时必须为 true（挂牌时冻结卖家资产），listing_kind=item_instance 时为 false（物品实例不需要冻结）'
        },
        locked_by_order_id: {
          type: Sequelize.BIGINT,
          allowNull: true,
          comment: '锁定订单ID（Locked By Order ID）：记录当前锁定该挂牌的订单ID，外键关联 trade_orders.order_id；业务规则：status=locked 时必填，用于防止并发购买和超时解锁'
        },
        locked_at: {
          type: Sequelize.DATE,
          allowNull: true,
          comment: '锁定时间（Locked At）：记录挂牌被锁定的北京时间；业务规则：status=locked 时必填，用于超时解锁检查（默认超时时间：15分钟）'
        },
        status: {
          type: Sequelize.ENUM('on_sale','locked','sold','withdrawn'),
          allowNull: false,
          defaultValue: 'on_sale',
          comment: '挂牌状态（Status）：on_sale-在售中（可被购买或撤回）| locked-已锁定（订单处理中，不可购买或撤回）| sold-已售出（终态，成交完成）| withdrawn-已撤回（终态，卖家主动下架）；业务规则：on_sale → locked → sold/withdrawn，locked 超时自动回滚为 on_sale'
        },
        created_at: {
          type: Sequelize.DATE,
          allowNull: false,
      ...
      await queryInterface.createTable('material_asset_types', {
        ...
        asset_code: {
          type: Sequelize.STRING(50),
          allowNull: false,
          comment: '资产代码（Asset Code - 唯一标识）：如 red_shard/red_crystal/orange_shard，必须唯一，与 account_asset_balances.asset_code 关联'
        },
        display_name: {
          type: Sequelize.STRING(100),
          allowNull: false,
        ...
        group_code: {
          type: Sequelize.STRING(50),
          allowNull: false,
          comment: '分组代码（Group Code - 材料分组）：如 red/orange/yellow/green/blue/purple，用于材料逐级转换的层级归类'
        },
        form: {
        ...
        tier: {
          type: Sequelize.INTEGER,
          allowNull: false,
          comment: '层级（Tier - 材料层级）：数字越大层级越高，如 1-碎片层级，2-水晶层级，用于转换规则校验'
        },
        sort_order: {
          type: Sequelize.INTEGER,
          allowNull: false,
        ...
        visible_value_points: {
          type: Sequelize.BIGINT,
          allowNull: true,
          comment: '可见价值锚点（Visible Value Points - 展示口径）：用户可见的材料价值锚点，如 1 red_shard = 10 visible_value_points，用于展示与比较，可选'
        },
        budget_value_points: {
          type: Sequelize.BIGINT,
          allowNull: true,
          comment: '预算价值锚点（Budget Value Points - 系统口径）：系统内部预算计算口径，用于成本核算与风控，可选'
        },
        is_enabled: {
          type: Sequelize.BOOLEAN,
          allowNull: false,
          defaultValue: 1,
          comment: '是否启用（Is Enabled - 启用状态）：true-启用（可展示可转换），false-禁用（不可展示不可转换）'
        },
        created_at: {
          type: Sequelize.DATE,
          allowNull: false,
      ...
      await queryInterface.createTable('material_conversion_rules', {
        ...
        to_asset_code: {
          type: Sequelize.STRING(50),
          allowNull: false,
          comment: '目标资产代码（To Asset Code - 转换目标）：如 DIAMOND/red_crystal，表示转换成哪种资产'
        },
        from_amount: {
          type: Sequelize.BIGINT,
          allowNull: false,
          comment: '源资产数量（From Amount - 转换输入数量）：如 1，表示消耗 1 个源资产（如 1 red_shard）'
        },
        to_amount: {
          type: Sequelize.BIGINT,
          allowNull: false,
          comment: '目标资产数量（To Amount - 转换输出数量）：如 20，表示获得 20 个目标资产（如 20 DIAMOND），比例 = to_amount / from_amount'
        },
        effective_at: {
          type: Sequelize.DATE,
          allowNull: false,
          comment: '生效时间（Effective At - 版本化关键字段）：规则从此时间开始生效，查询时取当前时间前的最新已启用规则（WHERE effective_at <= NOW() AND is_enabled=true ORDER BY effective_at DESC LIMIT 1），确保历史流水可回放'
        },
        is_enabled: {
          type: Sequelize.BOOLEAN,
          allowNull: false,
          defaultValue: 1,
          comment: '是否启用（Is Enabled - 启用状态）：true-启用（规则生效），false-禁用（规则不生效）'
        },
        created_by: {
          type: Sequelize.INTEGER,
          allowNull: true,
      ...
      await queryInterface.addIndex('material_conversion_rules', ['from_asset_code', 'to_asset_code', 'effective_at'], {
      ...
      await queryInterface.createTable('merchant_points_reviews', {
        review_id: {
          type: Sequelize.STRING(100),
          allowNull: false,
          primaryKey: true,
        ...
        status: {
          ...
          allowNull: false,
          defaultValue: 'pending',
          comment: '审核状态：pending=审核中/approved=审核通过/rejected=审核拒绝/expired=审核超时/cancelled=已取消'
        },
        expires_at: {
          type: Sequelize.DATE,
          allowNull: false,
      ...
      await queryInterface.createTable('redemption_orders', {
        order_id: {
          type: Sequelize.STRING(36),
          allowNull: false,
          primaryKey: true,
        ...
        code_hash: {
          type: Sequelize.STRING(64),
          allowNull: false,
          comment: '核销码哈希（Code Hash）：12位Base32核销码的SHA-256哈希值（64位hex字符串），用于验证核销码，不存储明文'
        },
        item_instance_id: {
          type: Sequelize.BIGINT,
          allowNull: false,
          comment: '物品实例ID（Item Instance ID）：关联的物品实例，外键指向 item_instances.item_instance_id'
        },
        redeemer_user_id: {
          type: Sequelize.INTEGER,
          allowNull: true,
          comment: '核销用户ID（Redeemer User ID）：执行核销操作的用户ID，外键指向 users.user_id，核销前为NULL'
        },
        status: {
          type: Sequelize.ENUM('pending','fulfilled','cancelled','expired'),
          allowNull: false,
          defaultValue: 'pending',
          comment: '订单状态（Status）：pending-待核销 | fulfilled-已核销 | cancelled-已取消 | expired-已过期'
        },
        expires_at: {
          type: Sequelize.DATE,
          allowNull: false,
        ...
        charset: 'utf8mb4',
        collate: 'utf8mb4_unicode_ci',
        comment: '兑换订单表（Redemption Orders）：管理核销码生成和核销流程，替代 UserInventory.verification_code'
      })
      ...
      await queryInterface.createTable('trade_orders', {
        ...
        asset_code: {
          ...
          allowNull: false,
          defaultValue: 'DIAMOND',
          comment: '结算资产代码（Asset Code）：交易市场结算币种，固定为 DIAMOND；业务规则：前端和后端都强制校验只允许 DIAMOND'
        },
        gross_amount: {
          type: Sequelize.BIGINT,
          allowNull: false,
          comment: '买家支付总额（Gross Amount）：买家本次交易支付的总金额，单位为 asset_code（DIAMOND）；业务规则：必须 >0，等于 fee_amount + net_amount'
        },
        fee_amount: {
          type: Sequelize.BIGINT,
          allowNull: false,
          defaultValue: 0,
          comment: '平台手续费（Fee Amount）：从成交总额中拆分的平台手续费，单位为 asset_code（DIAMOND）；业务规则：≥0，手续费入系统账户 SYSTEM_PLATFORM_FEE'
        },
        net_amount: {
          type: Sequelize.BIGINT,
          allowNull: false,
          comment: '卖家实收金额（Net Amount）：卖家实际收到的金额，单位为 asset_code（DIAMOND）；业务规则：必须 >0，等于 gross_amount - fee_amount'
        },
        status: {
          type: Sequelize.ENUM('created','frozen','completed','cancelled','failed'),
          allowNull: false,
          defaultValue: 'created',
          comment: '订单状态（Status）：created-已创建（订单初始状态）| frozen-已冻结（买家资产已冻结，等待结算）| completed-已完成（成交完成，终态）| cancelled-已取消（订单取消，解冻买家资产，终态）| failed-失败（不可恢复错误，终态）；业务规则：created → frozen → completed/cancelled/failed'
        },
        meta: {
          type: Sequelize.JSON,
          allowNull: true,
          comment: '订单元数据（Meta）：保存关键请求参数指纹和扩展信息，用于 409 冲突保护和数据审计；示例：{ product_id, product_name, request_params_hash }'
        },
        created_at: {
          type: Sequelize.DATE,
          allowNull: false,
      ...
      await queryInterface.createTable('trade_records', {
        ...
        trade_type: {
          type: Sequelize.ENUM('point_transfer','exchange_refund','prize_claim','admin_adjustment','system_reward','inventory_transfer','market_purchase'),
          allowNull: false,
      ...
      await queryInterface.createTable('user_premium_status', {
        id: {
          type: Sequelize.BIGINT,
          allowNull: false,
          primaryKey: true,
        ...
        is_unlocked: {
          ...
          allowNull: false,
          defaultValue: 0,
          comment: '是否已解锁高级空间（当前状态，TRUE=已解锁且在有效期内，FALSE=未解锁或已过期，用于前端权限判断）'
        },
        unlock_time: {
          type: Sequelize.DATE,
          allowNull: true,
        ...
        unlock_method: {
          ...
          allowNull: false,
          defaultValue: 'points',
          comment: '解锁方式（points=积分解锁100分，exchange=兑换码解锁，vip=VIP会员解锁，manual=管理员手动解锁，扩展性预留字段）'
        },
        total_unlock_count: {
          type: Sequelize.INTEGER,
          allowNull: false,
          defaultValue: 0,
          comment: '累计解锁次数（包括首次解锁和重新解锁，每次解锁+1，用于运营分析用户活跃度和付费意愿）'
        },
        expires_at: {
          type: Sequelize.DATE,
          allowNull: true,
          comment: '过期时间（24小时有效期，unlock_time + 24小时，NULL表示未解锁或已过期，用于判断是否需要重新解锁，查询时WHERE expires_at > NOW()）'
        },
        created_at: {
          type: Sequelize.DATE,
          allowNull: false,
        ...
        charset: 'utf8mb4',
        collate: 'utf8mb4_unicode_ci',
        comment: '用户高级空间状态表（极简版，无自动续费字段，降低维护成本60%，适合数据量<1000的小项目）'
      })
      ...
      await queryInterface.createTable('websocket_startup_logs', {
        log_id: {
          type: Sequelize.BIGINT,
          allowNull: false,
          primaryKey: true,
