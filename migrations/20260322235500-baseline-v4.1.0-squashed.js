...
/**
 * Baseline v4.1.0 Squashed Migration
 *
 * Generated: 2026-03-22T23:56:19.362Z
 * Database: restaurant_points_dev
 * Tables: 115
 *
 * 本迁移代表截至 v4.1.0 的完整数据库 schema。
 * 所有历史迁移（64条）已被压缩为此单一基线文件。
 * 旧迁移文件归档至 migrations/archived/2026-03-22/
 */
...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    try {
      console.log('[baseline-v4.1] Creating 115 tables...')

      // [1/115] account_asset_balances
...
) ENGINE=InnoDB AUTO_INCREMENT=421 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'账户资产余额表（可用余额 + 冻结余额）\'', { transaction })
...
) ENGINE=InnoDB AUTO_INCREMENT=421 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'账户表（统一用户账户与系统账户）\'', { transaction })

      // [3/115] ad_antifraud_logs
...
) ENGINE=InnoDB AUTO_INCREMENT=11 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'反作弊判定日志表 — Phase 5 无效流量识别\'', { transaction })

      // [4/115] ad_attribution_logs
...
) ENGINE=InnoDB AUTO_INCREMENT=6 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'归因追踪日志表 — Phase 6 点击→转化关联\'', { transaction })
...
) ENGINE=InnoDB AUTO_INCREMENT=122 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'竞价记录表 — Phase 4 竞价排名\'', { transaction })

      // [6/115] ad_billing_records
...
) ENGINE=InnoDB AUTO_INCREMENT=101 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'广告投放计划表 — Phase 3 广告主自助投放\'', { transaction })
...
) ENGINE=InnoDB AUTO_INCREMENT=11 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'广告点击日志表 — Phase 5 归因数据源\'', { transaction })
  ...
  CONSTRAINT `ad_creatives_primary_media_id_foreign_idx` FOREIGN KEY (`primary_media_id`) REFERENCES `media_files` (`media_id`) ON DELETE SET NULL ON UPDATE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=89 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'广告素材表 — Phase 3 素材审核\'', { transaction })

      // [10/115] ad_dau_daily_stats
...
) ENGINE=InnoDB AUTO_INCREMENT=27 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'DAU 每日统计表（广告定价的 DAU 系数数据源）\'', { transaction })

      // [11/115] ad_impression_logs
...
) ENGINE=InnoDB AUTO_INCREMENT=17 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'广告曝光日志表 — Phase 5 反作弊过滤\'', { transaction })

      // [12/115] ad_interaction_logs
...
) ENGINE=InnoDB AUTO_INCREMENT=117 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'通用内容交互日志表 — 统一记录弹窗/轮播/公告/广告的展示、点击、关闭等交互事件\'', { transaction })

      // [13/115] ad_price_adjustment_logs
  ...
  PRIMARY KEY (`ad_price_adjustment_log_id`)
      ...
      // [14/115] ad_report_daily_snapshots
...
) ENGINE=InnoDB AUTO_INCREMENT=13 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'每日报表快照表 — Phase 6 凌晨4点聚合\'', { transaction })
...
) ENGINE=InnoDB AUTO_INCREMENT=16 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'广告位配置表 — Phase 3 广告主自助投放\'', { transaction })

      // [16/115] ad_target_zones
...
) ENGINE=InnoDB AUTO_INCREMENT=4 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'广告地域定向表（商圈 + 区域两级分类）\'', { transaction })

      // [17/115] ad_zone_group_members
...
) ENGINE=InnoDB AUTO_INCREMENT=3 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'联合广告组成员关联表\'', { transaction })

      // [18/115] ad_zone_groups
...
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'联合广告组（多地域打包投放）\'', { transaction })

      // [19/115] admin_notifications
...
) ENGINE=InnoDB AUTO_INCREMENT=14857 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'管理员通知消息表 - 存储系统通知、告警提醒、任务通知等\'', { transaction })

      // [20/115] admin_operation_logs
  ...
  `operation_type` varchar(50) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT \'操作类型（已从 ENUM 迁移为 VARCHAR）\',
...
) ENGINE=InnoDB AUTO_INCREMENT=7555 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'操作审计日志表（记录所有敏感操作）\'', { transaction })

      // [21/115] administrative_regions
  ...
  KEY `idx_alert_silence_type_active` (`alert_type`,`is_active`)
      ...
      // [23/115] api_idempotency_requests
...
) ENGINE=InnoDB AUTO_INCREMENT=4024042 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'API入口幂等表 - 实现重试返回首次结果\'', { transaction })

      // [24/115] approval_chain_instances
      await queryInterface.sequelize.query('CREATE TABLE IF NOT EXISTS `approval_chain_instances` (
...
) ENGINE=InnoDB AUTO_INCREMENT=148 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'审核链实例\'', { transaction })

      // [25/115] approval_chain_nodes
      await queryInterface.sequelize.query('CREATE TABLE IF NOT EXISTS `approval_chain_nodes` (
...
) ENGINE=InnoDB AUTO_INCREMENT=11 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'审核链节点定义\'', { transaction })

      // [26/115] approval_chain_steps
      await queryInterface.sequelize.query('CREATE TABLE IF NOT EXISTS `approval_chain_steps` (
...
) ENGINE=InnoDB AUTO_INCREMENT=148 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'审核链步骤执行记录\'', { transaction })

      // [27/115] approval_chain_templates
      await queryInterface.sequelize.query('CREATE TABLE IF NOT EXISTS `approval_chain_templates` (
...
) ENGINE=InnoDB AUTO_INCREMENT=8 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'审核链模板\'', { transaction })

      // [28/115] asset_group_defs
  ...
  `transaction_no` varchar(32) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT \'资产流水号（TX 前缀）\',
  PRIMARY KEY (`asset_transaction_id`),
  UNIQUE KEY `uk_idempotency_key` (`idempotency_key`),
  UNIQUE KEY `uk_asset_transactions_transaction_no` (`transaction_no`),
...
) ENGINE=InnoDB AUTO_INCREMENT=118872 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'资产流水表（Asset Transactions）- 记录所有资产变动流水，支持幂等性控制和审计追溯\'', { transaction })

      // [30/115] attribute_options
      await queryInterface.sequelize.query('CREATE TABLE IF NOT EXISTS `attribute_options` (
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'属性预设值（EAV 商品中心 Layer 1）\'', { transaction })

      // [31/115] attributes
      await queryInterface.sequelize.query('CREATE TABLE IF NOT EXISTS `attributes` (
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'属性定义（EAV 商品中心 Layer 1）\'', { transaction })

      // [32/115] authentication_sessions
...
) ENGINE=InnoDB AUTO_INCREMENT=14013 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'用户会话管理表\'', { transaction })

      // [33/115] batch_operation_logs
...
) ENGINE=InnoDB AUTO_INCREMENT=75 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'批量操作日志表 - 幂等性控制与操作审计（阶段C核心基础设施）\'', { transaction })
...
) ENGINE=InnoDB AUTO_INCREMENT=5 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'竞价商品表（臻选空间/幸运空间竞价功能，7态状态机）\'', { transaction })
  ...
  CONSTRAINT `bid_records_ibfk_2` FOREIGN KEY (`user_id`) REFERENCES `users` (`user_id`) ON DELETE RESTRICT ON UPDATE CASCADE
      ...
      // [36/115] categories
      await queryInterface.sequelize.query('CREATE TABLE IF NOT EXISTS `categories` (
  ...
  KEY `icon_media_id` (`icon_media_id`),
  ...
  CONSTRAINT `categories_ibfk_2` FOREIGN KEY (`icon_media_id`) REFERENCES `media_files` (`media_id`) ON DELETE SET NULL ON UPDATE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=122 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'品类树（EAV 商品中心 Layer 1）\'', { transaction })

      // [37/115] category_attributes
      await queryInterface.sequelize.query('CREATE TABLE IF NOT EXISTS `category_attributes` (
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'品类绑定属性（EAV 商品中心 Layer 1）\'', { transaction })

      // [38/115] chat_messages
...
) ENGINE=InnoDB AUTO_INCREMENT=22705 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'聊天消息表\'', { transaction })

      // [39/115] consumption_records
...
) ENGINE=InnoDB AUTO_INCREMENT=2697 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'用户消费记录表 - 记录用户通过商家扫码提交的消费信息\'', { transaction })

      // [40/115] content_review_records
...
) ENGINE=InnoDB AUTO_INCREMENT=5572 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci', { transaction })

      // [41/115] customer_service_agents
...
) ENGINE=InnoDB AUTO_INCREMENT=5 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'客服座席管理表（记录哪些用户是客服、配置、状态）\'', { transaction })

      // [42/115] customer_service_issues
  ...
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT \'创建时间\',
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT \'更新时间\',
  `trade_order_id` bigint DEFAULT NULL COMMENT \'关联的交易订单 ID（纠纷类型时必填）\',
  `dispute_type` enum(\'item_not_received\',\'item_mismatch\',\'quality_issue\',\'fraud\',\'other\') COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT \'纠纷类型：未收到物品/物品不符/质量问题/欺诈/其他\',
  `dispute_evidence` json DEFAULT NULL COMMENT \'纠纷证据（截图URL数组、文字描述等）\',
  `dispute_deadline` datetime DEFAULT NULL COMMENT \'纠纷处理截止时间（超时自动升级）\',
  `approval_chain_instance_id` bigint DEFAULT NULL COMMENT \'关联的审批链实例 ID（仲裁流程）\',
  ...
  KEY `idx_csi_trade_order_id` (`trade_order_id`),
...
) ENGINE=InnoDB AUTO_INCREMENT=5 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'客服工单表 - GM工作台问题跟踪\'', { transaction })

      // [43/115] customer_service_notes
...
) ENGINE=InnoDB AUTO_INCREMENT=3 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'客服内部备注表 - 仅客服可见，用户不可见\'', { transaction })

      // [44/115] customer_service_sessions
...
) ENGINE=InnoDB AUTO_INCREMENT=2088 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'客户聊天会话表\'', { transaction })

      // [45/115] customer_service_user_assignments
...
) ENGINE=InnoDB AUTO_INCREMENT=5 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'客服用户分配表（记录用户被分配给哪个客服）\'', { transaction })

      // [46/115] exchange_channel_prices
      await queryInterface.sequelize.query('CREATE TABLE IF NOT EXISTS `exchange_channel_prices` (
...
) ENGINE=InnoDB AUTO_INCREMENT=124 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'兑换渠道定价（Layer 2，管理SKU在兑换商城的材料资产价格）\'', { transaction })

      // [47/115] exchange_item_attribute_values
      await queryInterface.sequelize.query('CREATE TABLE IF NOT EXISTS `exchange_item_attribute_values` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT \'主键\',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'SPU非销售属性值（EAV 商品中心 Layer 1）\'', { transaction })

      // [48/115] exchange_item_skus
      await queryInterface.sequelize.query('CREATE TABLE IF NOT EXISTS `exchange_item_skus` (
...
) ENGINE=InnoDB AUTO_INCREMENT=124 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'统一SKU（EAV 商品中心 Layer 1，替代exchange_item_skus）\'', { transaction })

      // [49/115] exchange_items
...
) ENGINE=InnoDB AUTO_INCREMENT=256 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'统一SPU（EAV 商品中心 Layer 1，替代exchange_items）\'', { transaction })

      // [50/115] exchange_order_events
      await queryInterface.sequelize.query('CREATE TABLE IF NOT EXISTS `exchange_order_events` (
...
) ENGINE=InnoDB AUTO_INCREMENT=336 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'兑换订单状态变更事件表（审计追踪）\'', { transaction })

      // [51/115] exchange_rates
...
) ENGINE=InnoDB AUTO_INCREMENT=8 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'固定汇率兑换规则表 — 平台设定的资产间兑换汇率配置\'', { transaction })

      // [52/115] exchange_records
  ...
  `order_no` varchar(32) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT \'订单号（16位统一：EM/BD+北京YYMMDD+6位序列+2位hex）\',
  `idempotency_key` varchar(100) COLLATE utf8mb4_unicode_ci NOT NULL,
  ...
  `shipping_company` varchar(50) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT \'快递公司代码（如 sf/yt/zt/yto）\',
  `shipping_company_name` varchar(100) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT \'快递公司名称（如 顺丰速运）\',
  `shipping_no` varchar(100) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT \'快递单号\',
  `approved_at` datetime DEFAULT NULL COMMENT \'审核通过时间（管理员审核通过的时间）\',
  `item_id` bigint DEFAULT NULL COMMENT \'本次兑换产出的物品实例（NULL=纯实物兑换无实例）\',
  `sku_id` bigint DEFAULT NULL COMMENT \'关联统一SKU（新商品中心）\',
  ...
  KEY `idx_exchange_records_shipping_no` (`shipping_no`),
  KEY `idx_exchange_records_item` (`item_id`),
  KEY `exchange_records_sku_id_fk` (`sku_id`),
  CONSTRAINT `exchange_records_exchange_item_id_fk` FOREIGN KEY (`exchange_item_id`) REFERENCES `exchange_items` (`exchange_item_id`),
  CONSTRAINT `exchange_records_ibfk_1` FOREIGN KEY (`user_id`) REFERENCES `users` (`user_id`) ON DELETE RESTRICT,
  CONSTRAINT `exchange_records_item_id_foreign_idx` FOREIGN KEY (`item_id`) REFERENCES `items` (`item_id`) ON DELETE SET NULL ON UPDATE CASCADE,
  CONSTRAINT `exchange_records_sku_id_fk` FOREIGN KEY (`sku_id`) REFERENCES `exchange_item_skus` (`sku_id`)
) ENGINE=InnoDB AUTO_INCREMENT=697 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'兑换市场记录表\'', { transaction })

      // [53/115] feature_flags
...
) ENGINE=InnoDB AUTO_INCREMENT=2753 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'功能开关表（Feature Flag）- 全系统通用灰度发布控制\'', { transaction })
...
) ENGINE=InnoDB AUTO_INCREMENT=274 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'用户反馈表 - 支持客服反馈功能\'', { transaction })
...
) ENGINE=InnoDB AUTO_INCREMENT=1441 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'物品锁定记录表（替代 item_instances.locks JSON 字段，可索引、可查询）\'', { transaction })
...
) ENGINE=InnoDB AUTO_INCREMENT=28068 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'物品所有权账本（唯一真相，双录记账，只追加不修改不删除）\'', { transaction })

      // [57/115] item_templates
  ...
  CONSTRAINT `item_templates_primary_media_id_foreign_idx` FOREIGN KEY (`primary_media_id`) REFERENCES `media_files` (`media_id`) ON DELETE SET NULL ON UPDATE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=200 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'物品模板表（Item Templates - 不可叠加物品模板定义）\'', { transaction })
...
) ENGINE=InnoDB AUTO_INCREMENT=40057 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'物品表（当前状态缓存，可从 item_ledger 重建）\'', { transaction })

      // [59/115] lottery_alerts
...
) ENGINE=InnoDB AUTO_INCREMENT=176 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'抽奖系统告警表 - 运营监控专用（独立于商家风控的 risk_alerts）\'', { transaction })

      // [60/115] lottery_campaign_pricing_config
  ...
  CONSTRAINT `fk_pricing_config_updater` FOREIGN KEY (`updated_by`) REFERENCES `users` (`user_id`) ON DELETE SET NULL ON UPDATE CASCADE
      ...
      // [61/115] lottery_campaign_quota_grants
  ...
  KEY `idx_grants_source_time` (`grant_source`,`created_at`)
      ...
      // [62/115] lottery_campaign_user_quota
...
) ENGINE=InnoDB AUTO_INCREMENT=50 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'抽奖活动配置表\'', { transaction })

      // [64/115] lottery_daily_metrics
...
) ENGINE=InnoDB AUTO_INCREMENT=34 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'抽奖日报统计表（按日聚合，永久保留，用于长期历史分析）\'', { transaction })

      // [65/115] lottery_draw_decisions
...
) ENGINE=InnoDB AUTO_INCREMENT=5546 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'抽奖决策快照表 - 记录每次抽奖的完整决策路径用于审计\'', { transaction })

      // [66/115] lottery_draw_quota_rules
...
) ENGINE=InnoDB AUTO_INCREMENT=15 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci', { transaction })

      // [67/115] lottery_draws
  ...
  `draw_seq` int unsigned NOT NULL AUTO_INCREMENT,
  `order_no` varchar(32) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT \'抽奖订单号（LT 前缀）\',
  ...
  UNIQUE KEY `draw_seq` (`draw_seq`),
  UNIQUE KEY `uk_lottery_draws_order_no` (`order_no`),
  KEY `idx_user_id` (`user_id`),
...
) ENGINE=InnoDB AUTO_INCREMENT=1527 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci', { transaction })

      // [68/115] lottery_hourly_metrics
...
) ENGINE=InnoDB AUTO_INCREMENT=149 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'抽奖监控指标表（按小时聚合，用于监控和分析）\'', { transaction })

      // [69/115] lottery_management_settings
  ...
  `budget_cost` int NOT NULL DEFAULT \'0\' COMMENT \'奖品总预算成本（过滤+扣减用），pvp 仅管分层阈值\',
  ...
  KEY `lottery_prizes_primary_media_id_foreign_idx` (`primary_media_id`),
  ...
  CONSTRAINT `lottery_prizes_primary_media_id_foreign_idx` FOREIGN KEY (`primary_media_id`) REFERENCES `media_files` (`media_id`) ON DELETE SET NULL ON UPDATE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=210 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci', { transaction })

      // [72/115] lottery_simulation_records
...
) ENGINE=InnoDB AUTO_INCREMENT=273 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'策略效果模拟记录表 — 保存模拟参数、结果、对比、风险评估和偏差追踪\'', { transaction })

      // [73/115] lottery_strategy_config
...
) ENGINE=InnoDB AUTO_INCREMENT=163 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'抽奖策略全局配置表（Budget Tier阈值/Pity配置/功能开关等）\'', { transaction })

      // [74/115] lottery_tier_matrix_config
...
) ENGINE=InnoDB AUTO_INCREMENT=31 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'BxPx矩阵配置表（Budget Tier × Pressure Tier 组合的乘数配置）\'', { transaction })

      // [75/115] lottery_tier_rules
...
) ENGINE=InnoDB AUTO_INCREMENT=16 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'抽奖档位规则表 - 定义各分层用户的档位概率（整数权重制）\'', { transaction })

      // [76/115] lottery_user_daily_draw_quota
  ...
  KEY `idx_user_id` (`user_id`)
) ENGINE=InnoDB AUTO_INCREMENT=351 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci', { transaction })

      // [77/115] lottery_user_experience_state
...
) ENGINE=InnoDB AUTO_INCREMENT=52 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'用户活动级抽奖体验状态表（Pity/AntiEmpty/AntiHigh）\'', { transaction })

      // [78/115] lottery_user_global_state
  ...
  `sort_order` int NOT NULL DEFAULT \'0\' COMMENT \'运营排序权重（数值越小越靠前）\',
  `is_pinned` tinyint(1) NOT NULL DEFAULT \'0\' COMMENT \'是否置顶\',
  `pinned_at` datetime DEFAULT NULL COMMENT \'置顶时间\',
  `is_recommended` tinyint(1) NOT NULL DEFAULT \'0\' COMMENT \'是否推荐（前端「推荐」标签）\',
  ...
  `idempotency_key` varchar(100) COLLATE utf8mb4_unicode_ci NOT NULL,
  `offer_category_id` int DEFAULT NULL,
  ...
  KEY `idx_market_listings_status_kind_category` (`status`,`listing_kind`),
  ...
  KEY `fk_market_listings_offer_category` (`offer_category_id`),
  CONSTRAINT `fk_market_listings_offer_category` FOREIGN KEY (`offer_category_id`) REFERENCES `categories` (`category_id`) ON DELETE SET NULL ON UPDATE CASCADE,
...
) ENGINE=InnoDB AUTO_INCREMENT=11533 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci', { transaction })

      // [80/115] market_price_snapshots
...
) ENGINE=InnoDB AUTO_INCREMENT=5 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'市场价格快照预聚合表（每日一次，按资产+类型+币种维度）\'', { transaction })

      // [81/115] material_asset_types
...
) ENGINE=InnoDB AUTO_INCREMENT=1181 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci', { transaction })

      // [82/115] material_conversion_rules
...
) ENGINE=InnoDB AUTO_INCREMENT=69 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci', { transaction })

      // [83/115] media_attachments
      await queryInterface.sequelize.query('CREATE TABLE IF NOT EXISTS `media_attachments` (
...
) ENGINE=InnoDB AUTO_INCREMENT=31 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'媒体关联表（多态关联 - 独立媒体服务方案 D+ 增强版）\'', { transaction })

      // [84/115] media_files
      await queryInterface.sequelize.query('CREATE TABLE IF NOT EXISTS `media_files` (
...
) ENGINE=InnoDB AUTO_INCREMENT=30 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'媒体文件表（纯存储层 - 独立媒体服务方案 D+ 增强版）\'', { transaction })

      // [85/115] merchant_operation_logs
...
) ENGINE=InnoDB AUTO_INCREMENT=252 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'商家操作审计日志表（商家员工域权限体系升级 - 2026-01-12）\'', { transaction })
...
) ENGINE=InnoDB AUTO_INCREMENT=7 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'商家信息表（多商家接入：餐厅/商铺/小游戏/服务商）\'', { transaction })

      // [87/115] preset_budget_debt
...
) ENGINE=InnoDB AUTO_INCREMENT=1379 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'兑换订单表（Redemption Orders）：管理核销码生成和核销流程，替代 UserInventory.verification_code\'', { transaction })

      // [92/115] reminder_history
...
) ENGINE=InnoDB AUTO_INCREMENT=1298 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'提醒历史记录表（存储每次提醒的触发和通知结果）\'', { transaction })

      // [93/115] reminder_rules
...
) ENGINE=InnoDB AUTO_INCREMENT=93 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci', { transaction })
...
) ENGINE=InnoDB AUTO_INCREMENT=202 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'角色管理表\'', { transaction })

      // [97/115] segment_rule_configs
...
) ENGINE=InnoDB AUTO_INCREMENT=6 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'用户分群规则配置表（运营可视化搭建分群条件）\'', { transaction })

      // [98/115] sequelizemeta
      await queryInterface.sequelize.query('CREATE TABLE IF NOT EXISTS `sequelizemeta` (
  `name` varchar(255) COLLATE utf8mb3_unicode_ci NOT NULL,
  PRIMARY KEY (`name`),
  UNIQUE KEY `name` (`name`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_unicode_ci', { transaction })

      // [99/115] sku_attribute_values
      await queryInterface.sequelize.query('CREATE TABLE IF NOT EXISTS `sku_attribute_values` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT \'主键\',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'SKU销售属性值（替代JSON spec_values，支持SQL级属性筛选）\'', { transaction })
...
) ENGINE=InnoDB AUTO_INCREMENT=9 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'门店员工关系表（员工-门店多对多，支持历史记录）\'', { transaction })
...
) ENGINE=InnoDB AUTO_INCREMENT=565 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'门店信息表（用于记录合作商家门店，业务员分派依据）\'', { transaction })

      // [102/115] system_dictionaries
...
) ENGINE=InnoDB AUTO_INCREMENT=454 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'系统字典表 - 存储各类枚举的中文显示名称映射\'', { transaction })

      // [103/115] system_dictionary_history
...
) ENGINE=InnoDB AUTO_INCREMENT=85 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'系统设置表：存储系统各模块的配置设置\'', { transaction })

      // [105/115] trade_orders
  ...
  `status` enum(\'created\',\'frozen\',\'completed\',\'cancelled\',\'failed\',\'disputed\') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT \'created\',
  ...
  `order_no` varchar(32) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT \'交易订单号（TO 前缀，统一生成器）\',
...
) ENGINE=InnoDB AUTO_INCREMENT=7375 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci', { transaction })

      // [106/115] user_ad_tags
...
) ENGINE=InnoDB AUTO_INCREMENT=6 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'用户行为标签表 — Phase 5 DMP 人群定向\'', { transaction })

      // [107/115] user_behavior_tracks
...
) ENGINE=InnoDB AUTO_INCREMENT=5289 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'用户通知表 — 系统通知独立存储（方案B），永久保留交易凭证\'', { transaction })

      // [110/115] user_premium_status
  ...
  CONSTRAINT `fk_ups_user_id` FOREIGN KEY (`user_id`) REFERENCES `users` (`user_id`) ON DELETE CASCADE ON UPDATE CASCADE
      ...
      // [111/115] user_ratio_overrides
      await queryInterface.sequelize.query('CREATE TABLE IF NOT EXISTS `user_ratio_overrides` (
...
) ENGINE=InnoDB AUTO_INCREMENT=3 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'用户消费比例覆盖表（三个消费比例均支持全局+个人两层配置）\'', { transaction })

      // [112/115] user_risk_profiles
...
) ENGINE=InnoDB AUTO_INCREMENT=9 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'用户风控配置表：存储用户等级默认配置和个人自定义配置\'', { transaction })
...
) ENGINE=InnoDB AUTO_INCREMENT=756 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci', { transaction })
...
) ENGINE=InnoDB AUTO_INCREMENT=12218 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci', { transaction })

      // [115/115] websocket_startup_logs
...
) ENGINE=InnoDB AUTO_INCREMENT=2885 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT=\'WebSocket服务启动日志表（记录所有启动/停止事件）\'', { transaction })

      await transaction.commit()
      console.log('[baseline-v4.1] All 115 tables created successfully')
    } catch (error) {
  ...
  async down(queryInterface, Sequelize) {
    ...
    try {
      ...
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `websocket_startup_logs`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `users`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `user_roles`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `user_risk_profiles`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `user_ratio_overrides`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `user_premium_status`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `user_notifications`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `user_hierarchy`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `user_behavior_tracks`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `user_ad_tags`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `trade_orders`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `system_settings`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `system_dictionary_history`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `system_dictionaries`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `stores`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `store_staff`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `sku_attribute_values`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `sequelizemeta`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `segment_rule_configs`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `roles`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `risk_alerts`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `report_templates`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `reminder_rules`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `reminder_history`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `redemption_orders`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `rarity_defs`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `preset_inventory_debt`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `preset_debt_limits`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `preset_budget_debt`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `merchants`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `merchant_operation_logs`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `media_files`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `media_attachments`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `material_conversion_rules`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `material_asset_types`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `market_price_snapshots`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `market_listings`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `lottery_user_global_state`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `lottery_user_experience_state`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `lottery_user_daily_draw_quota`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `lottery_tier_rules`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `lottery_tier_matrix_config`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `lottery_strategy_config`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `lottery_simulation_records`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `lottery_prizes`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `lottery_presets`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `lottery_management_settings`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `lottery_hourly_metrics`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `lottery_draws`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `lottery_draw_quota_rules`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `lottery_draw_decisions`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `lottery_daily_metrics`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `lottery_campaigns`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `lottery_campaign_user_quota`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `lottery_campaign_quota_grants`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `lottery_campaign_pricing_config`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `lottery_alerts`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `items`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `item_templates`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `item_ledger`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `item_holds`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `feedbacks`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `feature_flags`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `exchange_records`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `exchange_rates`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `exchange_order_events`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `exchange_items`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `exchange_item_skus`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `exchange_item_attribute_values`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `exchange_channel_prices`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `customer_service_user_assignments`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `customer_service_sessions`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `customer_service_notes`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `customer_service_issues`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `customer_service_agents`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `content_review_records`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `consumption_records`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `chat_messages`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `category_attributes`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `categories`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `bid_records`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `bid_products`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `batch_operation_logs`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `authentication_sessions`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `attributes`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `attribute_options`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `asset_transactions`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `asset_group_defs`', { transaction })
      ...
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `api_idempotency_requests`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `alert_silence_rules`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `administrative_regions`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `admin_operation_logs`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `admin_notifications`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `ad_zone_groups`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `ad_zone_group_members`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `ad_target_zones`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `ad_slots`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `ad_report_daily_snapshots`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `ad_price_adjustment_logs`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `ad_interaction_logs`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `ad_impression_logs`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `ad_dau_daily_stats`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `ad_creatives`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `ad_click_logs`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `ad_campaigns`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `ad_billing_records`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `ad_bid_logs`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `ad_attribution_logs`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `ad_antifraud_logs`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `accounts`', { transaction })
      await queryInterface.sequelize.query('DROP TABLE IF EXISTS `account_asset_balances`', { transaction })

      await queryInterface.sequelize.query('SET FOREIGN_KEY_CHECKS = 1', { transaction })
      await transaction.commit()
      console.log('[baseline-v4.1] All tables dropped')
    } catch (error) {
      await transaction.rollback()
      throw error
    }
