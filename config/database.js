...
const dbConfig = {
  ...
  pool: {
    /*
     * 最大连接数（R5 万级并发优化方案B，2026-05-30）
     * 从单进程 40 调整为每进程 20：进入 PM2 cluster 多进程后，连接池语义变为「每 worker 各自一份」。
     *   - 4 worker × 20 = 80 总连接（加到 8 worker 时 8 × 20 = 160），远低于 MySQL max_connections=2400，彻底杜绝连接风暴
     *   - 单进程语义下「固定40」的旧结论（2025-12-30）已被本方案显式覆盖：多进程下 40×4=160 反而偏大，20 更稳健
     *   - 单进程 `npm run dev` 下 20 连接对开发期低流量完全够用
     */
    max: 20,
    min: 5, // ✅ 最小连接数 - 避免冷启动延迟
